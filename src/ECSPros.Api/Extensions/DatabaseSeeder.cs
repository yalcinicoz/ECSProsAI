...
""");
    ...
    private static async Task SeedPushTemplatesAsync(IServiceProvider sp)
    {
        ...
        var liste = new (string Type, string Class, string Name, string Title, string Body, string Link, int Ttl, string Desc)[]
        {
            ...
            ("favorite_back_in_stock", P, "Favori yeniden stokta", "Favorin yeniden stokta 🎉", "{productName} tekrar satışta — kaçırma.", "/urun/{productCode}", 21600, "Favorilenen rengin stoğu 0'dan pozitife döndü (\"gelince haber ver\" bildirimi gittiyse tekrarlanmaz; gün başına 1)"),
            ("cart_price_drop", P, "Sepette fiyat düşüşü", "Sepetindeki ürün ucuzladı 🔥", "{productName} şimdi {newPrice} (eski {oldPrice}).", "/sepet", 21600, "Sepetteki ürünün kart fiyatı, sepete eklendiği andakine göre ≥ %10 düştü"),
