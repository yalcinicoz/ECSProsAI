...
public sealed class ReturnReportExecutor(NpgsqlDataSource source, IEtkinYetkiServisi authorization)
{
    ...
    public void Validate(DynamicReportPlan plan, EfektifYetkiler effective)
    {
        ...
        if (plan.Detail is not null) _ = ReturnReportSource.Dictionary.Details(_ => true, m => m.Id).Build(rows, plan.Detail, new(), permissions);
        else _ = ReturnReportSource.Dictionary.Aggregates(_ => true).Build(rows, plan.Aggregate!, permissions);
    }
    public async Task<StockReportResult> ExecuteAsync(Guid userId, DynamicReportPlan plan, ReportGridState grid, CancellationToken ct)
    {
        ...
        var rows = ReturnReportSource.Apply(db.Returns, plan, effective, period);
        var permissions = StockReportExecutor.ResolvePermissions(effective);
        var detail = plan.Detail is null ? null : ReturnReportSource.Dictionary.Details(_ => true, m => m.Id).Build(rows, plan.Detail, grid, permissions);
        var aggregate = plan.Aggregate is null ? null : ReturnReportSource.Dictionary.Aggregates(_ => true).Build(rows, plan.Aggregate, permissions);
