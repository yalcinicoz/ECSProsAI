...
public sealed class ProductCardReportExecutor(NpgsqlDataSource source, IEtkinYetkiServisi authorization)
{
    ...
    public void Validate(DynamicReportPlan plan, EfektifYetkiler effective)
    {
        ...
        var rows = ProductCardReportSource.Query(db, plan, effective);
        var permissions = StockReportExecutor.ResolvePermissions(effective);
        if (plan.Detail is not null) _ = ProductCardReportSource.Dictionary.Details(_ => true, m => m.Id).Build(rows, plan.Detail, new(), permissions);
        else _ = ProductCardReportSource.Dictionary.Aggregates(_ => true).Build(rows, plan.Aggregate!, permissions);
    }
    public async Task<StockReportResult> ExecuteAsync(Guid userId, DynamicReportPlan plan, ReportGridState grid, CancellationToken ct)
    {
        ...
        var rows = ProductCardReportSource.Query(db, plan, effective, period);
        var permissions = StockReportExecutor.ResolvePermissions(effective);
        var detail = plan.Detail is null ? null : ProductCardReportSource.Dictionary.Details(_ => true, m => m.Id).Build(rows, plan.Detail, grid, permissions);
        var aggregate = plan.Aggregate is null ? null : ProductCardReportSource.Dictionary.Aggregates(_ => true).Build(rows, plan.Aggregate, permissions);
