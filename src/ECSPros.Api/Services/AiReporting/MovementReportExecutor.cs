...
public sealed class MovementReportExecutor(NpgsqlDataSource source, IEtkinYetkiServisi authorization)
{
    private static readonly SemaphoreSlim Slots = ReportExecutionBudget.Slots;
    private InventoryDbContext Context() => new(new DbContextOptionsBuilder<InventoryDbContext>().UseNpgsql(source).Options);
    public void Validate(DynamicReportPlan plan, EfektifYetkiler effective)
    {
        ...
        if (plan.Detail is not null) _ = MovementReportSource.Dictionary.Details(_ => true, m => m.Id).Build(rows, plan.Detail, new(), permissions);
        else _ = MovementReportSource.Dictionary.Aggregates(_ => true).Build(rows, plan.Aggregate!, permissions);
    }
    public async Task<StockReportResult> ExecuteAsync(Guid userId, DynamicReportPlan plan, ReportGridState grid, CancellationToken ct)
    {
        ...
        var rows = MovementReportSource.Apply(db.StockMovements, plan, effective, period);
        var permissions = StockReportExecutor.ResolvePermissions(effective);
        var detail = plan.Detail is null ? null : MovementReportSource.Dictionary.Details(_ => true, m => m.Id).Build(rows, plan.Detail, grid, permissions);
        var aggregate = plan.Aggregate is null ? null : MovementReportSource.Dictionary.Aggregates(_ => true).Build(rows, plan.Aggregate, permissions);
