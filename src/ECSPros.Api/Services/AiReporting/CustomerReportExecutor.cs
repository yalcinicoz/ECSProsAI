...
public sealed class CustomerReportExecutor(NpgsqlDataSource source, IEtkinYetkiServisi authorization)
{
    ...
    public void Validate(DynamicReportPlan plan, EfektifYetkiler effective)
    {
        ...
        if (plan.Detail is not null) _ = CustomerReportSource.Dictionary.Details(_ => true, m => m.Id).Build(rows, plan.Detail, new(), permissions);
        else _ = CustomerReportSource.Dictionary.Aggregates(_ => true).Build(rows, plan.Aggregate!, permissions);
    }
    public async Task<StockReportResult> ExecuteAsync(Guid userId, DynamicReportPlan plan, ReportGridState grid, CancellationToken ct)
    {
        ...
        var rows = CustomerReportSource.Query(db, plan, effective, period);
        var permissions = StockReportExecutor.ResolvePermissions(effective);
        var detail = plan.Detail is null ? null : CustomerReportSource.Dictionary.Details(_ => true, m => m.Id).Build(rows, plan.Detail, grid, permissions);
        var aggregate = plan.Aggregate is null ? null : CustomerReportSource.Dictionary.Aggregates(_ => true).Build(rows, plan.Aggregate, permissions);
