    ...
    ILogger<MarketplaceMappingService> logger)
{
    ...
    /// (özellik/değer eşlemesi değişince etkilenen kapsam).</summary>
    private async Task<List<Guid>> HedefKategoriGruplariAsync(string marketplace, string mpCategoryExternalId, CancellationToken ct)
        => await db.MarketplaceCategoryMappings
            .Where(m => m.Marketplace == marketplace && m.TargetExternalId == mpCategoryExternalId && !m.IsDeleted)
        ...
        string marketplace, List<BulkCategoryMappingItem> items, Guid? userId, CancellationToken ct)
    {
        ...
        foreach (var item in items)
        {
            ...
            var (dto, error) = await SaveCategoryMappingAsync(new SaveCategoryMappingRequest(
                ...
                hedef.ExternalId, hedef.Name, hedef.Path, null, null), userId, ct, readinessTetikle: false);
    ...
    public async Task<(CategoryMappingDto? Dto, string? Error)> SaveCategoryMappingAsync(
        SaveCategoryMappingRequest req, Guid? userId, CancellationToken ct, bool readinessTetikle = true)
    {
        ...
        string marketplace, string mpCategoryId, CancellationToken ct)
    {
        ...
        if (MappingTargets.IsErp(marketplace))
        {
            ...
            var types = await db.ErpReferenceItems
                .Where(i => i.TargetSystem == marketplace && i.Kind == ErpReferenceKinds.AttributeType && i.IsActive)
            ...
            foreach (var t in types)
                mpAttrs.Add((t.Code, t.Name, false, true, false, "list", valueCounts.GetValueOrDefault(t.Code)));
        }
        if (ds is not null)
        {
            await using var cmd = ds.CreateCommand(
        ...
        string target, string? kind, string? query, int limit, CancellationToken ct)
    {
        ...
        return await q.OrderBy(i => i.Kind).ThenBy(i => i.Name).Take(limit)
            .Select(i => new ErpReferenceItemDto(i.Id, i.TargetSystem, i.Kind, i.Code, i.Name, i.ParentCode, i.IsActive, i.Source, i.LastSeenAt,
                i.Kind == ErpReferenceKinds.ProductGroup && mappedSet.Contains(i.Code)))
            .ToListAsync(ct);
    }
        ...
        string target, string kind, string code, string name, string? parentCode, Guid? userId, CancellationToken ct)
    {
        ...
        await db.SaveChangesAsync(ct);
        return (new ErpReferenceItemDto(item.Id, item.TargetSystem, item.Kind, item.Code, item.Name, item.ParentCode, item.IsActive, item.Source, item.LastSeenAt, false), null);
    }
