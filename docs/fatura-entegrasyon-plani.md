...
> Sürüm: **v1.3 — 2026-09-06** · Durum: **FE0+FE1+FE2 CANLIDA/UYGULANDI, FE3 altyapı + FE4 UYGULANDI ⚠️ restart bekliyor**; K3+K8 kapalı; **açık: K1 (ilk entegratör — FE3 gerçek adaptör), K2, K4-K7 (FE5/FE7), K9-K11**
...
|---|---|---|
| **FE0** Model ✅ **UYGULANDI (2026-09-06)** — migration `20260906120838_TypedInvoiceSeriesAndChannelBindings` (Order) + `20260906120857_DropFirmPlatformInvoiceSeriesId` (Core) ecommerce_db + ecommerce_demo'ya uygulandı (demo'da bekleyen 4 eski migration da geldi); sayaç 50 eşzamanlı tahsis testi 1..50 boşluksuz ✓; izole 5051 açılış ✓; **DB düzeyi tip kilidi** `20260906123027_EnforceInvoiceSeriesTypeConsistency`: seri (Id, InvoiceType) tekil + fatura ve kanal yuvası bileşik FK (InvoiceSeriesId, InvoiceType) → yanlış tipte kesim/bağ VERİTABANINDA imkânsız (demo DB'de aynı firma ikinci tip → unique ihlali, e_archive serisi e_invoice yuvasına → FK ihlali, farklı firmada aynı harf → OK doğrulandı); sözleşme alanı FE3'e kadar OPSİYONEL (katalogda einvoice servisi yok); uçlar: `invoice-series` GET/POST/PUT, `/{id}/deactivate|activate`, `invoice-settings/channels[/{id}]` GET/PUT; panel: seri modalı tipli, sipariş fatura formu tipe göre süzer (seri sayfası/pasifleştirme/kanal Faturalama bölümü FE2) | Tekil **tipli** seri + sayaç + kanal bağ tablosu + `InvoiceSendMethod` + fatura kayıt genişletmesi; migration'da mevcut üçlü setler tekil serilere **ayrıştırılır** (her kolon kendi tipiyle: EArchiveSerial→e_archive, EInvoiceSerial→e_invoice, ExportSerial→export; **aynı harf birden fazla kolonda ise yalnız e_archive tipiyle taşınır, diğerleri boş bırakılıp panelde kırmızı uyarıyla elle tanımlanır** — TST/TST/TST bu durumdadır), mevcut tek fatura yeni seri Id'sine bağlanır; `LegacyInvoiceImportSlice` yeni tabloya uyarlanır (ekip arkadaşıyla) | İki DB'de migration temiz; eski `InvoiceSeriesId` alanı kaldırıldı; import dilimi dry-run `0` hata; tip uyumsuz bağ/kesim 400 |
| **FE1** Numara güvencesi + dış kayıt ✅ **UYGULANDI (2026-09-06, K3 varsayılan öneriyle)** — `InvoiceDateRules` (TR takvim günü: gelecek tarih yasak, serinin son fatura tarihinden geriye yasak, seride daha yeni yıl açıldıysa eski yıla yasak) CreateInvoice'da; `POST /api/orders/{orderId}/invoices/external` (RegisterExternalInvoice: kaynak erp|marketplace|integrator + kaynak kodu + numara + tip + tarih + ETTN/dış belge/PDF; idempotent (ExternalSource, InvoiceNumber) → alreadyExisted; alıcı/tutar siparişten; seri ve sayaç KULLANILMAZ); `GET invoice-series/{id}/gaps` boşluk denetimi; seri/yıl/sıra tekil indeksi yalnız NumberSource=internal için (migration `20260906133908_InvoiceInternalNumberUniqueOnly`, dev+demo DB); panel: sipariş detayında "Dış Fatura Kaydet" modalı, fatura listesi/sipariş kartında kaynak rozeti, seri düzenleme modalında Boşluk Denetimi. 50 eşzamanlı tahsis testi FE0'da DB düzeyinde geçti. | Kilitli sayaç, tarih/yıl kuralları, boşluk raporu, `invoices/external` ucu, idempotent anahtar | 50 eşzamanlı kesimde 1..50 boşluksuz; geçmiş tarihli istek 400; aynı dış numara ikinci kez 409/idempotent |
| **FE2** Seri/kanal operasyonu (panel) ✅ **UYGULANDI (2026-09-06)** — `/orders/invoice-series` sayfası (Sipariş Yönetimi menüsü): firma/tip/durum/arama filtreli seri tablosu (firma, seri, tip, ad, kullanan kanallar, son numara, son fatura tarihi, durum), satır tıkla → düzenle (ad/açıklama; sözleşme FE3), Pasife Al diyaloğu (kullanan kanallar + aynı tipte yerine-geçecek zorunlu), Aktifleştir, Yeni Seri modalı; altında Kanal Yuvaları tablosu (gönderim yöntemi + üç yuva, yalnız aynı firma+tip aktif seriler, eksik yuva uyarı rozeti, başlıkta "N aktif kanalda eksik yuva" sayacı). Faturalar sayfasındaki popup kaldırıldı → sayfaya link. Rehber sayfası SONRA. | Seri sayfası (tip sütunu + oluşturmada tip seçimi), pasifleştirme+yerine-geçecek (**yalnız aynı tipte** seri önerilir), kanal Faturalama bölümü (her yuva kendi tipindeki serileri listeler), readiness uyarısı | Kullanılan seri yerine seri verilmeden pasife alınamaz (400); farklı tipte yerine-geçecek 400; verilince kanallar taşınır; serisiz aktif kanal panelde kırmızı |
| **FE3** Entegratör kataloğu + sözleşme + ilk adaptör — **ALTYAPI UYGULANDI (2026-09-06), gerçek adaptör K1 bekliyor** — katalog `einvoice` servis tipi 6 kayıt (elogo, uyumsoft, foriba, edm, nilvera, izibiz; şema: kullanıcı/şifre (credentials), apiUrl, senderAlias (GB etiketi), testMode (boolean)); sözleşme = firma entegrasyon kaydı (mevcut FirmDetailPage formu şemadan üretir); `GET invoice-series/contracts` seçici; seri oluştur/güncelle sözleşme ZORUNLU (panelde seçici + "sözleşmesiz" rozeti); mevcut `IEInvoiceAdapter` (Integration) yeniden kullanıldı, `IsStub` bayrağı eklendi — eLogo taslağı IsStub=true (gerçek API yok, yalnız testMode sözleşmede simülasyon). Mükellef sorgusu (K5) ve ilk gerçek adaptör K1 ile. | `einvoice` servis kaydı + şema, firma sözleşmesi formu, `IEInvoiceProvider`, K1 adaptörü sandbox | Sandbox'ta e-arşiv gönder/iptal/durum/PDF geçer; mükellef sorgusu doğru tip seçer |
| **FE4** Gönderim kuyruğu + takip ✅ **UYGULANDI (2026-09-06), worker VARSAYILAN KAPALI** — `order.ord_invoice_dispatches` outbox (send/cancel; pending→done | dead (6 deneme, 1/5/30/120/360 dk) | blocked (sözleşme/adaptör yok, taslak adaptör + canlı sözleşme)); kesimde kanal `integrator_api` ise send işi aynı transaction'da, iptalde gönderilmiş faturaya cancel işi; `InvoiceDispatchWorker` (Api/Services/Invoicing; GenelWorkerRolu + DistributedWorkerLock; `InvoiceDispatch:Enabled=false` — gerçek adaptörle açılır); her deneme `integration.integration_logs`'a; uçlar `invoice-dispatches` (status/invoiceId), `/{id}/retry`, `invoices/{id}/resend`; panel: Faturalar → "Gönderim Kuyruğu" sekmesi (durum süzgeci, Tekrar Dene), fatura detayında entegratör durumu + zaman çizelgesi + "Entegratöre Gönder". | Outbox + worker + geri çekilme + ölü-mektup; Faturalar detay çekmecesi, Tekrar Dene, Hata Kuyruğu | Entegratör kapalıyken kesilen fatura kuyrukta bekler, açılınca gönderilir; 3 hatalı deneme sonrası ölü-mektupta görünür ve elle tekrar denenebilir |
...
| K3 | Tarih kuralı | **UYGULANDI (varsayılan öneri, 2026-09-06):** son tarihten geri olmaz, gelecek tarih yasak, eski yıla kesim yasak — kullanıcı itiraz ederse `InvoiceDateRules` tek noktadan değişir |
