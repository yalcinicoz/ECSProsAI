...
| D | Üye oturumu (Razor tarafı) + SMS/OTP altyapısı | 🔵 Sürüyor — D1+D2+D3+D4+D6 ✅ (HttpOnly cookie + SSR kimlik + logout/session iptali; kayıt belgeleri CMS + Member.Consents; SMS/OTP girişi canlı — crm.otp_codes + ISmsSender + otp/send+verify, SMS varsayılan sekme geri döndü); kalan: D5 BCrypt geçişi, D7 QA |
