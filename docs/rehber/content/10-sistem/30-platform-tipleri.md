...
| TİP | `Pazaryeri` ya da `Özel Kanal` rozeti. |
...
|---|---|---|
| Pazaryeri kanalı | Hayır | İşaretliyse tip `Pazaryeri` sayılır (kanal kartlarında çanta ikonu ve rozet). |
...
- Varsayılan tipler (Web Sitesi, Trendyol, Hepsiburada, n11, Amazon, Çiçeksepeti, Pazarama, Mobil Uygulama, Mağaza / POS) sistemle birlikte gelir.

## Adım adım
### Yeni pazaryeri tipi ekleme
1. Sistem › **Platform Tipleri**'nde **Yeni Platform Tipi**'ne tıklayın.
2. **Pazaryeri kanalı** kutusunu işaretleyin; kaynak dilde **Ad** yazın (kod otomatik oluşur).
