...
| **Filtrele kartı** (listenin üstünde, başlığına tıklayınca genişler; açık/kapalı durumu hatırlanır) | Kapsamlı filtre paneli. Alanlar: Ürün kodu, Ürün adı, Tedarikçi ürün kodu, Varyant barkodu (tam eşleşme ya da içerir), Ürün Grubu (yazarak aranabilir), Kaynak, Satışta, **Resim Durumu** (Yok / Var / Kısmi — renk bazlı), **Video Durumu** (Var / Yok — modelde en az bir aktif video), **Stok adedi** (fiziksel; `≥`, `≤`, `aralık` ile en az / en çok), Satılabilir stok, Liste fiyatı, Varyant sayısı, Oluşturma tarihi, Son görsel tarihi. Altında **Özellikler** bölümü: "Özellikler" seçicisinden bir özellik (örn. Kumaş), "Değerler" listesinden bir ya da çok değer (Tümünü seç / kaldır) → **Ekle** ile "Seçilen Özellikler" tablosuna girer (✕ ile silinir, arama kutusuyla süzülür). Aynı özellikteki değerler VEYA, farklı özellikler VE ile birleşir; ürün düzeyi ya da herhangi bir varyanttaki özellik eşleşir. Seçilen filtreler çip olarak görünür; "Tümünü temizle" hepsini kaldırır. |
