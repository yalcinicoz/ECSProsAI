...
|---|------|-----------|-------|-------------|---------------|
| 1 | 🌐 **Web sitesi** (Razor storefront) | `src/ECSPros.Api` Views + `/opt/misharix` tasarım | **FAZ H+İ TAMAM**; menü taşıma CANLIDA (2026-07-29); **2026-07-30 performans turu:** brotli SmallestSize→Fastest (~1.2sn TTFB düzeldi, CANLIDA) + statik cache başlıkları + HTTP/2 + **PageSpeed D+B+A + CLS düzeltmesi CANLIDA** (hero preload; panel görsel uyarıları; Magick.NET varyantlar+srcset; banner aspect-ratio kuralı + gerçek width/height — Lighthouse ana sayfa mobil 63→84, desktop 77→98, CLS 1.078→0.043); site 404 sayfası + vitrin link seçici CANLIDA; **Şimdi Al hızlı satın alma (A: ayrı tek ürünlük sepet, kupon dahil) UYGULANDI ⚠️ restart bekliyor** | Kullanıcı gerçek PSI ile teyit etsin; sonra ürün görselleri için aynı varyant hattı (istenirse) | `docs/misharix-razor-tasima-plani.md` |
