**▶ OTURUM SONU DURUMU (2026-09-10) — YENİ OTURUM BURADAN DEVAM ETSİN:** Bu oturumda alan **Mobil API**: mobil ekibin sepet raporu üzerine `GET /cart` `lineTotal`/`subtotal` NET yapıldı (ön izleme ile aynı) — ⚠️ **RESTART BEKLİYOR** (ecspros + ecspros-staging; publish + publish-staging 08:46'da alındı). Ardından **Admin panel** hatası: yetki grubunda kanal seçimi (aşağıdaki blok) — admin `npm run build` yapıldı, publish yenilendi (09:20). Mobil sırada M5. Önceki oturum (2026-09-09) alanı **Admin panel** (mobil M0-M4 kapandı ve CANLIDA). **DataGrid yaygınlaştırma turu sürüyor:** kullanıcı "tüm tablolar /admin/orders gibi olsun + mobil Tablo/Kompakt düğmesi her grid'de" dedi. Bitti: kompakt geçiş 17/17 sayfa, 17 sayfanın tüm veri kolonları sortable+filter (istemci `sortable:true` + sunucu `.Sort()` çiftleri), 35/38 liste sayfası DataGrid'e dönüştürüldü (Kuponlar, Talepler, Ürün Grupları, Depolar, Transferler, Yorum Moderasyonu, İletişim Mesajları, Cari Kartlar, Cari Grupları, Firmalar, Üye Grupları, Koleksiyon Moderasyonu, Tedarikçi Gönderimleri, Özellik Tipleri, İçerik Sayfaları, Bülten Aboneleri, Yetki Logları, Yetki İçerikleri, Satın Almalar, Kanal Ürünleri, Kanal Kategorileri, Bildirim İzleme, Mal Kabul, Kampanya Tipleri, Platform Tipleri, Servis Kataloğu, Yetki Grupları, Fatura Serileri, Resim Setleri, Arayüz Çevirileri, Kart Mesajları, Sayım/Teslim, Hakedişler, Numara Serileri, Pazaryeri Eşleştirme) + kalıcı istemci↔sunucu anahtar denetim testi (DataGridKeyConsistencyTests). **⚠️ RESTART BEKLİYOR** (ecspros + ecspros-staging); admin `npm run build` yapıldı. **DataGrid KAMPANYASI KAPANDI (35/38):** kullanıcı kararıyla üç sayfa kapsam dışı — Etiket Basımı (yerel basım sepeti), Tedarik Raporu (gruplu rapor), Talep Ayarları (ayar yükünün alt koleksiyonları). Kalan iş: rehber sayfası + kullanıcı gerçek veri testi — sıra bende (kullanım yoğunluğu + risk), ayrıntı ve kurallar `memory/project_datagrid_standardi_2026-09-08.md` 2026-09-09 turu bölümünde. ★ ALTIN KURAL: dropdown kaynağı olan liste ucunu SAYFALAMA, ayrı `/grid` ucu aç.

**ÜRÜN LİSTESİ ÖZELLİK FİLTRESİ (2026-09-11, Admin panel; eski /urun/urun-yonetim 'Özellikler / Değerler / Seçilen Özellikler' örnek):** Filtrele akordeonunun altında `AttributeFilter` bileşeni (özellik: aranabilir seçici, değerler: çoklu liste + Tümünü seç/kaldır + Ekle, seçilenler tablosu ✕/arama); grid filtresi `attrs` = `tipId:degerId,degerId;tipId2:degerId` (URL'de kalır, çip metni adlarla `GridFilterDef.chipText`); sunucu `ProductGrid.ApplyAll` özel işler (aynı özellik VEYA, özellikler arası VE — eski kuralla aynı; ürün düzeyi `product_attributes` ya da herhangi bir varyantın `product_variant_attributes` eşleşmesi). Kaynak `GET /catalog/attribute-types?activeOnly=true&includeCounts=false` (değerler dahil). `FilterAccordion.extra` yuvası eklendi. Testler geçti (grid tutarlılık 3/3; admin 85/85). `publish`/staging/demo eşitlendi (DLL 3aa769a4) + `admin/dist` ✅ ecspros restart edildi (kullanıcı, 2026-09-11). Rehber güncellendi. ★ 2. tur: 'Değerler' aranabilir çoklu seçici (`MultiSearchSelect`: arama + onay kutuları, Görünenleri seç / Tümünü kaldır), 'Seçilen Özellikler' tablosu 200px kaydırmalı (yapışkan başlık) — restart gerekmez. ★ 3. tur: Değerler açılır listesi akordeon kartının overflow-hidden'ı yüzünden kesiliyordu → createPortal ile body'de sabit konumlu (SearchableSelect kalıbı); SEO sekmesinde kanallar alt alta değil Satış Kanalları sekmesindeki gibi SEKMELİ (+ 'Varsayılan meta' sekmesi), ilk kanal seçili gelir — restart gerekmez. ★ 4. tur: Değerler listesi alt boşluk yetmezse YUKARI açılır; ürünler sayfasında aktif filtre çipleri yalnız filtre ADI (`DataGrid.chipsMode='label'`, diğer sayfalar etiket+değer); özellik filtresi özellik başına BİR çip 'Özellikler: Marka' (`GridFilterDef.chips`), ✕ o özelliğin tüm değerlerini kaldırır — restart gerekmez.

**DEPO DÜZEYİ 'ONLINE SATIŞA AÇIK' BAYRAĞI ETKİNLEŞTİRİLDİ (2026-09-11, Admin panel + Stok; kullanıcı sorusu 'stoğu internet satışına kapatma depo mu raf mı?'):** Tespit: karar KISIM seviyesindeydi; `Warehouse.IsSellableOnline` entity'de ve Depo formunda vardı ama hiçbir stok hesabında okunmuyordu (yalnız `w.IsActive`), raf/gözde bayrak yok. Kullanıcı 'depodaki flag'i aktif hale getirebilir miyiz' dedi → kural: stok sitede/kanalda sayılır ancak **depo satışa açık + aktif VE kısım satışa açık** ise (depo = üst kapı; kapatınca kısım ayarlarına dokunmadan tüm depo düşer, açınca kısım ayarları geri geçerli). Uygulandı: 7 hesap noktasına `w.IsSellableOnline` eklendi — `StockOps` (satılabilir stok tekil/toplu + REZERVASYON raf seçimi), `InStockProductProvider` (4 SQL: ürün/varyant stokta-seti, kanal net stok, min-stok), `GetStoreProductDetailHandler`, `MarketplaceAdminService` pazaryeri stok, `FeedProductReader` reklam feed; domain/`IChannelStockCalculator` yorumları; admin Depo formunda yeni depo varsayılanı açık (önce KAPALI başlıyordu) + açıklama; rehber `07-stok/10-depolar.md` + /rehber derlendi. Canlı+demo DB doğrulandı: tüm depolar (MERKEZ/AYAKKABI/MAGAZA/SUPPLIER_STOCK) bayrağı AÇIK → site stoğu değişmez. Build ✓, tsc ✓, admin build ✓; `publish`/staging/demo eşitlendi (DLL 62ec3eac) ✅ restart edildi (kullanıcı, 2026-09-11 19:54; GitHub push b200ae8). Bilinen sınır: stok setleri 2 dk IMemoryCache'li → depo kapatılınca site en geç 2 dk sonra yansıtır; migration YOK.

**ÜRÜN DETAYI: RESİM SETİ ADI + GÖRSELLER YERLEŞİMİ + KANAL BAZLI SEO (2026-09-11, Admin panel + Web sitesi; kullanıcı bildirimi):** (1) 'Tüm görseller julude setinde' KÖK NEDEN: eski sistemin `dfresimsetleri` tablosunda tek set 'Julude' adıyla vardı, MigrationTool bunu birebir aktardı (212.530 resim + 866 video o sette; 'Varsayılan' set boş); vitrin setleri hiç okumaz, yalnız panel gruplaması → canlı DB'de set `standart` / 'Standart Resim Seti' olarak yeniden adlandırıldı (id değişmedi; demo'da set yoktu). (2) Görseller: Mevcut Resimler / Mevcut Videolar ÜSTTE, hedef seçimi + yükleme elemanları ALTTA. (3) **SEO kanal bazlı**: sitede ürün URL'si `channel_variants.Slug`'tan (kanal + renk; legacy `{ad}-{eskiAnaVaryantId}`) üretilir, ürün kaydındaki 'URL Slug'/meta alanları sitede HİÇ okunmuyordu → `channel_products.MetaTitleI18n/MetaDescriptionI18n` eklendi (migration `20260911135947_AddChannelProductSeo`, canlı+demo UYGULANDI); `GetChannelProductSeoQuery` (kanal meta ?? ürün meta) web detayında `<title>`/meta description'a bağlandı (`UrunDetayVm.MetaBaslik/MetaAciklama`); admin `GET/PUT /api/catalog/products/{id}/channel-seo[/{firmPlatformId}]` (`Api/Handlers/ProductChannelSeoHandlers`: renk başına slug, rengin TÜM bedenlerine yazılır, kanal içi benzersizlik, Normalize); SEO sekmesi kanal kartları (URL satırları + Aç + meta) + en altta 'Varsayılan meta'; 'URL Slug' alanı kaldırıldı (ürün slug'ı DB'de duruyor, kullanılmıyor). Rehber ürün detay sayfası güncellendi. Testler 14/14, admin 85/85. `publish`/staging/demo eşitlendi (DLL 9eb34777) + `admin/dist` ✅ ecspros restart edildi (kullanıcı, 2026-09-11). Bilinen sınırlar: slug değişince eski adrese 301 YOK; mobil detay meta alanlarını henüz taşımıyor; kanal kartı yalnız kanalda tanımlı (channel_products/slug'ı olan) kanalları listeler.

**ÜRÜN DETAYI GÖRSELLER + STOK SEKMESİ (2026-09-11, Admin panel; kullanıcı isteği):** (1) Görseller › Mevcut Resimler: sekme sırası renkler önce, **'Ürün Geneli' en sonda**; sayfa açılınca **ilk renk seçili** gelir (`activeViewTabState` '' → `viewTabs[0]`). (2) Stok sekmesindeki raf bazlı tablo **DataGrid (useLocalGrid)** oldu — DEPO/KISIM/RAF/VARYANT/TOPLAM/REZERVE/KULLANILABİLİR başlıklarında sıralama + filtre, arama kutusu, mobil kompakt; satır kümesi ürünün tüm rafları (sunucuda sayfalanmaz) → `DataGridKeyConsistencyTests.YerelGridSayfalari`'na eklendi. Admin derlendi, restart GEREKMEZ.

**TARİH FİLTRESİ 'ÖZEL ARALIK' DÜZELTMESİ (2026-09-11, Admin panel; kullanıcı bildirimi ürünler 'Son görsel tarihi' + siparişler TARİH başlığı):** kök neden ortak `grid/FilterBar.tsx DateField`: seçim kutusu yalnız UYGULANMIŞ filtreden türetiliyordu; 'Özel aralık' iki tarih girilmeden filtre oluşmadığından kutu 'Tümü'ye dönüyor, tarih seçiciler çıkmıyordu (önce hızlı seçim yapılınca cur oluştuğu için görünüyordu). Düzeltme: mod yerel durumda (`mode/a/b`), dışarıdan gelen değişiklik (çip kaldırma/URL/Temizle) senkronlanır; 'Özel aralık' seçilince seçiciler hemen görünür, iki tarih girilince uygulanır. Tek bileşen olduğundan TÜM grid sayfalarındaki tarih filtreleri (başlık filtresi, hızlı çubuk, gelişmiş/akordeon, mobil sheet) düzeldi; başka tarih filtresi kopyası yok (`ChannelCapabilities` 'custom' ilgisiz). Admin derlendi, restart GEREKMEZ.

**ŞANS OYUNLARI BACKEND + PANEL (2026-09-11, Mobil API + Admin panel; mobil ekibin `docs/BACKEND_OYUNLAR.md` rev.2 sözleşmesi UYGULANDI):** Çarkıfelek/Salla Kazan/Kazı Kazan. Uçlar: `GET /api/store/games` (cihaz/üye token'ı; misafir `login_required`), `POST /api/store/games/{code}/play` (yalnız üye; misafir 401 JSON; oyun bitti/tanımsız 409), `GET /api/store/account/games/history`. Panel: Pazarlama › Şans Oyunları (`/promotion/games` GameGrid liste + detay Genel/Ödüller/Metinler/Oynanışlar; sidebar ikon `dice`), admin uçları `api/promotion/games*` (PromotionView/Manage, audit EntityType=Game). Veri: `promotion.prm_games/prm_game_prizes/prm_game_plays` — migration `20260911123612_AddGames` **canlı + demo DB'ye UYGULANDI**. Kural TEK yer `Promotion.Application/Games/SansOyunuKurali` (dönem anahtarı İstanbul günü/ISO hafta/total, durum+etiket şablonları {n}/{next}/{prize}, ağırlıklı ödül seçimi, kazı kazan 6 hücre kurgusu kazanan=tam 3 / kaybeden=hiçbiri 3 kez, Dogrula); oynatma `Api/Handlers/PlayGameHandler` (kupon = kişiye özel tek kullanımlık `OYUNKODU+DEĞER-XXXX`, GET /account/coupons'ta görünür; puan = CRM LoyaltyAccount earn; ödül YANITTAN ÖNCE işlenir; hak bitince aynı playId+sonuç döner = idempotent). Kararlar `docs/BACKEND_OYUNLAR_CEVAP.md` (§6: misafir oynayamaz, hak kuralı panelden, kupon aynı sözleşme, renk panelden, kaybeden kurgu ≤2 tekrar, imageUrl URL alanı; BACKEND_OYUNLAR.md root'a ait → ayrı dosya). Referans `docs/mobil-api-referansi.md` §17, rehber `08-pazarlama/35-sans-oyunlari.md`. Testler: `SansOyunuKuraliTests` 5/5 + grid/yetki 20/20; admin node 85/85 (sidebar testine FAZ 15 sayfaları da eklendi — teammate commit'inden beri kırıktı). **İzole 5051 (demo DB, DevBypass) uçtan uca:** alwaysWin+pas ve scratch<3 ödül → 400; misafir liste login_required, play 401; üye play → kupon E2ECAR50-B3Y2 Kuponlarım'da; ikinci play aynı playId + cooldown 'Yarın tekrar gel'; kazı kazan kazanan 3 hücre; history; oynanmış oyun silinemez; demo test verisi TEMİZLENDİ. `publish`/staging/demo eşitlendi (DLL e743ff38→2e19b6f6) + `admin/dist` ✅ ecspros restart edildi (kullanıcı, 2026-09-11). Bilinen sınırlar: ödül türü v1 coupon/points/none (free_shipping/product kupon motoru desteklemiyor), push senaryoları yok, görsel yükleme yok (URL). ★ Yan bulgu: `/api/store/auth/register` aynı telefonla 500 (IX_crm_members_Phone) — 400 olmalı, düzeltilmedi.

**KATEGORİ–DETAY–SEPET FİYATI TEK KURAL (2026-09-11, Web sitesi + Mobil API; kullanıcı bildirimi P-00018501/Bordo: kategori 249,99, detay+S 149,99):** Kesin kaynak MySQL'den doğrulandı (plurunler platform 41 Bordo ana varyantı 249,99 = rengin TÜM bedenleri; apurunler 249,99; varyant başına fiyat eski sistemde YOK) → doğru fiyat **249,99**, 149,99 bayat varyant tabanıydı. Üç kök neden: (a) detay 'en düşük varyant', kart 'en yüksek' kuralı; (b) Faz 27/LegacySync kanal fiyatını yalnız rengin ana varyantına (L) yazıyordu; (c) SyncBasePrice yalnız ürünü güncelliyor, varyant tabanı ilk aktarımda kalıyordu (3.005 own ürün / 42K varyant). Düzeltme: (1) `StoreUrunDetayBuilder` + `GetStoreProductDetailHandler` varsayılan fiyat = `KartFiyatGorunumu.KartTabanFiyati` (kartla birebir; beden seçimi/sepet varyantın kanal ?? taban); (2) `LegacySyncService.KardesYayilimiSql` + MigrationTool Faz 27 kardeş bedenlere yayılım, `SyncBasePriceAsync` + Faz 26 varyant tabanı ürünü izler; (3) canlı DB tek seferlik veri düzeltmesi UYGULANDI (tüm kanallar kardeş satırı +470.514, mishar 334.258 satır; varyant tabanı drift 0; P-00018501 12 varyant 249,99). Testler 10/10. Mobil referans A10 notu. `publish`/staging/demo eşitlendi (DLL d6d5f542→2e19b6f6) ✅ ecspros restart edildi (kullanıcı, 2026-09-11; kategori cache 10 dk). Bilinen sınır: tozlu/julude kanal fiyatları canlı senkronda DEĞİL (yalnız mishar), bir kez yayıldı.

**PUBLISH + ADMIN DERLEME (2026-09-11 öğleden sonra, kullanıcı isteği):** GitHub'dan ekip arkadaşının AI raporlama commit'i (6ae426a) alındıktan sonra `publish`/`publish-staging`/`publish-demo` eşitlendi (DLL 16f485f8, appsettings korunarak rsync) + `admin/dist` derlendi ✅ ecspros restart edildi (kullanıcı, 2026-09-11). Gelen commit'te migration ve yeni appsettings anahtarı YOK; `DatabaseSeeder`'a OpenAI servis kataloğu satırı eklendi (seeder yalnız Development'ta çalışır — canlıda `definition.integration_services` kaydı ekip arkadaşının kararıyla açılır). Tedarik Raporu sayfası/servisi bu commit'le kaldırıldı.

**ÜRÜN LİSTESİ KAPSAMLI FİLTRE (2026-09-11, Admin panel; kullanıcı isteği, eski `/urun/urun-yonetim` filtre seti örnek alındı):** (1) **DataGrid'e açılır/kapanır "Gelişmiş filtre" paneli** (★ 2. tur 2026-09-11: ürünlerde düğme DEĞİL, listenin üstünde başlığına tıklayınca genişleyen tam genişlik "Filtrele" kartı — eski /urun/urun-yonetim gibi; 3. tur: kart grid araç çubuğu satırında kaldığı için TAM genişlik olmuyordu → yeni `components/grid/FilterAccordion` sayfa düzeyinde, grid'in DIŞINDA/üstünde tam satır akordeon (başlangıçta kapalı, üst kenarı marka rengi, localStorage); DataGrid `advancedFilters.layout:'external'` = alanlar çip/mobil listesinde ama çubukta hiçbir şey çizilmez; 'button'/'card' kipleri diğer sayfalar için duruyor) ★ 4. tur (içerik): 'Görsel sayısı' KALDIRILDI; 'Resim Durumu' seçici Yok/Var/Kısmi (renk bazlı, aynı imageState); yeni 'Video Durumu' Var/Yok (`videoState` enum ← mv_product_stats.VideoCount = product_videos Status=Active; MV DROP/CREATE migration `20260911132227_RecreateProductStatsWithVideo` canlı+demo UYGULANDI, canlıda 652 videolu ürün); 'Ürün Grubu' aranabilir (`GridFilterDef.searchable` → SearchableSelect); export 'Video Durumu' kolonu. DLL 2e19b6f6 ✅ restart edildi (kullanıcı, 2026-09-11) (`DataGrid.advancedFilters` → `FilterBar.advanced`; alanlar ızgara formda `FieldRow stacked`, açık/kapalı `localStorage grid:<id>:adv`, çip + mobil sheet'e de girer, "Diğer filtreler"den düşer). (2) **Ürünler**: kod/ad/tedarikçi kodu/varyant barkodu (özel işlenen `barcode`: eq/contains, `ApplyFilters(...,"barcode")`), grup/kaynak/satışta, **Görsel durumu Var/Kısmi/Yok (RENK bazlı: grubun IsPrimaryAxis ekseni, bugün hep renk)**, **Stok adedi (fiziksel) + Satılabilir stok** min/maks (number between), fiyat, varyant/görsel sayısı, oluşturma + son görsel tarihi; yeni GÖRSEL ve STOK kolonları (sortable+filtre), export kolonları. (3) **Veri kaynağı: `catalog.mv_product_stats` MATERIALIZED VIEW** (migration `20260911111739_AddProductStatsMaterializedView`, SQL `Migrations/ProductStatsSql.cs`; unique idx → CONCURRENTLY; **canlı + demo DB'ye UYGULANDI**; ilk hesap canlıda full 15.496 / partial 1.765 / none 11.908) — tam hesap ~2-3 sn olduğundan her istekte değil: `ProductStatsRefreshWorker` (GenelWorkerRolu, açılış+1 dk, 5 dk'da bir, advisory lock `product-stats-refresh`, ~3,7 sn) + `GET /catalog/products/stats` (son yenileme) + `POST /catalog/products/stats/refresh` (CatalogProductsManage, panel "Şimdi yenile"). EF: `ProductStats` okuma modeli `ToTable("mv_product_stats", ExcludeFromMigrations)` + `Product.Stats` 1-1; şema ifadeleri `p.Stats != null ? … : none/0` (yeni ürün ilk yenilemeye kadar Yok/0). Rehber `02-katalog/10-urun-kartlari.md` güncellendi. Testler 16/16. `publish`/staging/demo eşitlendi (DLL 58b9a2a3) + `admin/dist` ✅ ecspros restart edildi (kullanıcı, 2026-09-11). Bilinen sınırlar: filtre 5 dk tazelikle çalışır; POS/legacy görsel yok; ★ yeni liste filtresi eklerken MV kolonuna ihtiyaç varsa `ProductStatsSql.Create` + `DROP/CREATE` migration'ı (MV `CREATE OR REPLACE` desteklemez).

**FATURALAR LİSTESİ YENİDEN KURGULANDI (2026-09-11, Admin panel; kullanıcı isteği):** satır tıklama popup'ı (`InvoiceModal`: PDF adresi girişi, Entegratöre Gönder, iptal, yazdır) KALDIRILDI; kolonlar: FATURA NO / SİPARİŞ NO (link) / FATURA TARİHİ / OLUŞTURMA / ETTN / VKN-TCKN / FATURA TİPİ / PARA BİRİMİ / TOPLAM TUTAR (Subtotal) / ÖDENECEK (GrandTotal) / VERGİ MATRAHI (Subtotal−İndirim) / VERGİ TOPLAMI / ENTEGRATÖR (durum+tarih+PDF ✓) / ERP (durum+tarih+ref) / PAZARYERİ (numberSource=marketplace → kaynak+belge no) / DURUM / sağda SABİT **Görüntüle** + **URL**. ★ URL penceresi YALNIZ entegratörün ürettiği fatura adresini gösterir (kullanıcı kararı 2026-09-11: bizim site adresleri değil; pazaryeri vb. yerlere bu adres verilir). ★ 2. tur: düğmeler yerine İKON (FileText=Görüntüle, Link2=URL; adres yoksa soluk), sağ sabitleme yalnız ≥768px (mobilde static), PAZARYERİ kolonu sıralanır+filtrelenir (`marketplace` = NumberSource=marketplace ? ExternalSource : null). DLL e1ea44ef. Backend: `InvoiceListDto` additive alanlar (orderNumber, ettn, recipientTaxNumber, currencyCode=Order.InvoiceCurrencyCode, subtotal/totalDiscount/totalTax, integratorSentAt, erpStatus/erpSentAt/erpReference, externalDocumentId, sendMethod, integratorInvoiceUrl — admin ucu yetkili, mağaza DTO'ları ayrı); `InvoiceGrid` şemasına orderNumber/currency/erpStatus/erpReference/subtotal/taxBase/totalTax filtre+sort; export kolonları ettn/currency/taxBase/erpStatus/erpSentAt/erpReference/integratorSentAt. Yeni `GET /api/orders/invoices/{id}/pdf` (OrdersInvoicesView + kanal kapsamı 404) → `GetInvoicePdfSourceQuery` + `FaturaPdfProxy` (mağaza ucunun panel karşılığı); panel bearer'lı olduğundan blob alıp yeni sekmede açar; entegratör PDF yoksa `/yazdir/fatura/{id}` açılır. DataGrid'e `GridColumn.frozenRight` (sticky right, `.grid-frozen-right` CSS) eklendi. **Fatura iptali sipariş detayı Faturalar kartına taşındı** (İptal Et, confirm). Panelden elle 'PDF adresi' girişi ve 'Entegratöre Gönder' (kuyruk sekmesinde Tekrar Dene var) artık YOK — kullanıcı isterse geri eklenir. Rehber `04-siparis/30-faturalar.md` güncellendi + derlendi. Testler 10/10. `publish`/staging/demo eşitlendi (DLL 66376a9f) + `admin/dist` ✅ restart edildi (kullanıcı, 2026-09-11) (ecspros).

**KAMPANYA FORMU İYİLEŞTİRMELERİ (2026-09-11, Admin panel; kullanıcı bildirimi 7 madde):** (1) **Kod alanı kaldırıldı** — sunucu `KMP-yyyyMMdd-XXXX` üretir (`CampaignRules.KodUretAsync`, silinmişler dahil benzersiz; `CreateCampaignRequest.Code` nullable). (2) **Ad zorunlu** — istemci (`kaydet()` Genel sekmesine döner) + sunucu (`CampaignRules.AdGecerli`, create+update). (3) **Silme**: yeni `DELETE /api/promotion/campaigns/{id}` (PromotionManage) → `ECSPros.Api/Handlers/DeleteCampaignHandler` (Api katmanı: Order modülü gerekir) — YALNIZ hiçbir siparişte kullanılmamış kampanya silinir (kullanım izi: `ord_orders.CustomerNotes->>'campaigns' LIKE '%[KOD]%'` + `OrderGift.CampaignId`; canlıda 1 sipariş kampanyalı), soft delete; panelde yayındaysa (aktif + tarih aralığı) confirm'de 'SİTEDE YAYINDA' uyarısı. (4) **Denetim logu**: `PromotionController` `IVitrinAuditLogger` (genel amaçlı) ile Created/Updated/Deleted → `iam.audit_logs` EntityType=Campaign, eski+yeni = tam `CampaignDetailDto` (Kopyala da Created). (5) **'Kaydediliyor...' takılması**: düğme metni yerel `saving` state'e bağlı, `onSettled` sıfırlar. (6) **Kargo 'Ücretsiz' iken İndirim değeri görünüyordu**: kök neden tip seçilince şema varsayılanları settings'e yazılmıyordu → `schemaDefaults()` + `fieldVisible` etkin değer (girilen ?? varsayılan). (7) **ⓘ bilgi ikonu**: yeni `components/ui/InfoTip.tsx` (tıkla-aç, dışarı/Esc kapat; label İÇİNE KONMAZ), `Lbl` sarmalayıcı; Genel/Kapsam açıklamaları `GENEL_ACIKLAMA`, parametreler şema `helpI18n` ?? `PARAM_ACIKLAMA` (27 anahtar). Rehber `08-pazarlama/10-kampanyalar.md` güncellendi + derlendi. ★ 2. tur (2026-09-11): kampanya listesinde KOD kolonu KALDIRILDI (AD sabit ilk kolon, rozet filtresi AD'a taşındı; arama sunucuda kodu da tarar) — panel + rehber, restart gerekmez. Testler 19/19 (YetkiKaplama + kampanya). `publish`/staging/demo eşitlendi (DLL 7eeaeb7b, appsettings korunarak) + `admin/dist` ✅ restart edildi (kullanıcı, 2026-09-11) (ecspros). Bilinen sınır: kullanım kontrolü POS/legacy siparişlerini kapsamaz (oralarda kampanya izi yok); `/settings/audit-logs` listesi Campaign kayıtlarını gösterir ama vitrin geçmişi ekranı (`/api/pages/audit-logs`) göstermez (EntityTypes süzgeci).

**KULLANICI ROLÜ = SÜPER ADMİN / ÇALIŞAN (2026-09-11, Admin panel; kullanıcı bildirimi "popup'taki rolün yetki gruplarıyla ilgisi yok, 2 rol olsun"):** `/settings/users` popup'ındaki eski "Rol Ata" (legacy `POST /iam/users/{id}/roles`, aynı `iam_roles` tablosuna grup ekliyordu) KALDIRILDI. Yerine **Rol** seçici: **Süper Admin** (K5 sistem bayrağı `IsSuperAdmin`, `PUT /iam/users/{id}/super-admin`; yalnız değiştiyse çağrılır) / **Çalışan** (yetkilerini gruplardan alır). Seçici yalnız süper admin'e açık, kendi rolü kilitli (sunucu kuralları: yalnız süper admin verir/kaldırır, kendi bayrağını kaldıramaz, en az bir süper admin kalır). Yeni kullanıcıda da seçilir (oluştur → bayrak). Popup'ta **Yetki Grupları** salt-okunur + "Yetkileri düzenle →" (`/settings/users/:id/permissions`). Liste: yeni **ROL** kolonu (`isSuperAdmin` bool filtre + sıralama; UserGrid şeması + export "Rol"), eski ROLLER kolonu **YETKİ GRUPLARI** oldu (role filtresi aynı). Backend: `UserListDto.IsSuperAdmin` (additive). Testler 40/40 (DataGridKeyConsistency + IAM). `publish`/`publish-staging`/`publish-demo` eşitlendi (DLL e9a29b6f, appsettings korunarak rsync) + `admin/dist` derlendi ⚠️ **RESTART BEKLİYOR** (ecspros; staging/demo isteğe bağlı). Bilinen sınır: "Çalışan" seçmek kullanıcıyı `super_admin` GRUBUNDAN çıkarmaz (grup üyeliği Yetkiler ekranında); `/settings/roles` sayfası ve legacy rol uçları dokunulmadı.

**ÜRÜN DETAYI STOK SEKMESİ DÜZELTMESİ (2026-09-11, Admin panel; kullanıcı bildirimi):** (1) "Stok Hareketi" düğmesi genel `/inventory/stocks` yerine `/inventory/stocks?search=<ürün kodu>` açar — stok sayfası URL'deki `search` ile ürüne filtreli gelir. (2) Sekmedeki liste depo bazlı `/inventory/stocks?variantId=` yerine raf bilgili `/inventory/stocks/admin-list?search=<kod>&pageSize=250` (sayfalar tükenene kadar, satırlar ürünün varyantlarına daraltılır — arama "içerir" eşleşmesi) → kolonlar DEPO / KISIM / RAF / VARYANT / TOPLAM / REZERVE / KULLANILABİLİR; varyantlar sekmesindeki mevcut toplamı aynı veriden. Backend değişmedi; `admin/dist` derlendi, restart GEREKMEZ. Commit a99c937. **Kullanıcı doğruladı: çalışıyor (2026-09-11). Aynı gün `ecspros` restart edildi → önceki oturumlardan restart bekleyen publish'ler (sepet NET tutar, taksit tablosu, iade akışı, kart fiyatı vb.) canlıda; `ecspros-staging` + `ecspros-demo` da restart edildi (kullanıcı, 2026-09-11) → üç örnek de güncel binary'de, BEKLEYEN RESTART YOK.**

**PANEL — SATIŞ KANALI TANIMI AYRI SAYFA + SEKMELER (2026-09-10, kullanıcı kararı "popup kullanışsız oldu"):** `/settings/channels` kart tıklaması artık popup değil **`/settings/channels/:id`** sayfasını açar (Yeni Kanal → `/settings/channels/new?firmId=`). `ChannelDetailPage`: breadcrumb + başlık (tip/kod/firma/yetenek rozetleri) + `ChannelForm layout="tabs"`; sekmeler URL'de `?tab=`: **Genel** (firma/tip/ad/kod/fiyat/yetenekler/aktif) · **API & Şema Ayarları** (platform tipi şeması) · **Vitrin** (stok görünürlüğü) · **Ödeme & Taksit** · **Kargo** (ücret + gönderim + müşteri seçimi) · **Eski Sistem** (legacyPlatformId). Kaydet/İptal alt çubuğu yapışkan. Form içeriği tek kaynak: `ChannelForm` bölümlere ayrıldı (`bolumGenel/Sema/Vitrin/Odeme/Kargo/Entegrasyon`); Firma detayı ve Pazaryeri sayfalarındaki modal (`layout="stack"`, varsayılan) eski sırasıyla aynen çalışır. Yardımcılar `channelHelpers.ts`; sidebar `activePatterns` ile menü vurgusu detayda kalır. ★ Yeni kanal ayarı eklerken ilgili bölüme koy (popup ve sayfa aynı bölümü gösterir). tsc/eslint ✓, `admin/dist` derlendi (nginx doğrudan servis eder — restart gerekmez). Tarayıcı duman testi kullanıcıda.

**SATIŞ KANALI TAKSİT TABLOSU (2026-09-10, kullanıcı kararları K1-K4):** Taksitlendirmenin MÜŞTERİ tarafı artık kanal bazlı: panel Ayarlar › Satış Kanalları › kanal › "Kredi Kartı Taksitleri" bloğunda kaynak seçimi — **Ödeme aracısının tablosu (PayTR, varsayılan = eski davranış)** ya da **Kendi taksit tablomuz** (2..12 taksit × vade farkı %, aktif). Settings `installmentSource` (provider|own) + `installmentTable` [{count,rate}]. Kural TEK yerde `Shared.Contracts.TaksitKurali` (Secenekler / VadeFarki / KalemPaylari / KdvSatirlari): `POST /api/store/payment/paytr/taksit` (yanıt: `kaynak` + `vadeFarki`; sipariş yokken kanal X-Firm-Platform/Host; baz = vade farkı HARİÇ tutar) ve `/init` (own kanalda vade farkı SUNUCUDA hesaplanır, `SiparisTaksitUygulaCommand` ile `Order.InstallmentCount`/`InstallmentFee` yazılır, GrandTotal artar, PayTR'ye yeni toplam gider; tabloda olmayan adet 400; ödenmiş siparişte ret). Migration `20260910142406_AddOrderInstallment` canlı + demo DB'de UYGULANDI (⚠ bekleyen model kayması da girdi: `ord_returns.OrderId` indeks + FK — iki DB'de yetim kayıt yoktu). **Fatura:** vade farkı ürünlere yedirilmez, ürünlerin KDV oranına göre oranlanıp oran başına ayrı "Vade Farkı" satırı (CreateInvoice). **İade:** müşteriye ödenen = ürün tutarı + kalemin vade farkı payı (CreateStoreReturn). **Legacy sipariş senkronu:** `installmentCount` gerçek değer, vade farkı eski site kalıbıyla masraf tipi 5 "Taksitlendirme Komisyonu" kalem başına (`Legacy:OrderService:InstallmentExpenseTypeId`, varsayılan 5). Admin sipariş detayında Taksit/Vade Farkı satırları; web ödeme sayfasında "Vade farkı: X ₺ / Vade farksız" etiketi. **Eski `cctaksitler` tabloları canlı kanal ayarlarına AKTARILDI** (`tools/veri-bakim/2026-09-10-taksit-tablosu-aktarimi.sql`: tozlu 11, julude 11, olutbutik 11, mishar 5 satır; kaynak DOKUNULMADI = provider — personel panelden "Kendi tablomuz"a geçirir; eski platform 14'ün kanalı yok). ★ **PayTR paneli:** kendi tablo modunda taksit komisyonu "mağazadan" seçili olmalı, yoksa müşteriye iki kez biner. İzole 5051 (demo DB, kapı kapalı): kendi tablo seçenekleri (1000 → 2×500 vade farksız / 3×350 = 1050 / 6×183,33 = 1100), sipariş bazlı (599,70 → 629,69), tabloda olmayan 9 taksit → 400, 3 taksit mock ödeme → sipariş 3 / 29,99 / 629,69 paid, ödenmiş siparişte tekrar → ret, provider kanal → boş liste, admin detay alanları ✓; demo ayarı geri alındı (TEL0000010 mock ile paid kaldı). Testler 282/282 (TaksitKuraliTests 8). Doğrulanamayan: fatura satırı uçtan uca (demo'da entegratör sözleşmesi yok — birim test), gerçek PayTR (test modu kilidi). `publish`/staging/demo eşitlendi + `admin/dist` derlendi ⚠️ **RESTART BEKLİYOR** (ecspros, ecspros-staging, ecspros-demo). Mobil sözleşme `docs/mobil-api-referansi.md` §5 taksit satırı güncellendi.

**AI RAPORLAMA PLANI v1.0 (2026-09-10, kullanıcı "incele ve uygula") — ▶ KALDIĞIMIZ YER: kullanıcı K1-K8 kararlarını hazırlıyor; kararlar gelince plan v1.1 + FAZ 16.0 kapanır, R1 başlar:** `docs/AIRapor/` üçlüsü (plan PDF v1.0 + Claude/Gemini/ChatGPT konsolide değerlendirme + değerlendirme/iş planı) okundu, canlı DB ile doğrulandı ve **tek plana indirildi: `docs/ai-raporlama-plani.md`** — ilkeler, canlı tespit tablosu (171 sipariş/0 ödeme satırı/1 iade; inv_stocks 248.710 satır, hareket 0; tümü TRY, kur tablosu yok; LLM istemcisi yok; rapor izni yok; GridExport XLSX var), **K1-K8 kararları önerili** (v1 yalnız TRY; sözlük tek kaynak §4 JSON şeması; drill-down v1 yok; kapsam v1 satış+stok + satış dönemi a/b; Anthropic "yalnız tanım" modu + KVKK 90 gün prompt log; §5 mükerrer toplam mekanizması = olgu tabloları kendi tanesinde toplanır, join sonra + stok üç ölçü Quantity/Reserved/fark; takvim; karar verici ismi), R1-R5 fazları + R6 açık kapı, kabul kapıları (referans hesap birebir, eval ≥%90/sessiz yanlış varsayım 0, özet sayı doğrulaması). Kaynak PDF'in "aktarım gecikmesi/V3 güncelliği" cümlesi kullanıcı kararıyla dışarıda tutuldu. Yol haritasına **FAZ 16** eklendi. **Kod/migration YOK — K1-K8 onayı bekleniyor.**

**PANEL EKSİKLERİ → YOL HARİTASI FAZ 15 (2026-09-10):** `docs/ECSPros_Panel_Eksik_Islevler_Listesi.md` v1.1 kod doğrulamasından geçirildi (Ara Birleştirme→OP3 taşındı, Kombin gerekçesi hatalı, Script→Takip&Reklam, Fatura yeniden yazdır ucu var, raf backend kısmen hazır) ve `docs/acik-isler-yol-haritasi.md` FAZ 15 olarak eklendi (15.1 301 yönlendirme ŞART → 15.2 küçük hazır işler → 15.3 raf ekranları → 15.4 kısmi → 15.5 doğrulanacak → 15.6 karar).

**SİTE — KART FİYATI "EN YÜKSEK BEDEN" KURALI DÜZELTMESİ + STAGING EŞİTLEME (2026-09-10, kullanıcı bildirimi "restart yaptım, hâlâ düşük fiyat"):** İki ayrı neden vardı. (1) `publish-staging` 10:25'te (değişiklikten ÖNCE) alınmıştı — mobil ekibin probe'u staging'e (5055) bakıyor; publish/staging/demo 11:53'te aynı binary ile eşitlendi. (2) Sabahki kural yanlış katmandaydı: kanal fiyatlarının ayrı maks'ı, yoksa taban maks'ı alınıyordu; oysa kanal fiyatı çoğu üründe bedenlerin yalnız BİRİNDE var (canlı: 22.567 üründe kısmi) → P-00020386'da tek kanal fiyatı stoksuz L'de 299,99 olduğundan kart yine 299,99 gösteriyordu; S/M ise checkout'ta tabandan 399,99. Düzeltme: kural TEK yerde `KartFiyatGorunumu.KartTabanFiyati` — varyant başına efektif fiyat (kanal ?? taban; checkout'un varyant başına kuralıyla aynı) → EN YÜKSEK. Kategori listesi (KartPair.Price), arama listesi (GetStoreProducts MinPrice) ve `EffectivePriceProvider` (arama fiyat filtresi/sıralaması/facet + favori/sepet fiyat-düşüşü taraması; metot adı `GetKartFiyatlariAsync`) buna bağlandı; `channelcat:products` v12→v13. İzole 5051 (canlı DB, Node:Role=Api): arama+kategori kartı 399,99, çizili yok, fiyat filtresi 350-450 içinde / 250-320 dışında; kadin-bluz sayfa 1'de 25 karttan 3'ü değişti (hepsi aynı desen). Testler 274/274. ⚠️ **RESTART BEKLİYOR** (ecspros + ecspros-staging + ecspros-demo). ★ **AÇIK KARAR (kullanıcıya soruldu):** eski sistemde fiyat BEDEN başına değil RENK başına (`plurunler` satırı = ana varyant, stokAdedi bedenlerin toplamı; P-00020386 Siyah satış 299,99 / liste 399,99 = TÜM bedenler); Faz 27 senkronu bu fiyatı yalnız ana varyanta yazıyor → S/M kanal fiyatsız kalıp tabandan (399,99) satılıyor. Etkilenen: kısmi + fiyatı tabandan farklı 3.057 ürün (2.474'ü stoklu). Önerilen düzeltme senkron tarafında (plurunler fiyatını rengin TÜM varyantlarına yazmak); onay gelmeden yapılmadı. Detay sayfası kapsam dışı bırakıldı (varsayılan fiyatı hâlâ en düşük varyant).

**PANEL — YETKİ GRUBUNDA KANAL SEÇİMİ YAPILAMIYORDU (2026-09-10, kullanıcı bildirimi):** Yetki Grupları ve Kullanıcı Yetkileri ekranları kanal listesini `GET /api/core/firm-platforms`'tan çekiyordu; **böyle bir uç hiç yoktu** (kanallar yalnız `firms/{firmId}/platforms` altında) → 404 → boş liste → "Kanallar: 0/0", kanal paneli çeksiz. Düzeltme: `GET /api/iam/permission-channels` (YetkilendirmeController; aktif kanallar id/code/nameI18n, `iam.permissions.manage`) — CoreController'a konmadı çünkü sınıf düzeyi `definitions.view` istiyor, yetki yöneticisinde olmayabilir. Kaynak sunucunun "tüm kanallar (bugünkü liste)" kısayoluyla AYNI (aktif kanallar). İki sayfa yeni uca bağlandı. İzole: yeni uç 401 (rota var), eski yol 404. ⚠️ **RESTART BEKLİYOR**; canlıda doğrulama: yetki grubu aç → kanal kapsamlı yetki seç → "Kanallar: 5/5" ve tıklayınca 5 kanal çeki.

**MOBİL — SEPET TUTAR TUTARLILIĞI (2026-09-10, mobil ekibin raporu):** `GET /api/store/cart` satır `lineTotal` = `AddedPrice × adet` (kampanya ÖNCESİ brüt) verirken `POST /checkout/preview` `UnitPrice × adet` (net) veriyordu; `subtotal` da öyle (1.999,97 brüt / 1.759,97 net), `total` ikisinde aynıydı. Düzeltme `GetCartQueryHandler`'da: `LineTotal = UnitPrice × Quantity`, `Subtotal = Σ LineTotal` (net), ödenecek ürün tutarı = `Subtotal − CartDiscount` (SiparisTutarKurali ile aynı aritmetik). `campaignDiscount` BİLİNÇLİ sepet-seviyesi kaldı (MK1, ön izleme ile aynı anlam); `totalDiscount` bilgi alanı — ürün-bazlı payı artık `subtotal`'a gömülü, bir daha düşülmez; `addedPrice` kampanya öncesi taban olarak korundu (web çizik satır bundan). ★ **Web'de gizli hata bulundu ve kapandı:** M1 (2026-09-09) sunucu-fiyatına geçince `addedPrice` taban fiyat oldu; teslimat/ödeme sayfaları Σ`lineTotal` − `campaignDiscount` hesapladığından ürün-bazlı kampanyalı üründe ÖDEME sayfası fazla tutar gösteriyor, checkout doğru tahsil ediyordu. Net `lineTotal` ile o sayfalar kendiliğinden doğru; sepet sayfası özeti `unitPrice` (net) ile, üst mini sepet çizik brüt `addedPrice × adet` ile güncellendi. `POST /cart/items.price` yok sayılıyor (mobil teyit). İzole 5051 doğrulaması (mishar, P-00021945 ×2 + P-00020386 ×1): sepet = ön izleme → 1.359,98 / 1.659,97; test sepeti silindi. Sözleşme: `docs/mobil-api-referansi.md` §4 tablo; `docs/mobil-backend-istekleri-2026-09-09.md` §1 ek. Doğrulanamayan: sepet-seviyesi kampanya (`campaignDiscount`>0) senaryosu — canlıda aktif 2-al-1-öde yok; aritmetik SiparisTutarKurali'nın aynısı. **⚠️ RESTART BEKLİYOR.**

**PANEL DATAGRID — TUR 12 (2026-09-09, KAMPANYA KAPANIŞI):** Kalan 7 gerçek liste dönüştürüldü (toplam 35/38): Resim Setleri (`catalog/settings`), Arayüz Çevirileri (`settings/translations`), Kart Mesajları (`storefront/product-card`), Sayım/Teslim (`procurement/sorting`), Hakedişler (`commission`), Numara Serileri (`orders/number-series`), Pazaryeri Eşleştirme (`marketplaces/eslestirme` — Gözden Geçir + Özellikler sekmeleri). Kullanıcı kararıyla 3 sayfa kapsam dışı: Etiket Basımı, Tedarik Raporu, Talep Ayarları.

★ **Yeni kalıp 1 — PİVOT tablo sunucuda ANAHTAR bazlı sayfalanır.** Çeviriler ekranı satır=anahtar, kolon=dil pivotudur; veritabanında her satır bir (anahtar, dil) çiftidir. Sıra: (1) şema süzgeçleri ham satırlara → hangi ANAHTARLAR kalıyor, (2) sayfaya düşen anahtarların TÜM dilleri yeniden çekilir → süzgeç satır seçer, hücreleri KIRPMAZ ("en değeri X içeren" filtresi Türkçe hücreyi gizlemez). "Eksik çevirisi olanlar" anahtar seviyesinde bir süzgeç olduğu için dışa aktarmada da uygulandı — yoksa liste ile Excel ayrışırdı.

★ **Yeni kalıp 2 — `useLocalGrid` (yerel değerlendirme).** Bazı tablolar LİSTE değil FORM'dur: satır kümesi "kanal başına bir satır" (numara serileri; kaynak sorgu core_firm_platforms LEFT JOIN seri, serisiz kanallar da görünmek zorunda), "entegrasyon başına barkod aralığı" ya da eşleme tezgâhının çalışma kümesidir. Sunucuda sayfalanamazlar; `admin/src/components/grid/useLocalGrid.ts` grid durumunu (arama/f.*/sort/sayfa) TAM listeye yerelde uygular ve sunucu operatör sözleşmesini birebir izler. ⚠ Sayfalı ucu OLAN liste bunu kullanmaz — muafiyet `DataGridKeyConsistencyTests.YerelGridSayfalari`'nda tek tek sayılır ve yeni test o sayfaların gerçekten `useLocalGrid` kullandığını doğrular (sessiz muafiyet yok).

★ **DataGrid'e genişleyen satır eklendi** (`expandedRow`): satır tıklanınca altında tüm kolonları kaplayan panel açılır — Özellikler sekmesinin değer eşleme paneli bu sayede satır-içi düzenlemeyle birlikte korundu. Açık satırın anahtarı ÇAĞIRANDA tutulur (panel kaydettiğinde açık kalmalı).

★ Test koşum notu: `ECSPROS_TEST_DB` bayrak değil BAĞLANTI DİZESİDİR — `=1` verirsen 7 DB testi "Format of the initialization string" ile patlar. Doğrusu appsettings'ten `DefaultConnection`.

Doğrulama: 280 test geçiyor (313 toplam, +1 yeni yerel-grid denetimi), GridSchemasDbTests 3 yeni şemayı gerçek DB'de SQL'e çevirdi, izole 5056'da 10 rota 401 + 0 açılış hatası. ⚠️ **RESTART BEKLİYOR.**

**PANEL DATAGRID — TUR 11 (2026-09-09):** Yetki Grupları (`/settings/permission-groups`) ve Fatura Serileri (`/orders/invoice-series`) dönüştürüldü (toplam 28/38). İkisi de dropdown kaynağı → ayrı `/grid` + `/export` uçları (`/iam/permission-groups/grid`, `/orders/invoice-series/grid`); düz GET'ler dokunulmadı. ★ **Fatura Serileri'nde tam liste EKRANDA DA gerekiyor**: kanal yuvası seçicileri, "pasife alırken yerine geçecek seri" adayları ve kullanan-kanal kodları tam listeden çizilir → sayfa iki sorgu tutuyor (satırlar sayfalı grid'den, seçiciler tam listeden). Bu, dropdown kaynağı olan sayfaların genel kalıbı olmalı. ★ Firma adı (Core) ve entegratör sözleşme adı (firma entegrasyonları) başka modülde çözüldüğü için sıralanamıyor; süzgeçler kimlik (firmId) ve bayrak (`sozlesmeVar`) üzerinden — Mal Kabul/Satın Alma kalıbı. Doğrulama: 279 test geçiyor, GridSchemasDbTests iki yeni şemayı gerçek DB'de SQL'e çevirdi, izole 5056'da 6 rota 401 + 0 açılış hatası. ⚠️ **RESTART BEKLİYOR.**

★ **TRİYAJ (tur 11):** kalan 10 sayfa tek tek incelendi; üçü sunucu listesi değil → kampanya kapsamından çıkarılması öneriliyor: **Etiket Basımı** (`procurement/labels`, tablo yerel basım sepeti — satırlar istemcide kurulur, sunucuda liste yok), **Tedarik Raporu** (`procurement/report`, gruplu rapor çıktısı), **Talep Ayarları** (`crm/tickets/settings`, tek ayar yükünün içindeki konu/durum alt koleksiyonları — "detay sayfası alt tabloları kapsam dışı" kuralının aynısı). Kalan 7: catalog/settings (image-sets sekmesi), settings/translations, storefront/product-card, orders/number-series, procurement/sorting, commission, marketplaces/eslestirme — son dördü satır-içi düzenleme tablosu.

**PANEL DATAGRID — TUR 10 (2026-09-09):** Platform Tipleri (`/settings/platform-types`) ve Servis Kataloğu (`/settings/integration-services`) dönüştürüldü (toplam 26/34). Her ikisi de hem dropdown kaynağı (Kanallar, Firma detayı, Pazaryerleri, firma entegrasyon formu) hem de ağır jsonb şema taşıyor → tur 9'daki iki kalıp birlikte uygulandı: ayrı `/grid` + `/export` uçları (eski sayfalanmamış GET dokunulmadı) ve grid satırında şemanın kendisi değil `hasSchema`/alan sayısı; şema rozetleri ile düzenleme modalı tam tanımı 5 dk önbellekli ayrı sorgudan alıyor. ★ **Yeni ders: jsonb kolonu boş DİZGEYLE karşılaştırılmaz** — `SettingsSchemaJson != ""` PostgreSQL'de `22P02 invalid input syntax for type json` veriyor; yalnız `!= null` denetle. Tuzak sinsi: `PlatformType.SettingsSchemaJson` **text**, `IntegrationService.SettingsSchemaJson` **jsonb** — aynı ada sahip iki kolonun tipi farklı. Doğrulama: 279 test geçiyor (GridSchemasDbTests iki yeni şemayı gerçek DB'de SQL'e çevirdi), izole 5056'da 6 rota 401 + 0 açılış hatası. ⚠️ **RESTART BEKLİYOR.**

**PANEL DATAGRID — TUR 9 (2026-09-09):** Mal Kabul ve Kampanya Tipleri dönüştürüldü (toplam 24/34). ★ **Ağır jsonb'yi satırdan çıkarma kalıbı:** Kampanya Tipleri'nin ayar şeması (`SettingsSchema`) grid satırında gönderilmiyor, yalnız alan SAYISI taşınıyor; satır tıklanınca açılan parametre şablonu modalı şemayı talep anında tam listeden çekiyor (5 dk önbellek). Aynı kalıp Özellik Tipleri'nde de kullanıldı (değer listesi yerine sayı) — ağır jsonb taşıyan liste ekranlarında varsayılan yaklaşım bu olmalı. ★ Mal Kabul'de tedarikçi VE depo adları başka modüllerde (Accounts/Inventory) çözüldüğü için iki kolon sıralanamıyor; süzgeçleri kimlik üzerinden (aynı kalıp Satın Almalar, Stoklar, Denetim, Yetki logu). ★ `/promotion/campaign-types` yine dropdown kaynağı çıktı (kampanya listesi + kampanya detayı) → ayrı `/campaign-types/grid` ucu. Doğrulama: 279 test geçiyor; GridSchemasDbTests 2 yeni şemayı gerçek DB'de SQL'e çevirdi; izole 5056'da 5 rota 401, 0 açılış hatası. ⚠️ **RESTART BEKLİYOR.**

**PANEL DATAGRID — TUR 8 (2026-09-09, kanal devamı):** Bildirim İzleme dönüştürüldü — sayfada İKİ ayrı grid var (stok alarmları + kayıtlı aramalar), ikisi de Y3 kanal kapsamlı (toplam 22/34). ★ **ÇOK GRID'Lİ SAYFA TUZAĞI BULUNDU:** `useGridState` URL parametrelerini (page, search, sort, dir, f. ve fq. önekli filtreler) gridId'ye göre AD ALANINA ALMIYOR — yalnız localStorage tercihleri gridId'li. Sekme değiştirince bir grid'in sıralaması diğerine taşınıyor ve o şemada olmayan anahtar yüzünden liste 400 "Geçersiz sıralama alanı" veriyor. Sekme geçişinde grid parametrelerini temizleyen `sekmeyeGec` kalıbını ekledim; çok grid'li her sayfada şart. ★ Kendi anahtar denetim testim (`DataGridKeyConsistencyTests`) bu sayfayı "kayıtlı değil" diye reddetti — testi çok şemalı sayfayı destekleyecek şekilde genişlettim (sayfa → şema dizisi, anahtarlar birleşime karşı denetlenir). ★ **C# tuzağı:** blok yorumun içine `f.*/fq.*` yazmak `*/` içerdiği için yorumu erken kapatıyor ve dosya anlaşılmaz sözdizimi hatalarıyla patlıyor — yaşadım, yorumu düzelttim. Doğrulama: 279 test geçiyor; GridSchemasDbTests 2 yeni şemayı gerçek DB'de SQL'e çevirdi; izole 5056'da 4 rota 401, 0 açılış hatası. ⚠️ **RESTART BEKLİYOR.**

**PANEL DATAGRID — TUR 7 (2026-09-09, kanal sayfaları):** Kanal Ürünleri ve Kanal Kategorileri dönüştürüldü (toplam 21/34). ★ **TOPLU İŞLEMDE KAPSAM TUZAĞI BULUNDU VE KAPATILDI:** Kanal Ürünleri'nde "Filtreye uyan tümünü seç" düğmesi toplu işlem besliyor (kanala al/çıkar, satışı durdur, pazaryerine gönder) ama id ucu BAŞLIK FİLTRELERİNİ bilmiyordu — kullanıcı listeyi 5 ürüne süzüp "tümünü seç" derse binlerce ürün seçilip toplu işlem yanlış ürünlere uygulanabilirdi. Artık `manage` ve `manage/ids` aynı `GridRequest`'i alıp aynı şemayı uyguluyor; `KanalUrunleriSecimKapsamiTests` bunu sabitliyor. **Toplu işlemli bir listeyi grid'e alırken İLK kontrol edilecek şey bu.** ★ **Y3 değişmezi beni düzeltti:** Kanal Kategorileri şemasına `.Kanal()` koymamıştım (liste tek kanala kilitli, parametre denetimi var diye) — mevcut `KanalKapsamiTests` reddetti. Kural doğru: kanal kolonu olan şema her zaman kapsam bildirir, çünkü aynı şemayı kullanan yeni bir uç parametre denetimini atlayabilir. Kapsam liste + export'ta uygulanıyor. ★ **Sayfalamayla bozulan istemci sayımı:** "N kategori tanımsız" uyarısı tüm listeden hesaplanıyordu (sayfalı listede yalnız açık sayfayı görür); şemaya `tanimsiz` bayrağı eklendi, sayı sunucudan geliyor ve uyarıya tıklayınca liste o filtreye süzülüyor. ★ Kanal Ürünleri'nde toplu seçim DataGrid.selection'a taşındı; DTO'ya kaynak/liste fiyatı/varyant sayısı eklendi (boş hücre gösteren kolon bırakmamak için). Kanal durumu ve listeleme kolonları sunucuda sıralanamaz (bellekte çözülüyor) — gerekçe şemada yazılı. Doğrulama: 279 test geçiyor (2 yeni); izole 5056'da 4 rota 401, 0 açılış hatası. ⚠️ **RESTART BEKLİYOR.**

**PANEL DATAGRID — TUR 6 (2026-09-09):** Yetki İçerikleri (katalog) ve Satın Almalar dönüştürüldü (toplam 19/34). ★ **Gruplu görünüm → düz grid kararı:** Yetki İçerikleri satırları MODÜLE göre gruplayıp ayrı kartlarda gösteriyordu; DataGrid düz tablo olduğu için gruplamayı "MODÜL" sütunu + filtresi olarak korudum (varsayılan sıra modül→kod). Bilgi kaybı yok, üstelik artık modüle göre sıralanabiliyor. ★ `/iam/permissions` yine dropdown kaynağı çıktı (yetki grubu + kullanıcı yetkisi ekranları) → ayrı `/permissions/grid` ucu. ★ Satın Almalar'da tedarikçi adı Accounts modülünde çözüldüğü için o kolon sıralanamıyor; süzgeç adlandırılmış `supplierId` ile (aynı kalıp Stoklar `productCode`, Denetim `user`, Yetki logu aktöründe de var). ★ **Kanal Ürünleri'ni bilinçli olarak ERTELEDİM:** 466 satır, toplu seçim + listeleme özeti + kanal durumunun BELLEKTE birleştirilmesi var (`IsSelected`/`SaleStopped*` sunucuda sıralanamaz) — günlük kullanılan bir ekranı tur sonuna sıkıştırmak yerine ayrı odaklı tura bıraktım. Doğrulama: 277 test geçiyor; GridSchemasDbTests 2 yeni şemayı gerçek DB'de SQL'e çevirdi; izole 5056'da 5 rota 401, 0 açılış hatası. ⚠️ **RESTART BEKLİYOR.**

**PANEL DATAGRID — TUR 5 (2026-09-09):** Bülten Aboneleri, Yetki Logları dönüştürüldü (toplam 17/34). ★ **GERÇEK HATA DÜZELTİLDİ — yetki logu araması bozuktu:** arama BELLEKTE, sayfalamadan SONRA uygulanıyordu ve `toplam` filtresiz sayılıyordu → eski bir kaydı arayınca panel "N kayıt" yazıp BOŞ sayfa gösteriyordu (yalnız o sayfaya düşen eşleşmeler görünüyordu). Artık arama terimi önce kullanıcı/grup/yetki kayıtlarına ÇÖZÜLÜP DB'de `UserId`/`EntityId`/`EntityType` üzerinden süzülüyor; sayım aynı küme üzerinde. Koruyan yeni test: `YetkiLogAramaDbTests` (gerçek DB, salt okunur). ★ **jsonb sözlükte sunucu taraflı filtre/sıralama — üç deneme, tek doğru yol:** (1) `Dictionary<string,string>` için `GridJson.Text` DbFunction ÇALIŞIR; (2) sözlük indeksleyicisi/`ContainsKey` **çevrilmez** (string sözlükte bile — yeni arama kodumda bu hatayı yaptım, DB testi yakaladı); (3) `Dictionary<string,object>` (denetim logunun `Context`) **hiçbir yolla olmaz** — DbFunction parametresi olarak EF model doğrulamasını kırıyor ve tüm DbContext'i geçersizleyip 3 alakasız testi birden patlattı; o alanlar sıralanamaz. Bu yüzden yetki logunda özet kolonu sıralanamaz (kararın gerekçesi şemada yazılı). ★ Açılır ayrıntı satırı DataGrid'de desteklenmiyor → yetki logunun ayrıntısı MODALA taşındı, hiçbir alan kaybolmadı. ★ Bülten abonelerinde Y3 kanal kapsamı listeye de uygulandı. Doğrulama: 277 test geçiyor (1 yeni); izole 5056'da 3 rota 401, 0 açılış hatası. ⚠️ **RESTART BEKLİYOR.**

**PANEL DATAGRID — TUR 4 (2026-09-09):** Tedarikçi Gönderimleri, Özellik Tipleri, İçerik Sayfaları dönüştürüldü (toplam 15/34). İki sayfa daha "dropdown kaynağı" çıktı, ayrı `/grid` ucu açıldı: `/catalog/attribute-types` (ürün grubu detayı + özellik tipi detayı + pazaryeri eşleme + FilterBuilder, TÜM tipleri DEĞERLERİYLE döner — grid satırı yalın, değer/grup SAYISI ile) ve `/cms/pages` (CMS detayı + sipariş detayındaki sözleşme bağlantısı). ★ **İçerik Sayfaları'nda Y3 boşluğu kapandı:** listede kanal kapsamı yoktu (yalnız kanal parametresi denetleniyordu) → şemaya `.Kanal(FirmPlatformId)` eklendi, liste+export kapsamı kullanıcının yetkisinden çözüyor. ★ `lastContentUpdatedAt` (sözleşme sürüm tarihi) hesaplanmış alan olarak şemaya girdi — kuralı `GetStoreLegalPages` ile aynı tutulmalı. ★ **⚠ İZOLE TEST PORTU DERSİ:** 5055 ecspros-staging'in portu; izole örneği oraya bağlamaya çalıştım, bind edemeyip core dump'la kapandı — staging etkilenmedi (doğruladım), ama izole testlerde 5051-5054/5056 kullanılmalı; 5000 canlı, 5055 staging. Doğrulama: 276 test geçiyor; GridSchemasDbTests 3 yeni şemayı gerçek DB'de SQL'e çevirdi; izole 5056'da 8 rota 401 (404 değil), 0 açılış hatası. ⚠️ **RESTART BEKLİYOR.**

**PANEL DATAGRID — TUR 3 (2026-09-09):** Cari Kartlar, Cari Grupları, Firmalar, Üye Grupları, Koleksiyon Moderasyonu dönüştürüldü (toplam 12/34). ★ **TUZAK — `GridJson.Text` DbFunction kaydı:** Firmalar'ı i18n adına göre süzerken `InvalidOperationException: LINQ expression could not be translated` geldi; 2026-09-08'de bu kayıt 10 DbContext'e eklenmiş ama **Core, Accounts, Storefront, Requests, Cms, Procurement atlanmış**. Kayıt şema değiştirmiyor ve migration gerektirmiyor (ilk commit 6506966 migration'sız) — altısına da eklendi. Bu hatayı yakalayan yer `GridSchemasDbTests`; şema yazarken i18n alan kullanıyorsan o DbContext'in kaydını kontrol et. ★ **Y3 boşluğu kapandı:** Koleksiyon moderasyonu LİSTESİNDE kanal kapsamı yoktu (yalnız kanal parametresi denetleniyordu) → kuyrukta kapsam dışı kanalın koleksiyonu görünebiliyordu; şemaya `.Kanal(FirmPlatformId)` eklendi, liste ve export kapsamı kullanıcının yetkisinden çözüyor. ★ Üç sayfa daha "dropdown kaynağı" çıktı, ayrı `/grid` ucu açıldı: `/accounts/groups` (cari süzgeci), `/core/firms` (birçok firma seçicisi), `/crm/member-groups` (kupon hedefi + üye formu) — eski uçlar dokunulmadı. Doğrulama: 276 test geçiyor; GridSchemasDbTests 5 yeni şemayı gerçek DB'de SQL'e çevirdi; izole 5054'te 13 rota 401 (404 değil), 0 açılış hatası; istemci↔sunucu anahtar denetimi temiz. ⚠️ **RESTART BEKLİYOR.**

**PANEL DATAGRID — TUR 2 (2026-09-09):** Transferler, Yorum Moderasyonu, İletişim Mesajları dönüştürüldü (toplam 7/34). ★ Yorum ve iletişim mesajları **Y3 kanal kapsamlı**: şemaya `.Kanal(FirmPlatformId)` eklendi, kapsam liste ucunda `GridRequestParser.Parse(..., kapsam)` ve export'ta `body.ToGridRequest(kapsam)` ile taşınıyor — kapsam **gövdeden değil kullanıcının yetkisinden** çözülüyor (istemci kapsam gönderemez). ★ `GridRequest` sealed CLASS (record değil) → `with` ifadesi yok; kapsamı `ApplyAll(..., kanalKisiti)` açık parametresiyle geçirdim, eski `KanalKisiti` parametreli çağrılar bozulmadı. ★ **KALICI DENETİM TESTİ: `DataGridKeyConsistencyTests`** — istemcide `sortable: true` olan her kolonun sunucuda `.Sort()` karşılığı ve her başlık filtresinin şemada bildirimi zorunlu; ayrıca `<DataGrid` kullanan her sayfa denetim eşleşmesinde kayıtlı olmalı (yeni sayfa kaydedilmezse test patlar). **Test yazıldığı anda gerçek bir hatayı yakaladı:** yorum moderasyonunda `text` kolonu sortable'dı ama şemada `.Sort("text")` yoktu → tıklamada 400 gelecekti; eklendi. Şemaya konamayan alanlar (Users `role`, Tickets `readByMe`, Stocks `product`) handler'da `ApplyFilters(q, grid, "alan")` ile atlanıp özel işlendiği için test bu bildirimleri kaynaktan tarayıp geçerli sayıyor — sessiz kaçış listesi yok. ★ Ekip arkadaşının kaynak-metin testi `ReviewModerationOrderingTests` sıralamayı satır içi LINQ'te aradığı için kırıldı; garanti korunuyor (`ApplySort` varsayılanı + TieBreaker'ı aynı yönde ekler = eski `OrderByDescending(CreatedAt).ThenByDescending(Id)`), test yeni yapıya uyarlandı (şemada DefaultSort/TieBreaker/Kanal + handler'da filtre→sıralama→sayfalama ve sayımın sayfalamadan önce olması). Doğrulama: 276 test geçiyor; GridSchemasDbTests 3 yeni şemayı gerçek DB'de SQL'e çevirdi; izole 5053'te 6 yeni rota 401 (404 değil), 0 açılış hatası. ⚠️ **RESTART BEKLİYOR.**

**PANEL DATAGRID YAYGINLAŞTIRMA (2026-09-09) — kullanıcı isteği: "tüm tablolar orders gibi olsun, tüm sütunlarda sıralama ve filtre; mobil Tablo/Kompakt düğmesi tüm datagridlere".** (1) **Ölçüm:** 17 sayfa zaten DataGrid'liydi ama Tablo/Kompakt geçişi YALNIZ Siparişler'de vardı ve 17 sayfada ~20 veri kolonunda sıralama/filtre eksikti; ayrıca 34 liste sayfası hâlâ ham `<table>` ile çalışıyordu (detay sayfalarındaki 17 alt tablo + 7 modal/drawer parçası kapsam dışı — sayfalı uç değil, çocuk koleksiyon). (2) **Kompakt geçiş 17/17:** her sayfaya `compact={{title, subtitle, right, badge}}` yazıldı; mobilde düğme çıkıyor, tercih `prefs.mobileView`'da kalıcı. (3) **Eksik sıralama/filtre kapandı:** istemcide `sortable: true` + sunucuda karşılık gelen `.Sort()` çiftleri eklendi (MemberGrid `name`, ProductGrid `group`, StockGrid `warehouse`, TicketGrid `order`/`caller`, PickingPlanGrid `assignment`, IntegrationLogGrid `error`, TrackingOutboxGrid `lastError`/`targets`, InvoiceGrid `integratorStatus`/`hasPdf`, ReturnGrid `returnType`/`cargoReturnCode`, CampaignGrid `campaignTypeCode`/`fillType`, AuditLogGrid `entityId`; ProductsPage GRUP kolonuna başlık filtresi). Aksiyon kolonları (Detay→/Düzenle→) bilinçli olarak sıralamasız. **Sıralanamayan 5 kolon teknik sınır:** Stoklar `productCode` (ürün adı Catalog modülünde, modüller arası join yok), Stoklar `section`/`bin` (Stock entity'sinde navigation property yok), Denetim `user` (AuditLog yalnız UserId tutar), Kullanıcılar `roles` (string dizisi). (4) **Dönüştürülen 4 sayfa:** Kuponlar, Talepler, Ürün Grupları, Depolar — her biri `XGrid.cs` (GridSchema + ApplyNamed + ExportRow + ExportQuery) + `XExportColumns` + controller (GridRequestParser + POST export) + sayfada DataGrid. (5) **★ TURUN EN ÖNEMLİ BULGUSU — dropdown kaynağı olan liste ucu SAYFALANMAZ:** `GET /catalog/product-groups` 4 ekranın, `GET /inventory/warehouses` 7 ekranın dropdown kaynağı; sayfalasaydım o ekranlar sessizce kırılırdı. Çözüm: liste ekranı için AYRI `/grid` ucu + yalın satır DTO'su, eski uç dokunulmadan. Kalan sayfalarda önce tüketici sayılacak. (6) **★ DERS: `npx tsc --noEmit` YETMEZ** — eksik import'u yalnız `npm run build` (tsc -b) yakaladı (TicketsPage `Badge`). (7) **Doğrulama:** 274 test geçiyor; `GridSchemasDbTests` 4 yeni şemanın TÜM filtre alanlarını ve sıralama anahtarlarını asc/desc gerçek DB'de çalıştırdı (SQL'e çevrilemeyen ifade orada patlar); istemci `sortable` anahtarları ↔ sunucu `.Sort()` eşleşmesi betikle doğrulandı; izole 5052 publish'inde 10 yeni/etkilenen rota 401 döndü (404 değil = rota kayıtlı, çakışma yok), açılışta 0 hata. ⚠️ **RESTART BEKLİYOR.** SIRADA: kalan 30 liste sayfası.

**MOBİL M4 UYGULANDI (2026-09-09): durum kodları → etiket, tek kural.** Store uçları ham kod döndürüyordu (`pending`, `shipped`…), etiket/renk eşlemesi yalnız admin panelindeydi; mobil kendi listesini gömüyordu ve yeni durum eklenince sessizce kodu gösteriyordu. (1) **★ UYGULAMA SIRASINDA ÇIKAN GERÇEK: iki etiket seti var ve ayrı kalmalı.** Panelin metinleri (`Bekleyen`/`Onaylı`) ile sitenin müşteriye gösterdiği metinler (`pending` ve `confirmed` → ikisi de **"Sipariş Alındı"**) örtüşmüyor: müşteri iç onay adımını görmemeli, personel ise ikisini ayırmak zorunda. Bu yüzden tek dosyada iki ad alanı: `DurumEtiketleri.Panel.*` (panel/Excel) ve `DurumEtiketleri.Vitrin.*` (site/mobil); store uçları HER ZAMAN Vitrin'i döner. Ayrımın kazara silinmesini test koruyor. (2) **Tek kural dosyası `Shared.Contracts/DurumEtiketleri.cs`** (M2 `SiparisTutarKurali`, M3 `VaryantSecenekMetni` kalıbı) — 6 aile (sipariş/ödeme durumu, ödeme yöntemi, iade, yorum, soru durumu), her kodda `label`+`color` (hex)+`variant` (success/warning/danger/neutral/info); bilinmeyen kod ham kod olarak döner, ASLA boş metin. Dört yüzey artık buradan besleniyor: `lookups` ucu, store satır alanları, sitenin SSR Hesabım sayfası (`HesabimController` — metin oradan kalktı, CSS sınıfı/sekme/adım sunumda kaldı), panelin Excel export'u (`OrderGrid`/`ReturnGrid.StatusLabel` delegasyona indi, metinler birebir aynı). (3) **`GET /api/store/lookups`** (anonim) — `{version, families{…}, timelines{…}}`. **`version` katalog içeriğinin SHA256'sından türer**, elle sayı artırma yok; aynı değer `ETag` olarak döner → `If-None-Match` ile gövdesiz **304** (izole test: doğru etag→304, yanlış etag→200), `Cache-Control: public, max-age=3600`. (4) **Sipariş liste/detay + `orders/track`:** `statusLabel`/`statusColor`/`statusVariant`/`paymentStatusLabel`/`paymentMethodLabel` + `canCancel`/`canReturn`/`canReview` + detayda `timeline:[{code,label,done,current}]`. Bayraklar komutların dayattığı kuralın aynısı (iptal `pending`+`confirmed`, iade/yorum yalnız `delivered`) — tek kaynak. Misafir takibinde bayrak YOK (o uçlar üyelik ister). **İptal edilen siparişte `timeline` BOŞ dizi** (tasarımda iptal akışı yok, site de gizler). İade liste/detayda da etiket + iade akışı (reddedilende boş). (5) **`GET /payment-options`:** düz kod dizisi → `methods:[{code,label,description,fee,maxOrderTotal}]` (katalog sırası: kart → kapıda nakit → kapıda kart, kanal sırası değil); ⚠ eski düz dizi **`methodCodes`** adıyla korundu — ucu web kullanmıyor ama geri uyum bedava. (6) `reviews/mine` + `questions/mine`: `statusLabel`/`statusColor`/`statusVariant`. (7) **Türetilmiş alan tekniği:** etiketler DTO'lara `=>` özelliği olarak eklendi → hiçbir handler değişmedi, JSON'a otomatik girdi; panel bu alanları okumaz (kendi `orderConstants.ts` haritasını kullanır). (8) Doğrulama: 268 test geçiyor (11 yeni `DurumEtiketleriTests`), izole 5051 publish'inde canlı veriyle 3 sipariş denendi — MIS0000058 delivered (4 adım tamam), MIS0000061 cancelled (timeline boş), MIS0000055 pending (1. adım current). Belge: `docs/mobil-api-referansi.md` §16 + `docs/mobil-backend-istekleri-2026-09-09.md` madde 4. ⚠️ **RESTART BEKLİYOR** (ecspros + ecspros-staging).

**MOBİL M3 UYGULANDI (2026-09-09): ürün detayı — liste ile tutarlılık.** (1) **★ Liste ↔ detay çizili fiyat farkının sebebi CACHE DEĞİL, KURAL FARKIYMIŞ:** liste ürünün varyantlarındaki **en yüksek** `CompareAtPrice`'ı alıyordu, detay yalnız **en ucuz varyantın** çizili fiyatına bakıyordu. O varyantta referans yoksa detay indirimi HİÇ göstermiyor (P-00020538, P-00021410), başka varyant seçilince listeden FARKLI değer çıkıyordu (P-00021624: liste 799,99 ↔ detay 599,99). Detay artık listeyle aynı kuralı kullanıyor; **site SSR detayı (`StoreUrunDetayBuilder`) da aynı kurala çekildi** (aynı tutarsızlık sitede de vardı). Canlı veriyle izole doğrulama: P-00020538 → liste 299,99/799,99 = detay 299,99/799,99; P-00021624 → 509,99/799,99 = 509,99/799,99; P-00021410 → 499,99/999,99 = 499,99/999,99. (2) **MK4:** `variants[].compareAtPrice` artık her varyantta dolu — kendi kanal referansı yoksa ürün düzeyindeki referans kopyalanır (yalnız o varyantın satış fiyatından büyükse). (3) **`variants[].variantInfo`** eklendi ve metin TEK kurala bağlandı (`Shared.Contracts/VaryantSecenekMetni`): sepet `optionsText` ile detay `variantInfo` artık birebir aynı; **iç filtre ekseni `filtre_rengi` metinden çıkarıldı** ("Beden: 44, Filtre Rengi: Lacivert, Renk: Lacivert" üretiyordu), sıra renk → beden → diğerleri, en çok 3 seçenek. (4) **Öznitelik tablosuna** `kategori_grubu` ("Kategori Grubu") ve `stok_durumu` ("Stokta"/"Tükendi") satırları eklendi (tr/en); stok satırı hiç yoksa "Tükendi" yazılmaz, ürünün satış durumu esas alınır. (5) **Sepet satırında da çizili fiyat:** `IChannelPricingService.GetProductCompareAtPricesAsync` eklendi — varyantın KENDİ çizili fiyatı yoksa ürün düzeyi referans kullanılıyor; mobilin P-00020386 örneğinde artık sepet satırı da 299,99/399,99 + %25 gösteriyor. **Bilinen incelik:** kart ürünün EN UCUZ varyantının fiyatını gösterir; sepette seçilen varyant daha pahalıysa (ör. P-00020386'nın 399,99'luk bedeni) sepet o varyantın gerçek fiyatını gösterir — bu tutarsızlık değil, varyant bazlı fiyatlandırmadır. Test: 296 test (263 geçti, 33 atlandı). ⚠️ **restart: `ecspros` + `ecspros-staging`.** Sırada M4 (durum etiketleri + `lookups`), M5 (küçükler).

**MOBİL M2 UYGULANDI (2026-09-09): sipariş ön izleme ucu + tutar aritmetiğinin tek kurala çıkarılması.** (1) **`POST /api/store/checkout/preview`** (anonim erişilebilir, misafir dahil): gövde `{cartId | sessionId+firmPlatformId, couponCode?, paymentMethod?, requestedCargoIntegrationId?, shippingNeighborhoodId?}`, yanıt `{subtotal, campaignDiscount, couponDiscount, codFee, shippingFee, shippingFreeReason, remainingForFreeShipping, total, currencyCode, lines[{itemId, variantId, quantity, unitPrice, compareAtPrice, lineTotal, compareAtLineTotal, discountPercent}], campaigns, couponError}`. **Yan etkisi YOK:** sipariş oluşmaz, kupon kullanımı yazılmaz, stok düşmez. **Kalemler SEPETTEN okunur** — istemci fiyat/kalem göndermez (M1 kuralının devamı). (2) **★ Ayrışma riski kökten kapatıldı:** tutar aritmetiği `Order.Application/Services/SiparisTutarKurali`'na çıkarıldı ve **checkout da artık onu çağırıyor** — "ön izlemede 1.000 TL, siparişte 1.049 TL" durumu kod düzeyinde imkânsız. Kural: indirim = (kupon + sepet-seviyesi kampanya) ve ara toplamı AŞAMAZ; kapıda ödeme masrafı yalnız kapıda yöntemlerde; kargo `KargoUcretiKurali` ile, eşik tabanı indirim SONRASI tutar; genel toplam = ara toplam − indirim + masraf + kargo. (3) **Kupon davranışı bilinçli olarak checkout'tan FARKLI:** checkout geçersiz kuponda siparişi reddeder (müşteri beklediği indirimi almadan ödemesin); ön izleme reddetmez, `couponError` ile sebebi döner ve tutarı kuponsuz gösterir — ekran akışı durmasın. (4) İzole doğrulama (demo, 49,90 kargo + 1.500 eşik): ön izleme `total=993,90` → gerçek sipariş `GrandTotal=993,90`; kapıda ödemede `1.043,90` → `1.043,90` (masraf 50). İstemcinin gönderdiği `unitPrice: 1,00` yine yok sayıldı (subtotal 944). Test verisi (2 sipariş, 2 sepet, kanal kargo ayarı) temizlendi. Test: 288 test (255 geçti, 33 atlandı). ⚠️ **restart: `ecspros` + `ecspros-staging`.** Sırada M3 (ürün detayı), M4 (durum etiketleri/`lookups`), M5 (küçükler).

**MOBİL BACKEND İSTEK LİSTESİ — M0 + M1 UYGULANDI (2026-09-09; değerlendirme `docs/mobil-backend-istekleri-2026-09-09.md`):** (1) **★ M0 — staging bir gün eski binary çalıştırıyordu:** mobilin test ettiği `ecspros-staging` (:5055, canlı DB) **8 Eylül 23:17** derlemesindeydi, canlı ise 9 Eylül'de; restart'lar eski binary'yi yeniden başlatıyordu. Listedeki maddelerin bir kısmı bu yüzden "eksik" görünüyordu — `shippingFee`/`freeShippingThreshold` alanları, ürün detayında `descriptionI18n` ve ürün düzeyi `compareAtPrice`, PayTR init'in `{success,data:{html}}` zarfı ve `reviews/reviewable`'ın nesne biçimi ZATEN VARDI. `publish-staging` güncellendi. **Kalıcı kural: canlıya yayın yapınca staging de eşitlenecek** (aynı DB'yi kullandıkları için eski binary yeni migration'da 500 de üretebilir). (2) **★ M1 — ÇİFTE İNDİRİM HATASI KAPANDI (gerçek hata):** `POST /cart/items` istemcinin gönderdiği fiyatı olduğu gibi `AddedPrice`'a yazıyor, `GET /cart` ise kampanyayı **bu fiyatın üstüne** uyguluyordu. Web listede kampanya ÖNCESİ fiyatı gönderdiği için doğru çalışıyor, mobil B9 sözleşmesine uyup KAMPANYALI fiyatı gönderdiği için indirim İKİ KEZ uygulanıyordu (679,99 → 577,99). Çözüm, checkout'ta 2026-07-31'de yapılanın aynısı: **fiyatı sunucu belirler, istemciden geleni yok sayar** (kanal fiyatı → varyant taban fiyatı). İzole doğrulama: istemci 1,00 TL gönderdi → sepette 944,00; kampanyalı 802,40 gönderdi → yine 802,40 (682,04 DEĞİL); fiyat hiç göndermeyince de doğru. Yan kazanç: sepet artık ekleme anındaki donmuş fiyatı değil GÜNCEL fiyatı gösteriyor, yani checkout'ta tahsil edilenle aynı. (3) **Sepete yeni alanlar (additive):** satırda `unitPrice` / `compareAtPrice` / `compareAtLineTotal` / `discountPercent` — liste kartıyla **aynı** `KartFiyatGorunumu` kuralından; sepet düzeyinde `total`, `totalDiscount`, `shippingFeeResolved`, `shippingFreeReason` (`threshold|campaign|none`), `remainingForFreeShipping` — kargo `KargoUcretiKurali` ile SUNUCUDA çözülüyor, istemci kuralı taklit etmiyor. **MK1 kararı gereği `campaignDiscount`'un anlamı DEĞİŞMEDİ** (yalnız sepet-seviyesi indirim); toplam indirim yeni `totalDiscount` alanında → web istemcisi kırılmadı. (4) `VariantDisplayInfo`'ya `BasePrice` eklendi (toplu okunur — sepette N+1 sorgu yok). (5) **Doğrulanan "zaten var"lar:** checkout `items[].unitPrice`'ı ve kupon tutarını zaten sunucuda yeniden hesaplıyor (F4 + 9.4). **Doğrulanan gerçek eksikler:** `cms/pages/by-slug/sss` (gerçek slug `sik-sorulan-sorular`), adreslerde mahalle (97 adresin 67'sinde dolu). Kullanıcı kararları MK1-MK4 planda. Sırada: M2 `checkout/preview`, M3 ürün detayı, M4 durum etiketleri/`lookups`, M5 `discountPercent` yayılımı + küçükler. Test: 282 test (249 geçti, 33 atlandı). ⚠️ **restart: `ecspros` VE `ecspros-staging`.**

**YETKİSİZ ERİŞİM DENEMELERİNİN LOGLANMASI TAMAM (2026-09-09; §J.1'in son maddesi):** Default-deny sessizdir — biri yetkisi olmayan ekranı ısrarla deniyorsa bu ya yanlış yetkilendirmedir (kişi işini yapamıyor) ya da kötüye kullanımdır; ikisi de görülmeden yönetilemez. Artık dört ret noktası kayıt yazıyor: `RequirePermission` (403), `KanalKapsamiKontrol` (kanal kapsamı dışı, 404), toplu işlemlerde `KapsamDogrulama` (kapsam dışı sipariş, 404), `NotificationHub.Subscribe` (yetkisiz bildirim konusu). Kayıtlar `iam_audit_logs`'ta `yetki.reddedildi` ailesinde, insan-okur cümleyle: *"Y8 Red kapsamı dışındaki bir satış kanalına erişmeye çalıştı: GET /api/navigation/menus (gereken yetki: storefront.channels.view, yanıt: 404)."* **★ ÖRNEKLEME (tasarımın açık şartı):** olay istemci tetiklidir, tek bir döngü dakikada binlerce satır yazabilirdi — `YetkisizErisimOrnekleyici` (singleton) aynı *(kullanıcı + ret türü + yetki + uç)* anahtarını **10 dakikalık pencerede bir kez** yazar, aradaki denemeleri sayar ve bir sonraki kayda *"son 10 dakikada aynı denemeden N tane daha oldu"* diye taşır; sözlük 5.000 anahtarla sınırlıdır (yol değiştirerek bellek şişirilemez, sınır dolunca kayıt yine YAZILIR, yalnız izlenmez). **Fail-safe:** denetim kaydı yazılamazsa istek yine reddedilir (hata yutulur, uygulama loguna düşer) — ret asla kaybolmaz. **Anonim istek (401) yazılmaz** (aktörü yok, gürültüden başka bir şey üretmez); süper admin zaten bypass ettiği için kaydı oluşmaz. Panelde Yetki Logları ekranına "Yetkisiz erişim denemesi" filtresi + kırmızı **reddedildi** rozeti eklendi (Y8'in şablon/kopyalama/kaldırma olayları da filtre listesine girdi). İzole doğrulama: yetkisiz kullanıcı `/api/orders`'a 6 kez vurdu → 403 × 6, **tek satır**; kanal kapsamı dışına 4 istek → 404 × 4, **tek satır**; anonim 3 istek → kayıt artmadı. Test: 275 test (242 geçti, 33 atlandı) — örnekleme birimi ayrıca 5 testle sabitlendi (pencere, sayaç sıfırlama, ayrı anahtarlar, bellek sınırı). ⚠️ restart bekliyor.

**YETKİLENDİRME Y8 UYGULANDI (2026-09-09; K8 geçişinin araçları + yazdırma kararı):** (1) **`/yazdir/*` kararı KULLANICIDAN GELDİ:** yazdırma sayfaları (fatura, etiket) yetki sisteminin DIŞINDA kalıyor — iframe/tablet akışı kimlik taşıyamıyor, GUID "anahtar" sayılıyor. Kabul edilen risk tasarım dokümanına açıkça yazıldı: bağlantıdaki GUID'i eline geçiren o belgedeki müşteri ad/adresini görebilir. Alan yetkileri bu yüzeye uygulanmaz; madde KAPANDI. (2) **Departman grubu şablonları** (`Shared.Kernel/Authorization/DepartmanGrupSablonlari`): Yönetim (38), Müşteri İlişkileri (18), Depo & Operasyon (12), Satın Alma (11), Muhasebe & Finans (16), İçerik & Pazarlama (21), Salt Okunur (23). Şablon yalnız BAŞLANGIÇ değeridir — kurulduktan sonra grup normaldir, şablon bir daha dokunmaz (senkron yok). Testler sınırı koruyor: hiçbir şablon `iam.*` / `definition.manage` / `system.migration.manage` / credential açığa çıkarma vermez, hiçbiri katalogun tamamını vermez, salt okunur şablonda tek bir `.manage` ya da hassas alan yoktur, depo ve müşteri ilişkileri maliyet GÖRMEZ. (3) **Yeni uçlar:** `GET/POST /api/iam/group-templates[/{kod}]` (şablon listesi + kurma), `POST /api/iam/permission-groups/{id}/copy` (grup kopyalama — yetkiler+kanallar taşınır, ÜYELER TAŞINMAZ), `DELETE /api/iam/permission-groups/{id}` (grup kaldırma — **üyesi varsa REDDEDİLİR**, sistem grubu kaldırılamaz, soft delete + denetim kaydı), `GET /api/iam/transition-status` (K8 panosu). Şablondan kurmada da eskalasyon koruması geçerli: izole testte yalnız `orders.view` taşıyan bir yetkilendirici 23 yetkilik "Salt Okunur" şablonunu kurunca gruba SADECE `orders.view` yazıldı. (4) **★ Geçiş grubu artık diriltilmiyor:** `DatabaseSeeder` kaldırılmış (soft-deleted) `gecis_tam_erisim`'i yeniden kurmuyor — aksi hâlde her restart geçici tam-erişim grubunu geri getirip K8 adım 6'yı sonsuza kadar geri alırdı. İzole restart ile doğrulandı: "kaldırılmış — yeniden kurulmadı". (5) **Panelde:** Yetki Grupları ekranına Şablondan Kur / Kopyala / Kaldır aksiyonları, "tam erişim" rozeti ve **K8 geçiş panosu** (aktif kullanıcı, süper admin, tam erişimli, geçiş grubu üyesi; süper admin olmadığı hâlde %90+ yetki taşıyanlar isim isim listelenir). (6) **§O.12 boş durum ekranı:** hiçbir ekrana yetkisi olmayan kullanıcı artık boş panel yerine "yöneticinizden departman grubunuza eklenmenizi isteyin" karşılamasını görüyor. (7) **★ CANLI ÖLÇÜM — Y8'in gerçek işi burada:** 8 aktif kullanıcının TAMAMI `super_admin` DB grubunda (73/73 yetki), 4'ünde ayrıca süper admin bayrağı var; `gecis_tam_erisim` hiç kullanılmadı (0 üye, çünkü herkesin zaten rolü vardı). Yani canlıda default-deny'ın kazancı henüz ORTAYA ÇIKMADI — kalan adım "kimin hangi departmana gireceği" iş kararı: Emre Sağlam, Halis Yorgancı, Murat Doğan, Serhat Pala bayraksız oldukları hâlde tam erişimdeler. (8) Panel derlemesini kıran bir tip hatası (Yetki İçerikleri ekranındaki `as const`) düzeltildi. Test: 270 test (237 geçti, 33 atlandı). ⚠️ restart bekliyor.

**YETKİLENDİRME Y6 ARTIKLARI KAPANDI (2026-09-09; alan maskelemenin son taraması):** (1) **Gerçek sızıntı bulundu ve kapatıldı:** `GET /api/procurement/sorting/entries` (Sayım / Teslim ekranı) `unitCost` alanını herkese döndürüyordu — bu ekran DEPO personelinindir, K6'nın hedeflediği "maliyeti sadece yetkili görsün" kuralı burada delikti. Artık `IAlanYetkileri.IzinlerAsync` → `izin.Maliyetle(...)` ile maskeleniyor; yetkisiz kullanıcıda alan `null` (JSON'da hiç yok). İzole 5051 doğrulaması: tek kullanımlık depo kullanıcısı (`procurement.manage`+`procurement.sort`, `common.view_cost` YOK) → `unitCost` yok; aynı kullanıcıya `common.view_cost` verilince → `unitCost = 123.4567`; süper admin her zaman görüyor. Test verisi (kullanıcı, grup, sayım kaydı, oturum) temizlendi. (2) **Cache riski (§O.7) bugün OLUŞMUYOR — uydurma iş yapılmadı:** `ICacheService` kullanan tek yer vitrin listeleri (`GetChannelCategoryFacets/Products`), bunlar hassas alan taşımıyor; maskeleme istek başına API sınırında uygulanıyor, hiçbir maskeli yanıt paylaşımlı cache'e girmiyor. Kural gelecekte bozulmasın diye `AlanIzinleri` başına ★ CACHE KURALI notu yazıldı: hassas alan taşıyan yanıt cache'lenecekse izinler cache anahtarına girer. (3) **Kâr / marj için henüz ZORLAMA NOKTASI YOK:** `common.view_margin` katalogda var ama bugün hiçbir DTO marj/kâr döndürmüyor; marj raporu yazıldığında bu yetkiye bağlanacak (tasarım ekine yazıldı). (4) **Bilinçli sınır:** satın alma siparişi / mal kabul ekranlarındaki `UnitPrice` maskelenmedi — fiyat o ekranın konusudur, ekran zaten `procurement.manage` ile korunuyor; maskelemek ekranı işlevsiz kılardı. (5) **⚠️ AÇIK GÜVENLİK MADDESİ (kullanıcı kararı gerekiyor):** `/yazdir/*` sayfaları (fatura, etiket) OP2 (2026-08-09) kararıyla `[AllowAnonymous]` — masa tabletindeki iframe JWT taşıyamıyor, GUID "anahtar" sayılıyor. Kimlik olmadığı için ALAN YETKİSİ ÇALIŞTIRILAMAZ: müşteri adı/adresi bu yüzeyde yetki sisteminin dışında. Seçenekler tasarım dokümanına yazıldı (kısa ömürlü imzalı link / tek kullanımlık anahtar / ağ kısıtı). Tüm kararlar `docs/panel-yetki-sistemi-tasarim.md` "Uygulama sırasında verilen ek kararlar" tablosunda. Test: 263 test (230 geçti, 33 atlandı). ⚠️ restart bekliyor.

**YETKİLENDİRME Y3 ARTIKLARI KAPANDI (2026-09-09; SignalR kapsamı + toplu işlem doğrulaması + inceleme sonuçları):** (1) **★ SignalR'da gerçek sızıntı bulundu ve kapatıldı:** `NotificationHub.Subscribe(topic)` yetki de kanal da SORMUYORDU — panele girebilen herkes `topic:orders`'a abone olup TÜM kanalların sipariş olaylarını (onay/kargo/iptal, POS satışı) alabiliyordu; liste filtrelense bile "başka kanalda sipariş oluştu" bilgisi sızıyordu. Artık: topic → gerekli yetki eşlemesi (`orders→orders.view`, `stock→inventory.view`, `pos→pos.view`, `fulfillment→fulfillment.view`, `questions→storefront.moderation.view`), bilinmeyen topic reddedilir (`HubException`), yetkisi olmayan abone olamaz; **kanal kapsamlı kullanıcı YALNIZ görebildiği kanalların gruplarına alınır** (`topic:<konu>:<kanal>`), kısıtsız kullanıcı (süper admin / kanal kapsamsız yetki) `topic:<konu>:all` grubuna. Gönderim tarafı da kanal duyarlı: `SendOrderEventAsync`/`SendQuestionEventAsync` artık kanal alır; sipariş olaylarının kanalı siparişten, POS satışınınki kasadan, ürün sorusununki sorudan çözülür. **Kanal bilinmiyorsa bildirim yalnız kısıtsız abonelere gider** (sızıntı yerine eksik bildirim). (2) **Toplu işlemlerde kayıt-bazlı doğrulama (§O.8):** yeni `KapsamDogrulama` yardımcısı — toplama planı oluşturma ve kargo yönlendirme uçları, istemciden gelen sipariş listesinin HER kaydının kanalını yeniden doğrular; kapsam dışı kayıt varsa istek **404** ile reddedilir (hangi siparişin var olduğu sızmaz). (3) **İnceleme sonucu iş çıkmayan iki kalem:** *Pazaryeri eşleme* — `MarketplaceMappingService` tüm sorgularında `FirmPlatformId == null` filtresi kullanıyor, yani eşlemeler tasarımı gereği FİRMA GENELİ (kanal bazlı değil); kanal kısıtı uygulanacak veri yok, `marketplaces.view` sayfa yetkisi doğru sınır. *Arama uçları* — panelin komut paletindeki üç arama (`/catalog/products`, `/orders`, `/crm/members`) zaten ya kanal kapsamlı (siparişler) ya da kanaldan bağımsız; ek iş gerekmedi. (4) **Bilinçli sınır:** toplama planı LİSTESİ kanal filtresine tabi değil — planlar depo operasyonudur ve kanal kolonu taşımaz; ama artık plana kapsam dışı sipariş ALINAMAZ. **Doğrulama (izole 5051, demo DB — test verisi ve oluşan plan silindi):** amazon kapsamlı kullanıcı telemania siparişiyle toplama planı oluşturmayı denedi → **404**, kargo yönlendirme → **404**; aynı istek süper adminle **201**. `SignalRKanalGruplariTests` (grup ayrımı regresyonu) + paket **230/230**. `publish` (DLL 5071d206) ⚠️ **restart**. **Y3 KAPANDI.** Yetkilendirme yol haritasında kalanlar: Y8 geçiş grubunun kaldırılması, grup kopyalama, Y6 artıkları (kâr-marj, tedarik/stok maliyeti, yazdırma/e-posta/mobil, alan izninin cache anahtarına girmesi), yetkisiz erişim denemelerinin loglanması.

**YETKİ İÇERİKLERİ EKRANI TAMAMLANDI (2026-09-09; Y4'ün kalan kalemi):** Panelde `/settings/permissions` — modül modül gruplanmış katalog (73 yetki: 30 sayfa, 38 işlem, 5 alan), her satırda görünen ad + açıklama, teknik anahtar (soluk), tür rozeti, kanal bazlı mı, **kullanım sayacı** (kaç grup / kaç kullanıcı istisnası) ve durum; ad/anahtar araması, tür filtresi ve **"yalnız pasif / uygulamada karşılığı olmayanlar"** süzgeci. Satır tıklanınca düzenleme: görünen ad, açıklama, sayfa grubu, sıra, aktiflik ve kanal kapsamı bayrağı — kanal bayrağı değiştirilirken "bu karar artık panelde kalır, mevcut atamaların kanal listesini gözden geçirin" uyarısı çıkar. Ekranın başında **K4 açıklaması**: yetkiler uygulamadan gelir, panelden üretilmez; ihtiyaç varsa geliştiriciye iletilir (koda bağlı olmayan yetki hiçbir şeyi korumaz, yalnız yanlış güven verir). Teknik anahtar hiçbir koşulda değişmez (audit geçmişi ona bağlı). **Doğrulama (izole 5051, demo DB):** katalog 73 kayıtla listelendi; panelden ad+açıklama değiştirildi → kayıt döndü, **tür ve kanal bayrağı koddan gelen değerini korudu**; değişiklik yetki loglarına düştü (*"orders.view yetkisinin gösterim bilgilerini güncelledi…"*); **uygulama yeniden başlatıldı ve katalog senkronu panelin yazdığı adı EZMEDİ** (`+0 eklendi, 0 güncellendi`) — K4'ün asıl güvencesi doğrulandı; test verisi geri alındı. Paket **228/228**. Backend değişmedi (uçlar Y4'te hazırdı) — yalnız `admin/dist` derlendi ⚠️ **restart gerekmez, panel yenilemesi yeter** (nginx `admin/dist`'i doğrudan sunar). **Yetkilendirme yol haritasında kalanlar:** Y8 geçiş grubunun kaldırılması (gerçek departman grupları kurulunca), grup kopyalama, Y3 artıkları (pazaryeri eşleme/arama/SignalR/toplu işlem kapsamı), Y6 artıkları (kâr-marj, tedarik/stok maliyeti, yazdırma/e-posta/mobil, alan izninin cache anahtarına girmesi), yetkisiz erişim denemelerinin loglanması.

**YETKİLENDİRME Y7 UYGULANDI (2026-09-09; KULLANICI SİMÜLASYONU — "bu kullanıcı panelde ne görüyor?"):** (1) **Salt okunur önizleme:** `GET /api/iam/users/{id}/simulation` — kullanıcının hesabına GİRİŞ (impersonation) YOK; hesabın sonucu hesaplanır. Kaynak, gerçek davranışla ayrışmasın diye **aynı `IEtkinYetkiServisi`**'dir (ayrı hesap yolu yazılmadı). Dönen veri: gruplar, görebildiği SAYFALAR, kullanabildiği İŞLEMLER (kanal adlarıyla), hassas alanlardan görebildikleri/göremedikleri, eriştiği kanallar ve panel menü filtresinin birebir uygulanabilmesi için yetki kodları. (2) **Ayrı yetki + LOG:** uç `iam.permissions.simulate` ister (yetkisiz kullanıcı **403**) ve her çalıştırma denetim kaydına yazılır — "kim neyi görüyor" bilgisi izsiz dolaşmaz. (3) **Panel:** Kullanıcı Yetkileri ekranına **Simülasyon** sekmesi — sol tarafta kullanıcının göreceği MENÜ (panelin gerçek `permittedSections` fonksiyonuyla üretilir, yani menüyle birebir aynı kural), sağda kullanabileceği işlemler + hassas alan durumu + eriştiği kanallar; hiç menü görünmüyorsa "panele girince boş ekranla karşılaşır" uyarısı. **Doğrulama (izole 5051, demo DB — veriler sonra silindi):** yalnız `orders.view/orders.manage` (telemania) + `customer.view_phone` yetkili kullanıcı için simülasyon → sayfalar: *Siparişleri Görüntüle*; işlemler: *Sipariş İşlemleri (Telemania)*; görünen alan: *Müşteri Telefonunu Gör*; gizli alanlar: maliyet, kâr, adres, personel notu; kanallar: *Telemania*. Simülasyon logu: *"Y7 User kullanıcısının panel görünümünü simüle etti (1 sayfa, 1 işlem yetkisi)"*. Yetkisiz kullanıcı 403. Paket **228/228**. `publish` (DLL f75a9096) + `admin/dist` ⚠️ **restart**. **Kalan (yol haritası):** Y4'ten Yetki İçerikleri ekranı + grup kopyalama; Y3'ten pazaryeri eşleme/arama/SignalR/toplu işlem kapsamı; Y6'dan kâr-marj, tedarik/stok maliyet yüzeyleri, yazdırma/e-posta/mobil çıktıları ve alan izninin cache anahtarına girmesi; Y8 geçiş grubunun kaldırılması (gerçek departman grupları kurulunca).

**YETKİLENDİRME Y6 UYGULANDI (2026-09-09; HASSAS ALAN YETKİLERİ — K6'nın beş alanı):** (1) **Kural tek yerde:** yeni `Shared.Kernel/Authorization/AlanIzinleri` — `Telefonla/Adresle/Notla` metin alanını maskeler (`•••`), `Maliyetle/Karla` sayısal alanı **null** yapar, `KolonGorunur(etiket)` export kolonunu süzer. **Varsayılan kurucu HER ŞEYİ KAPALI üretir** (default deny): izin geçirmeyi unutan çağıran veri sızdırmaz, maskeler. Boş değer maskelenmez ("boş" ile "gizli" karışmaz). (2) **İstek servisi:** `IAlanYetkileri` — süper adminde tam, aksi hâlde efektif yetkiden (`common.view_cost`, `common.view_margin`, `customer.view_phone`, `customer.view_address`, `common.view_internal_notes`). (3) **Uygulandığı yüzeyler:** sipariş DETAYI (telefon + adres + personel notu), üye listesi ve üye detayı (telefon), müşteri talebi listesi (müşteri/arayan telefonu), ürün detayı (ürün + VARYANT maliyeti). Sipariş/üye/talep LİSTELERİ zaten telefon-adres taşımıyordu (yalnız export satırları taşıyor). (4) **Export kolonları:** `GridExportColumn.AlanYetkisi` etiketi eklendi (`phone/address/cost/margin/notes`); merkezî `GridExportEndpoint` yetkisi olmayan kolonu **hiç oluşturmaz**. **★ Yakalanan kaçak:** sipariş export'u merkezî yardımcıyı KULLANMIYOR, kolonları elle seçiyordu → süzme oraya da eklendi; izole ölçüm: alan yetkisi olmayan kullanıcının dosyası **16 kolon (Telefon/Adres YOK)**, süper adminin **18 kolon**. **Doğrulama (izole 5051, demo DB — veriler sonra silindi):** alan yetkisi olmayan kullanıcı sipariş detayında telefon/adres `•••`, üye listesinde telefon `•••`, ürün maliyeti `null`; süper admin gerçek değerleri görüyor (05522088867 / Fevzi Çakmak Mah… / 42,50). `AlanIzinleriTests` 5/5 → paket **228/228**. `publish` (DLL 0398647a) ⚠️ **restart**. **Not:** geçiş grubu bu beş yetkiyi İÇERİYOR → mevcut 4 kullanıcı için görünüm değişmez; yeni gruplar bunları açıkça vermelidir. **Y6 kalanı:** kâr/marj alanları (henüz DTO'da yok), tedarik/stok maliyet yüzeyleri, fatura/etiket YAZDIRMA çıktıları, e-posta şablonları ve mobil/dış API cevapları; ayrıca alan izinlerinin cache anahtarına girmesi (DTO cache'i olan yüzeylerde). **SIRADA:** Y7 (simülasyon) ya da Y4 kalanı (Yetki İçerikleri ekranı) / Y3 kalanı.

**YETKİLENDİRME Y5 UYGULANDI (2026-09-09; DENETİM KAYDI + Yetki Logları ekranı + süper admin kuralları):** (1) **Denetim yazıcısı:** `IYetkiDenetimi`/`YetkiDenetimi` — kayıtlar mevcut `iam.iam_audit_logs` tablosunda `yetki.*` EntityType ailesiyle tutulur; her kayıt **insan-okur ÖZET cümle** + aktör + IP + "önce → sonra" taşır. Bağlam (aktör/IP/UA) API katmanından (`YetkiDenetimBaglami`) gelir. Kayıtlar **yalnız EKLENİR** — uygulamada bu satırları güncelleyen/silen hiçbir yol yoktur. (2) **Loglanan olaylar:** grup oluştur/güncelle, grubun yetkileri (önce/sonra listesi + kaç eklendi/çıkarıldı), gruba üye ekle/çıkar, kullanıcı istisnası ver/kapat/temizle (kanal sayısıyla), katalog gösterim güncelleme, **süper admin ver/kaldır (ayrı ve kritik olay)**. (3) **Süper admin kuralları (tasarım §K) uçla birlikte geldi:** `PUT /api/iam/users/{id}/super-admin` — yalnız süper admin yapabilir, **kullanıcı KENDİ bayrağını kaldıramaz**, sistemde en az bir aktif süper admin kalmalıdır. (4) **Panel:** `/settings/permission-logs` — olay tipi filtresi, özet araması, satır tıklayınca teknik ayrıntı (olay kodu, hedef, önce/sonra, IP); kritik olaylarda rozet. Sidebar'a `iam.audit.view` yetkisiyle eklendi. **Doğrulama (izole 5051, demo DB — veriler sonra silindi, pasife alınan demo süper admini geri açıldı):** grup oluştur → yetki ver → üye ekle → istisna → süper admin ver/kaldır zinciri **6 kayıt** üretti ve hepsi doğru cümleyle listelendi (ör. *"Y5 User kullanıcısında \"orders.view\" yetkisini kapattı (2 kanal)"*, *"Y5 User kullanıcısına SÜPER ADMİN yetkisi verdi"*); süper adminin **kendi bayrağını kaldırma denemesi reddedildi**; süper admin olmayan kullanıcının süper admin verme denemesi **403**. Paket **223/223**. `publish` (DLL 5a859870) + `admin/dist` hazır ⚠️ **restart**. **Kalan:** yetkisiz erişim denemelerinin (403/404) örneklenerek loglanması, log ekranına aktör/hedef kullanıcı filtreleri, DB rolü seviyesinde UPDATE/DELETE kısıtı (uygulama tarafı zaten kapalı). **SIRADA:** Y6 (alan yetkileri) ya da Y4 kalanı (Yetki İçerikleri ekranı) / Y7 (simülasyon).

**YETKİLENDİRME Y4 UYGULANDI (2026-09-09; PANEL EKRANLARI — Yetki Grupları + Kullanıcı Yetkileri):** (1) **Backend:** `Iam.Application/Yetkilendirme` — 4 sorgu (katalog, grup listesi, grup detayı, kullanıcı yetkileri) + 5 komut (grup kaydet, grup yetkilerini topluca kaydet, grup üyeliği, kullanıcı istisnası ver/kaldır/temizle, katalog gösterim alanları) ve `YetkilendirmeController` (`api/iam/permissions`, `permission-groups`, `users/{id}/permissions`; hepsi `iam.permissions.manage`). Efektif yetki ekranı **aynı `EfektifYetkiHesabi` kuralını** kullanır — ekran ile gerçek davranış ayrışmaz. (2) **ESKALASYON KORUMASI çalışıyor (tasarım §O.4):** yetkilendiren kullanıcı yalnız KENDİ taşıdığı yetki ve kanalları dağıtabilir; izole testte `iam.permissions.manage` sahibi bir kullanıcının kendine `system.migration.manage` vermesi **reddedildi**. Süper admin muaf. (3) **Panel ekranları:** `/settings/permission-groups` (grup listesi + detay: modül ağacında yetki seçimi, modül başlığından toplu seç/kaldır, arama, kanal kapsamlı yetkide `Kanallar: n/m` hapı ve kanal seçici, üyeler sekmesi) ve `/settings/users/:id/permissions` (gruplar + **efektif yetki tablosu: kanallar, KAYNAK, açık/kapalı ve KAPALI SEBEBİ**, satır içi "bu kullanıcıda aç/kapat", istisnayı kaldır). Kullanıcı listesine **"Yetkileri"** kısayolu eklendi (§19), sidebar'a Yetki Grupları kalemi (yetkisiz kullanıcıya görünmez). (4) **★ Y4'te yakalanan gerçek hata:** panelden "bu kullanıcıda kapat" isteği kanal listesi olmadan gelince kaldırma kaydı BOŞ kümeyle yazılıyor ve **hiçbir kanalı düşürmüyordu** (yetki açık kalıyordu). API artık kanal verilmediğinde "tüm kanallar (bugünkü liste)" yazıyor (Ek-2); kural `EfektifYetkiHesabiTests`'e regresyon testi olarak sabitlendi. **Doğrulama (izole 5051, demo DB, uçtan uca — veriler sonra silindi):** katalog 73 yetki listelendi; panelden grup oluşturuldu, gruba `orders.view` (yalnız telemania) + `crm.members.view` verildi, kullanıcı gruba eklendi; kullanıcı yetkileri ekranı **kaynak = grup adı** ve kanal sayısıyla döndü, kapalı yetkiler sebebiyle listelendi; kullanıcı sipariş listesini gördü (10 kayıt), "bu kullanıcıda kapat" sonrası **aynı token ile hemen 403**, istisna kaldırılınca **hemen 200**. Paket **223/223**. `publish` (DLL b649ca2b) + `admin/dist` hazır ⚠️ **restart**. **Y4 kalanı:** Yetki İçerikleri ekranı (katalog gösterim düzenleme — ucu hazır, ekranı yok), simülasyon sekmesi (Y7), grup kopyalama. **SIRADA Y5:** yetki olaylarının audit'i + Yetki Logları ekranı.

**YETKİLENDİRME Y3 3. TUR UYGULANDI + CANLI HATA DÜZELTMESİ (2026-09-09):** (1) **Kanal PARAMETRELİ ekranlar tek düzenekle kapsandı:** yeni `[KanalKapsamiKontrol(yetki)]` eylem filtresi — model bağlandıktan sonra aksiyon argümanlarında `firmPlatformId` (ya da platformId/channelId) arar, kullanıcının o kanala erişimi yoksa isteği **404** ile keser (403 değil: kanalın/kaydın varlığı sızmasın). Sınıf düzeyinde 9 controller'a kondu: Navigation (menü/kanal ürünleri/kanal kategorileri), Pages (vitrin blokları), Cms, CardMessages, StoreNotifications (push/bülten), Marketplaces, Collections, Fulfillment, TrackingAdmin. Böylece bu ekranların **56+ kanal parametreli ucu** tek satırla korundu. Süper adminde ve kanal kapsamsız yetkide denetim yapılmaz; kanal parametresi taşımayan uçlar etkilenmez. (2) `IKanalKapsami.ErisebilirMiAsync(yetki, kanalId)` eklendi (tek kanal sorusu). (3) **Doğrulama (izole 5051, demo DB, amazon kapsamlı kullanıcı):** vitrin blokları / menü listesi / CMS sayfaları → **telemania 404, amazon 200**; filtre birim testi (kapsam dışı 404, kapsam içi geçer, süper admin geçer) + paket **222/222**. **(4) ★ CANLI HATA (benim değişikliklerimden DEĞİL, 2026-09-07 CRM talep işinden kalma):** restart sonrası logda `column p.Name does not exist` — CRM talep ekranındaki **sipariş arama** ucu (`api/crm/tickets/order-lookup`) `core_firm_platforms.Name` kolonunu okuyordu; o tabloda `NameI18n` var. `CrmTicketOrderLookup` sorgusu `COALESCE(p."NameI18n"->>'tr', p."Code", '')` olarak düzeltildi; izole ortamda uç artık kanal adıyla (`platformName: "Telemania"`) 200 dönüyor. `publish` eşitlendi (DLL 4836e11e) ⚠️ **restart**. **Y3'ün kalanı (küçüldü):** pazaryeri eşleme listeleri (kanal parametresizler), arama uçları, SignalR bildirim kapsamı, toplu işlemlerde kayıt-bazlı kapsam doğrulaması, toplama planlarının paket üzerinden kanal filtresi. **SIRADA: Y4 — panel ekranları (Yetki Grupları, Kullanıcı Yetkileri + efektif yetki/kaynak görünümü, Yetki İçerikleri).**

**YETKİLENDİRME Y3 2. TUR UYGULANDI (2026-09-09; kanal kapsamı — sipariş türevi ve moderasyon listeleri):** (1) **Kanal kolonu OLMAYAN listeler bağlandı:** iade (`r => r.Order.FirmPlatformId`) ve fatura (`i => i.Order.FirmPlatformId`) siparişten, POS satışı kasadan (`s => s.Register.FirmPlatformId`) kanalını alır; bunun için `Return.Order` ve `PosSale.Register` navigasyonları eklendi (FK'lar zaten vardı → **migration YOK**). İade/fatura/POS liste + export uçları artık gerçek kapsamdan geçiyor (önceki turda `null` geçiyorlardı). (2) **Moderasyon listeleri:** iletişim mesajları, ürün soruları ve yorum moderasyonu sorgularına `KanalKisiti` eklendi (grid dışı sorgular) — kapsam dışı kanalın müşteri mesajı/sorusu/yorumu listede görünmez. (3) **Detay 404 genişletildi:** kampanya detayı ve müşteri talebi detayı da kapsam dışında **404** döner (sipariş detayı 1. turda yapılmıştı) — varlık sızmaz. (4) **Dashboard:** panel dashboard'u zaten aynı liste uçlarını (`/orders?pageSize=1`, `/pos/sales?pageSize=1` …) çağırdığından **kendiliğinden kapsamlı** hale geldi; ayrı iş gerekmedi. **Doğrulama (izole 5051, demo DB, amazon kapsamlı tek kullanımlık kullanıcı + süper admin karşılaştırması, veriler sonra silindi):** telemania siparişine bağlı iade → kapsamlı kullanıcı **0**, süper admin **1** (Order join'i SQL'de çalışıyor); siparişler **0** / **10**; iletişim mesajı **0** / **1**; telemania sipariş detayı **404** / **200**. Paket **221/221**. `publish` eşitlendi (DLL 5a72b01e) ⚠️ **restart**; canlıda geçiş grubu tüm kanalları içerdiğinden görünür davranış değişmez. **★ Y2'den gelen bilinen sınır doğrulandı:** iade/fatura/teklif sayfaları OrderController altında olduğundan **`orders.view` yetkisi de gerekir** (sınıf düzeyi sayfa yetkisi) — gruplar kurulurken `orders.view` + ilgili alt yetki birlikte verilmelidir. **Y3'ün KALANI (3. tur):** vitrin/CMS listeleri (cms_pages, page_blocks, channel_products, collections, newsletter, push_notifications, card_messages), toplama planları (paket üzerinden kanal), pazaryeri eşleme listeleri, arama uçları, SignalR bildirimleri ve toplu işlemlerde kayıt-bazlı kapsam doğrulaması. **SIRADA:** Y3 3. tur ya da Y4 (panel ekranları).

**YETKİLENDİRME Y3 (1. tur) UYGULANDI (2026-09-09, API + çekirdek; tasarım Y3 — KANAL KAPSAMININ VERİYE İNMESİ, karar K2):** (1) **Çekirdek mekanizma:** `GridSchema<T>.Kanal(selector)` ile liste şeması kanal kolonunu BİLDİRİR; `ApplyKanalKapsami(query, izinliKanallar)` kısıtı sorguya uygular — **null = kısıt yok** (süper admin ya da kapsamsız yetki), **boş küme = hiçbir satır** (default deny), bildirimsiz şemada kısıt istenirse **hata fırlatır** (sessiz sızıntı yerine gürültü). `GridRequest.KanalKisiti` kısıtı liste/sayaç/export boyunca taşır (`Without` ve `ToGridRequest` de korur). (2) **Kapsam kaynağı:** yeni `IKanalKapsami` (istek kapsamlı) — süper adminde null, kanal kapsamsız yetkide null, aksi hâlde `IEtkinYetkiServisi`'nden gelen kanal kümesi. (3) **Derleyici zorlaması:** `GridRequestParser.Parse` ve `GridExportRequest.ToGridRequest` kanal kısıtını **ZORUNLU parametre** olarak alır — 19 liste + 14 export ucunun HEPSİ kapsam kararını açıkça verir (kanaldan bağımsız listelerde gerekçeli `null`). (4) **Kapsanan listeler:** Siparişler (liste + durum sayaçları + export), Teklifler (+export), Kampanyalar, Müşteri Talepleri, Takip Outbox. Sipariş **detayında** kapsam dışı kayıt **404** döner (403 değil — varlığı sızmasın, tasarım §C.2). (5) **Koruyucu test:** `KanalKapsamiTests` — `FirmPlatformId` kolonu olan her grid şeması kapsam bildirmek ZORUNDA (bildirmezse test kırılır), boş küme → 0 satır, bildirimsiz şemada kısıt → hata. **★ Yakalanan gerçek kaçak:** export ucu gövdeden `ToGridRequest()` ile geçtiği için ilk turda kapsamı ATLIYORDU — izole testte kapsam dışı kullanıcının Excel'i süper adminle BİREBİR aynı çıktı; parametre zorunlu yapılıp tüm export uçları bağlandı, tekrar ölçüldü: kapsam dışı export sayfası 186 bayt (boş), süper admin 10.329 bayt. **Doğrulama (izole 5051, demo DB, tek kullanımlık kullanıcı+grup — sonra silindi):** telemania kapsamlı kullanıcı 10 sipariş görüyor; kapsam amazontest'e çevrilince **liste 0**, **durum sayaçları hepsi 0**, **detay 404**, **export boş**; süper admin 10 sipariş + dolu export. `/auth/me` kanal kümesini döndürüyor (`orders.view → [telemania]`). Paket **221/221** (33 atlanan). `publish` eşitlendi (DLL 904af4db) ⚠️ **restart** — canlıda geçiş grubu tüm kanalları içerdiğinden **görünür davranış değişmez**. **Y3'ün KALAN işleri (2. tur):** iade/fatura listeleri (kanal kolonu yok — sipariş üzerinden join), POS satışları (kasa üzerinden), toplama planları (paket üzerinden), vitrin/CMS/moderasyon listeleri (contact_messages, product_questions, product_reviews, collections, cms_pages, channel_products, newsletter, push), dashboard sayaçları, arama, SignalR bildirimleri, toplu işlemler ve teklif/kampanya/talep DETAY uçlarının 404 kontrolü. **SIRADA:** Y3 2. tur ya da Y4 (panel ekranları) — kullanıcı kararı.

**YETKİLENDİRME Y2 UYGULANDI (2026-09-09, Admin panel + API; tasarım Y2 — UÇ KAPLAMASI + K8 geçiş grubu):** (1) **Katalog 19 → 73 yetki:** panel sayfa envanterinden (`sidebarNavigation`) türetilen 54 yeni sayfa/aksiyon yetkisi — her alan için `.view` (sayfa) + `.manage` (işlem) çifti (orders/returns/invoices/quotes/pos/fulfillment/crm/moderation/accounts/commission/finance/promotion/notifications/tracking/content/cms/definitions/system/iam…); sipariş, POS, operasyon, kampanya, vitrin, kanal ve moderasyon yetkileri **kanal kapsamlı** işaretlendi (K1). (2) **Uç kaplaması:** 32 panel controller'ına sınıf düzeyi görüntüleme yetkisi + tüm yazma uçlarına (POST/PUT/DELETE/PATCH) işlem yetkisi eklendi — **303 yeni attribute**; rota bazlı ezmelerle sipariş modülü içinde iade/fatura/teklif/hediye-kartı ayrıştırıldı. **Export/önizleme uçları OKUMA sayılır** (19 uçtan işlem yetkisi geri alındı; sayfa yetkisi yeterli — K2/K6 gereği kolon kısıtı Y6'da). Panel ucu sayısı 514 → **kaplanan %100**. (3) **Kaplama düzeneği:** `YetkiKaplamaTests` — panel yüzeyindeki her aksiyon ya bir yetkiye bağlı olmalı ya da GEREKÇESİYLE istisna listesinde olmalı (bugün 7 istisna: login/refresh/me/şifre değiştir/api-token/kendi tercihleri); ikinci test attribute'lardaki her key'in katalogda tanımlı olmasını ister (hayalet yetki koruması). Yeni uç eklenip yetkisi unutulursa **test kırılır**. (4) **K8 geçiş grubu:** açılışta idempotent `gecis_tam_erisim` grubu — bugünkü erişime yakın kapsam (56 yetki; kanal kapsamlılara O ANKİ kanalların listesi, Ek-2), yalnız **hiç rolü olmayan aktif kullanıcılara** atanır (gerçek gruba taşınan bir daha eklenmez). **Bilinçli sıkılaştırma:** `iam.*` (kullanıcı/yetki yönetimi) ve `system.migration.manage` geçiş grubuna DAHİL DEĞİL — uçları bugüne kadar korumasızdı, artık yalnız süper admin. (5) **Panel menüsü:** 56 menü kalemi sayfa yetkisine bağlandı (yetkisiz kullanıcı kalemi görmez; menü gizlemek güvenlik değildir, uçlar sunucuda ayrıca korunur). **Doğrulama:** paket **218/218** (33 atlanan); izole 5051 demo DB uçtan uca: açılışta `Katalog senkronu: +54`, `Geçiş grubu: +56 yetki, +1 kullanıcı`; geçiş grubundaki kullanıcı Siparişler/Üyeler/Kampanyalar/Stoklar/CMS **200**, iam.users/migration/procurement **403**, platform yazma uçları **403**, sipariş yazma ucu yetkiden geçti (400 = gövde hatası); süper admin bayrağıyla iam.users **200**; kullanıcı gruptan çıkarılıp yönetici tarafından güncellenince (önbellek düşer) **403** → default deny doğrulandı. `publish` eşitlendi (DLL 75403cd0), `admin/dist` derlendi ⚠️ **restart** — restart'ta canlı DB'de katalog 19→73'e çıkar ve geçiş grubu 4 aktif kullanıcıya (Emre, halisy, murat, serhat) atanır; süper adminler (Gokce, admin, ekremtopkara, esmatopkara) bayrakla geçer. **Bilinen sınırlar:** kaplama alan bazında KABA (sayfa+işlem çifti; "sipariş iptal" gibi tekil aksiyonlar Y2 kapsamında ayrıştırılmadı — brief §7 ayrıntısı sonraki turda), kanal kümeleri hesaplanıyor ama listeleri henüz FİLTRELEMİYOR (Y3), alan yetkileri katalogda var ama zorlama Y6'da, geçiş grubu Y8'de kaldırılacak. **SIRADA Y3:** kanal kapsamının veriye inmesi (grid çekirdeği + sayaç/dashboard/arama/export/toplu işlem/SignalR).

**YETKİLENDİRME Y1 UYGULANDI (2026-09-09, IAM + panel; tasarım Y1 fazı — efektif yetki servisi + K3 "anında etkili"):** (1) **Yetki artık TOKEN'DA TAŞINMIYOR:** `JwtTokenService` permission claim'i yazmaz (login/refresh'teki permission sorguları da kalktı); token yalnız kimlik + `sa` (süper admin) taşır. Kontrol her istekte yeni `IEtkinYetkiServisi`'nden okunur → verilen yetki HEMEN geçerli, kaldırılan yetki HEMEN kapalı (60 dk token ömrü beklenmez). (2) **Tek kural:** `Shared.Kernel/Authorization/EfektifYetkiHesabi.Hesapla(superAdmin, kaynaklar)` → `(gruplar ∪ kullanıcı-ver) − kullanıcı-kaldır`; kaldırma EN SONDA uygulanır (Ek-1 üstünlüğü), kanal kapsamlı yetkide sonuç kümesi boşsa yetki yok (default deny), kapsamsız yetkide kanal sorulmaz. Sonuç `EfektifYetkiler` iki bilgi taşır: izin + KANAL KÜMESİ (`Var(key)`, `Var(key, kanalId)`, `Kanallar(key)`). (3) **Servis + önbellek:** `EtkinYetkiServisi` (Iam.Infrastructure) — L1 `IMemoryCache` 10 sn (düğüm-yerel) + L2 `ICacheService`/Redis 5 dk; `GecersizKilAsync(userId)` / `GrupIcinGecersizKilAsync(roleId)`; **cache erişilemezse DB'den okunur — fail-safe KAPALI** (cache yokluğu yetkiyi asla açık varsaymaz). Rol atama ve kullanıcı güncelleme komutlarına geçersiz kılma bağlandı. (4) **`RequirePermissionAttribute` artık async** ve servisten okur; servis DI'da yoksa 503 döner (açık varsaymaz). (5) **`/api/auth/me`** efektif yetkileri + kanal kümelerini (`channels`) + `isSuperAdmin` döndürür; panel menü/buton görünürlüğü buradan beslenir. **Doğrulama:** `EfektifYetkiHesabiTests` 11/11 (tasarım §D çakışma tablosunun her satırı: iki grup birleşimi, grubun vermemesi≠yasak, kullanıcı verme, kanal bazlı kaldırma — kararlar belgesindeki G+J+O → Julude kaldır → G+O örneği dahil, kaldırmanın kullanıcı vermesini de yenmesi, kapsamsız yetki, süper admin) + `RequirePermissionAttributeTests` 5/5 (regresyon: **token'daki permission claim'i artık yetki VERMEZ**) + `EtkinYetkiServisiDbTests` (gerçek DB, işlem geri alınır: grup/istisna/kanal/pasif permission/pasif kullanıcı/önbellek) → paket **216/216** (33 atlanan). **İzole 5051 uçtan uca (demo DB, tek kullanımlık test kullanıcısı — sonra silindi):** token'da permission claim'i YOK ✓; yetkisiz uç 403 ✓; uygulama ucundan rol atandı → **AYNI token ile hemen 200** (yeniden giriş yok) ✓; kullanıcı istisnası "kaldır" eklendi → **aynı token ile hemen 403** (grup hâlâ veriyor olmasına rağmen — Ek-1) ✓; `/auth/me` efektif listeyi doğru döndü ✓. `publish` eşitlendi (DLL 5d42b583) ⚠️ **restart** — restart anında tüm panel kullanıcıları yetkilerini yeni yoldan alır; **mevcut 4 süper admin bayrakla, diğer 45 kullanıcı zaten yetkisizdi, davranış değişmez.** Bilinen sınır: çoklu düğümde diğer düğümün L1'i en fazla 10 sn eski kalabilir (tek düğümde anında); panel `/auth/me`'yi sayfa yüklemede çeker — açık sekmede yetki değişimi bir sonraki yüklemede görünür (Y4'te canlı tazeleme). **SIRADA Y2:** uç kaplaması (514 panel ucundan 37'si kaplı; 404/403 ayrımı).

**YETKİLENDİRME Y0 UYGULANDI (2026-09-09, Admin panel + IAM; tasarım `docs/panel-yetki-sistemi-tasarim.md` Y0 fazı — katalog + süper admin bayrağı + kanal kümesi altyapısı):** (1) **Kod sahipli katalog (K4):** yeni `Shared.Kernel/Authorization/PermissionKatalogu` — her yetkinin değişmez key'i + türü (`page|action|field`) + modül/sayfa + kanal-kapsamlı bayrağı + görünen ad; `PermissionKatalogSenkronu` açılışta DB ile eşitler: katalogdaki yeni yetkiyi ekler (kimseye ATAMAZ — default deny), kod sahipli alanları günceller, **panelin yazdığı görünen ad/açıklama/sıra ASLA ezilmez**, katalogda olmayan kayıt `IsCodeDefined=false` + pasif olur (silinmez, audit korunur). Kanal-kapsamlı bayrağında heuristik yok: `ChannelScopedOverridden` açık bayrağı — panel değiştirene kadar kod değeri geçerli. (2) **Süper admin artık BAYRAK (K5):** `iam_users.IsSuperAdmin` + token'da `sa` claim'i + `RequirePermissionAttribute` bayrakla bypass; **ölü `permission == "*"` dalı kaldırıldı** (DB'de böyle bir permission hiç yoktu; super_admin 11 permission'ı tek tek aldığı için geçiyordu → koda yeni permission eklenince süper admin de kilitleniyordu, artık kilitlenmez). Migration veri adımı: super_admin rolündeki 4 kullanıcıya bayrak yazıldı (Gokce, admin, ekremtopkara, esmatopkara). `/api/auth/me` artık `isSuperAdmin` döner; panel `hasPermission` bayrağı önce kontrol eder. (3) **Kanal kümesi altyapısı (K1/Ek-2):** `iam_role_permissions.ChannelIds` + `iam_user_permissions.ChannelIds` (jsonb Guid listesi); `UserPermission` artık kullanıcı istisnası olarak belgelendi (grant=ver, revoke=kaldır) ve benzersizlik `(UserId, PermissionId)` yapıldı — eski `(UserId, PermissionId, FirmId)` indeksi FirmId NULL iken Postgres'te tekillik SAĞLAMIYORDU. `FirmId` kolonu kullanılmıyor (kapsam birimi kanal). (4) **Katalog içeriği:** mevcut 11 yetki + 3 yetkilendirme yetkisi (`iam.permissions.manage` / `iam.permissions.simulate` / `iam.audit.view`) + K6'nın 5 hassas alan yetkisi (`common.view_cost`, `common.view_margin`, `customer.view_phone`, `customer.view_address`, `common.view_internal_notes`) — yeni 8 kayıt bilinçli olarak `AllPermissions` seed listesine EKLENMEDİ (kimseye otomatik verilmez). Migration'lar: `20260909112218_AddPermissionCatalogAndSuperAdmin` + `20260909112941_AddPermissionChannelScopeOverride` — ikisi de additive, **canlı + demo DB'ye uygulandı** (eski binary'ler etkilenmez). Doğrulama: `PermissionKataloguTests` 6/6 + `RequirePermissionAttributeTests` 4/4 (regresyon: `"*"` artık geçmez) + `PermissionKatalogSenkronuDbTests` (gerçek DB, işlem geri alınır: ekleme, panel alanlarının korunması, yetim pasifleme, idempotenslik, kanal bayrağı önceliği) → paket **203/203** (33 atlanan); izole 5051 demo DB açılışı: `Katalog senkronu: +8 eklendi, 11 güncellendi, 0 pasife alındı`, ikinci çağrı 0/0/0 (idempotent). `publish` eşitlendi (DLL ffb18cd5), `admin/dist` derlendi ⚠️ **restart (`sudo systemctl restart ecspros`)** — restart'ta katalog senkronu canlı DB'de de çalışacak (11 → 19 yetki). **Davranış değişmedi:** yeni yetkiler kimseye atanmadı, mevcut permission kontrolleri aynı; süper adminler bayrağı token yenilenince (en geç 60 dk / bir sonraki girişte) alır, o ana kadar zaten 11 permission'la geçiyorlar. **SIRADA Y1:** efektif yetki servisi + K3 tazeliği (permission claim'lerinin token'dan çıkarılması, L1/L2 cache, `pv` sürümü).

**PANEL YETKİLENDİRME — NİHAİ TASARIM YAZILDI (2026-09-09, Admin panel; kullanıcı kararları `docs/panel-yetki-sistemi-soru-cevaplari.md` K1-K8 + 2 ek karar → çıktı `docs/panel-yetki-sistemi-tasarim.md` v1, 436 satır; belge gereği KOD YAZILMADI):** Kararlar: kapsam birimi KANAL (firma ayrı scope değil), kanal kapsamı listeleri/raporu/exportu da filtreler, yetki değişikliği ANINDA etkili, katalog KOD SAHİPLİ (panel yalnız ad/açıklama/tür/sıra/aktiflik/kanal-bayrağı), süper admin ayrı SİSTEM BAYRAĞI (permission değil), alan yetkisi v1'de 5 hassas alanla sınırlı, geçici yetki v1'de yok, geçiş legacy grupla kademeli; gruplar yalnız VERİR, kullanıcı istisnası (kanal kapsamlı) daha güçlüdür; "tüm kanallar" = anlık liste (yeni kanal otomatik kapsanmaz). **Tasarımın kritik teknik kararları:** (1) K3 için permission claim'leri JWT'den ÇIKARILIR — efektif yetki sunucuda L1 (süreç içi) + L2 (Redis) cache'ten, değişimde invalidation + `pv` sürüm claim'i; cache yoksa DB'den okunur (fail-safe kapalı, asla açık varsayılmaz); (2) efektif hesap `(gruplar ∪ kullanıcı-ver) − kullanıcı-kaldır`, sonuç İKİ bilgi taşır: izin + KANAL KÜMESİ; (3) grup modeli REFERANS (materyalize değil) — "nereden geliyor" doğal cevaplanır, grup değişikliği anında yansır; (4) kapsam dışı kayıt 404 / kapsam içi yetkisiz aksiyon 403; (5) kanal filtresi TEK NOKTADAN (istek bağlamı + ortak `GridSchema<T>` çekirdeği — 18 grid şeması) uygulanır, sayfa sayfa Where yazılmaz; SignalR bildirimleri de kapsama tabi; (6) süper admin kendi bayrağını KALDIRAMAZ + en az bir süper admin kuralı + ayrı audit tipi; (7) permission taslağı fikri REDDEDİLDİ — yerine katalog ekranında "yeni yetki iste" notu; (8) yetki verme eskalasyon koruması: yetkilendiren kullanıcı yalnız KENDİ sahip olduğu yetki/kanalı dağıtabilir. Uygulama sırası Y0-Y8 olarak eklendi (Y0 katalog+bayrak → Y1 efektif servis/tazelik → Y2 uç kaplaması (514 uçtan 37'si kaplı) → Y3 kanal kapsamının veriye inmesi → Y4 ekranlar → Y5 audit → Y6 alan yetkileri → Y7 simülasyon → Y8 geçiş). **Kod/SQL/migration YOK; canlı etkilenmedi.** Sıradaki iş: kullanıcı onayı sonrası Y0.

**KANAL BAZLI KARGO ÜCRETİ + ÜCRETSİZ KARGO LİMİTİ UYGULANDI (2026-09-09, Web sitesi + Admin panel + Order/Promotion; kullanıcı bildirimi "sepette kargo ücretsiz yazıyor, neden?"):** Teşhis: kargo ücreti sistemde HİÇ hesaplanmıyordu — sepet/ödeme/onay özetlerindeki "Ücretsiz" tasarım şablonundan kalma SABİT metindi (`_SepetSayfasi.cshtml` satır 259 vb.), `CartDto`'da kargo alanı yoktu, `CargoOptionDto`'da fiyat yoktu, checkout `TotalExpense`'e yalnız kapıda ödeme bedelini yazıyordu. İkinci bulgu: definition kataloğunda `free_shipping` ("Kargo Kampanyası") tipi + tam şeması (thresholdType/thresholdValue/paymentMethods/coverage/coverageValue) vardı ama `CampaignEngine` switch'inde karşılığı YOKTU → panelde tanımlanan kargo kampanyaları sessizce çalışmıyordu (2026-08-27'de discount/bundle'da görülen hatanın aynısı). **Kullanıcı kararları:** (K1) kanal başına TEK sabit ücret, (K2) ücretsiz kargo eşiği İNDİRİMLER SONRASI ödenecek tutara bakar, (K3) kampanya yalnız ücreti düşürür — koşulu tutmazsa kanal ayarı işler, iki sonuçtan müşteri lehine olan uygulanır, (K4) kapsam = kanal ayarı + kampanya motoru birlikte. **Uygulama:** yeni `Shared.Contracts.KargoUcretiKurali` (TEK kural: `Hesapla(kanalUcret, kanalEsik, odenecekUrunTutari, kampanyaUcreti) → (Ucret, BedavaSebebi, KampanyaEtkili)` + `EsigeKalan`); `IShippingOptionsProvider`/`ShippingOptionsProvider` (FirmPlatform.Settings `shippingFee`/`freeShippingThreshold`, 1 dk cache — PaymentOptionsProvider kalıbı); `CampaignEngine.KargoUcreti` (full/percent/amount kapsamı + sepet eşiği + ödeme yöntemi kısıtı — yöntem seçilmemişken kısıtlı kampanya UYGULANMAZ, sepette bedava yazıp ödemede tahsil etmemek için); `ProductCampaignResolver` kargo kampanyalarını (CampaignType.Scope='shipping') ürün-bazlı KAZANAN seçiminden ayırır (indirim kampanyasını bastırmasın) ve `CartCampaignResult.Shipping` olarak döndürür; `CartDto`'ya additive `shippingFee`/`freeShippingThreshold`/`freeShippingCampaign`/`shippingCampaignFee`; checkout kargoyu SUNUCUDA hesaplayıp yeni `Order.ShippingFee` kolonuna yazar ve `GrandTotal`'a ekler (migration `20260909104823_AddOrderShippingFee`, additive — eski binary'ler etkilenmez); legacy sipariş senkronu kargoyu ayrı masraf kalemi + `expenseTotal`'a ekler (`Legacy:OrderService:CargoExpenseTypeId`, 0 = satır yazılmaz); analytics `shipping` alanı artık gerçek kargo bedeli (önceden kapıda ödeme bedeli yazılıyordu). **Site:** ortak ayna `_KargoHesap.cshtml` (`window.msKargoHesapla`/`msKargoYaz` — tek kopya) → sepet (kargo satırı + "… TL daha ekleyin, kargo ücretsiz." + ödenecek tutara ekleme), teslimat ("Kargo Ücreti" satırı; üstteki "Kargo" satırı taşıyıcı adıdır), ödeme (kargo satırı + toplam), sipariş tamamlandı (siparişin gerçek bedeli); onay özetindeki `odenecek` artık kampanya indirimini de düşüyor (eski gösterim tutarsızlığı). **Panel:** Ayarlar → Kanallar'da "Kargo Ücreti" bölümü (kargo ücreti + ücretsiz kargo sepet limiti). **Doğrulama:** `KargoUcretiKuraliTests` 9/9 + `KargoKampanyaMotoruTests` 7/7 → paket **188/188** (37 atlanan DB testi); izole 5051 DEMO DB'de uçtan uca: 199,90 TL sepet → ShippingFee 59,99 / GrandTotal 259,89; 1.199,40 TL (kanal eşiği 1000) → 0 / 1.199,40; kampanya (500 TL üstü bedava) + 599,70 TL sepet → sepet ucu kampanya adını döndü, sipariş ShippingFee 0; demo test verisi (kampanya + kanal ayarı) geri alındı. `publish` eşitlendi (DLL e0f811bb), `admin/dist` derlendi, rehber (Satış Kanalları → Kargo Ücreti) + `docs/mobil-api-referansi.md` §4 güncellendi ⚠️ **restart (`sudo systemctl restart ecspros`)**. **Kanal ayarları BOŞ olduğu sürece davranış bugünküyle aynı: kargo ücretsiz** — ücret panelden girilince devreye girer. Bilinen sınırlar: ücret kanal başına sabittir (kargo firmasına/desiye göre değişmez); kargo kampanyası ürün kartlarında rozet olarak GÖSTERİLMEZ; sepette kupon istemci tarafında olduğundan ekran kuralın aynasını uygular (tahsilat her hâlükârda sunucuda yeniden hesaplanır); `LegacyOrderSyncService` için `CargoExpenseTypeId` ayarı girilmezse eski sisteme kargo ayrı kalem olarak DEĞİL yalnız toplamda gider.

**PANEL YETKİLENDİRME BRIEF'İ DEĞERLENDİRİLDİ (2026-09-09, Admin panel; `docs/panel-yetki-sistemi-promtu.md` → `docs/panel-yetki-sistemi-degerlendirme.md` v1; kullanıcı "incele ve değerlendir, uygulama yapma"):** Brief yön olarak doğru (default deny, union, allow/deny/inherit reddi, üç katmanlı zorlama, kaynak gösterme, field verisini backend'de kesme, silme yerine pasifleştirme). **İki düzeltme gerekiyor:** (1) §5 "kullanıcı özel istisnası" tanımı gereği bir DENY katmanıdır — adı konmalı, deny YALNIZ kullanıcı seviyesinde ve KAPSAM bazında olmalı, gruplar asla deny üretmemeli; (2) §14 "panelden permission tanımlama" hayalet yetki üretir (panelden açılan ama koda bağlı olmayan yetki yanlış güven verir) → katalog KOD SAHİPLİ olmalı, panel yalnız ad/açıklama/gruplama/aktif-pasif düzenlemeli. **Brief'te hiç geçmeyen asıl maliyet:** kanal kapsamı yalnız "yapabilir mi" değil "hangi satırları görür" demektir — liste/sayaç/dashboard/rapor/DataGrid export filtresi (~işin %70'i); ayrıca yetki tazeliği (permission'lar JWT'ye gömülü, 60 dk → kaldırma anında etkili değil), export'ta alan yetkisi, son super_admin koruması, oturum sonlandırma, kanalsız permission bayrağı, firma katmanı (`User.FirmId`), geçiş planı. **Ölçülen mevcut durum (kod + canlı DB):** 514 panel/iç endpoint'in yalnız **37'sinde** permission kontrolü (~%7) — kalanı düz `[Authorize]`; 11 permission, 3 rol, 49 kullanıcı ama **4 rol ataması (hepsi super_admin)** → 45 kullanıcı rolsüz, buna rağmen uçların %93'ünü çağırabiliyor; `iam_user_permissions` **boş** ve `GrantType='revoke'` satırları Login/Refresh'te işlenmiyor (§5 şemada var, davranışta yok); `UserPermission.FirmId` hiç okunmuyor, kanal kapsamı yok (5 aktif kanal / 2 firma); `iam_admin_menus` boş (ölü), menü `sidebarNavigation.ts`'te sabit ve 8 kalemde permission var; `RequirePermissionAttribute`'taki `"*"` dalı ÖLÜ (DB'de `*` permission'ı yok) — super_admin 11 permission'ı tek tek aldığı için geçiyor, dolayısıyla **yeni permission eklenince seed'e kadar super_admin de kilitlenir** (brief §12'nin "ayrı sistem bayrağı" beklentisi haklı); yetki değişikliği yazan audit kodu YOK. **Karar bekleyen K1-K8** dokümanda (kapsam birimi, liste filtresi, tazelik, kod sahipli katalog, super_admin bayrağı, field kapsamı, geçici yetki, 45 rolsüz kullanıcının geçişi) — K1/K2/K3 modelin şeklini değiştirdiğinden tasarım çıktısı bunlar netleşmeden üretilmemeli. Kod değişikliği YAPILMADI.

**KİŞİYE/GRUBA ÖZEL KUPON UYGULANDI (2026-09-09, Admin panel + Promotion; kullanıcı bildirimi "kullanıcı bazlı indirim kuponu tanımlayamıyoruz"):** Teşhis: `promotion.coupons` tablosunda `MemberId` ve `MemberGroupId` kolonları BAŞTAN vardı ve müşteri tarafı (Hesabım → İndirim Kuponlarım / sepetteki "Kuponlarım" — `GetMemberCouponsQuery`) bu alanlara göre listeliyordu; eksik olan **tanım** ucuydu — `CreateCouponCommand`/`UpdateCouponCommand`/`CouponRequest` bu iki alanı hiç taşımıyordu, dolayısıyla panelden (ve API'den) kişiye özel kupon tanımlanamıyordu. İkinci ve daha ciddi bulgu: `ValidateCoupon`'daki sahiplik kontrolü `if (request.MemberId.HasValue)` bloğunun İÇİNDEYDİ → **misafir sepetinde (MemberId null) kişiye özel kupon doğrulamadan geçiyordu**; `MemberGroupId` ise doğrulamada hiç kontrol edilmiyordu (yalnız listelemede kullanılıyordu). **Uygulama:** yeni `Promotion.Application.Services.KuponHedefKurali` (TEK kural: `Engel(kuponMemberId, kuponMemberGroupId, istekMemberId, istekMemberGroupId)` + `TanimHatasi` + `UyeGrubuGerekli`); `ValidateCouponQueryHandler` ve `UseCouponCommandHandler` bu kuralı uygular (grup gerektiğinde üyenin grubu `IMemberService` ile çözülür — modüller arası doğrudan referans yok, gereksiz CRM sorgusu yapılmaz); Create/Update komutları + `CouponRequest` + controller `memberId`/`memberGroupId` taşır (üye+grup birlikte seçilemez; seçilen üye `MemberExistsAsync` ile doğrulanır); `GetCouponsQuery` `memberId`/`memberGroupId` filtreleri + listede gösterilecek `memberName` (yalnız hedefli satırlar için CRM'den çözülür). **Panel:** Kuponlar penceresinde "Kimler Kullanabilir?" bölümü (Herkes / Belirli bir üye / Üye grubu) — üye araması ad-soyad-e-posta-telefon (≥2 karakter, `crm/members`), seçilen üye rozet + "değiştir"; grup seçimi `crm/member-groups`; listeye **KİM KULLANABİLİR** sütunu. Rehber `docs/rehber/content/08-pazarlama/20-kuponlar.md` güncellendi ve `/rehber` yeniden derlendi (kuponlar sayfasının ekran görüntüsü yeni sütunu içermiyor — sonraki görsel turunda yenilenecek). Doğrulama: `KuponHedefKuraliTests` 9/9 (misafir regresyonu dahil) + `KuponHandlerDiTests` (2026-09-08 DI dersi: handler'lar gerçek modül kayıtlarıyla, DB'ye bağlanmadan kurulur) → paket **172/172** (37 atlanan DB testi); admin `tsc`+`npm run build` temiz (`admin/dist` canlı); izole 5051 Production/Role=Api açılış temiz, kupon uçları kimliksiz 401. `publish` eşitlendi (DLL 2ea7a821) ⚠️ **restart (`sudo systemctl restart ecspros`)** — ⚠️ restart'a kadar panel yeni alanı GÖSTERİR ama eski binary `memberId`/`memberGroupId` alanlarını yok sayar, kupon "Herkes" olarak kaydedilir. Migration YOK (kolonlar mevcuttu) → staging/demo binary'leri etkilenmez. Bilinen sınır: toplu kişiye-özel kupon üretimi (bir gruba tek tek kod dağıtma) kapsam dışı; kupon kullanım listesinde üye adı hâlâ yok (yalnız sipariş no).

**VİTRİN TEK FİYAT SÖZLEŞMESİ + KAMPANYA ROZETLERİ UYGULANDI (2026-09-08, Mobil API + vitrin; mobil ekip isteği B9, kullanıcı kararlarıyla):** İnceleme sonucu: (1) favori/gezilen/koleksiyon satırı zaten liste sorgusundan besleniyordu ama `StoreKartZenginlestirici.Ozet` izdüşümü yalnız 7 alan kopyalıyordu → rozetler orada düşüyordu (ek sorgu maliyeti YOK); (2) "indirim iki biçimde" görünmesinin nedeni iki farklı biçim değil, ÜÇ indirim kaynağı: kanal fiyat indirimi (188.841/282.078 varyantta çizili fiyat), ürün-bazlı kampanya (percent/amount → CampaignPrice), sepet-bağımlı kampanya (2 al 1 öde/kargo bedava → yalnız rozet). Canlıda 3 aktif kampanyadan yalnız "%15 İndirim" fiyatı düşürüyor; ekibin "Süper Fırsat" örneğindeki 299,99/399,99 kampanyadan değil KANAL indiriminden geliyordu. **Kullanıcı kararları:** çizili fiyat salt gösterim (hesaba girmez) → kampanyada en yüksek referans gösterilir; geçiş additive (eski alanlar bir sürüm korunur); detaya da rozet bandı eklenir. **Uygulama:** yeni `Shared.Contracts.KartFiyatGorunumu.Hesapla(minPrice, kanalCizili, kampanyaFiyat) → (price, compareAtPrice)` TEK kural; `price` alanı liste (`StoreProductDto`), kanal kategori listesi (`ChannelCategoryProductItemDto`), detay (`StoreProductDetailDto`), üye özeti (`StoreUrunOzetDto`) ve favori/gezilen satırlarına eklendi; `compareAtPrice` artık kampanya öncesi fiyatı da kapsıyor; detay `ResolveAllForProductsAsync` ile `campaignBadges` döner; üye satırlarına `campaignName` + `campaignBadges`; `PageBlockSourceResolver` kart indirgemesinde `Price` korunur (alan düşürme tuzağı). **Web görünümü değişmedi:** `UrunKartMap` çizili satırı yalnız kanal indiriminde gösterir (`CompareAtPrice > satış fiyatı` guard'ı bugünkü değeri birebir üretir), Razor detayı zaten kendi hesabını yapıyor. Kategori cache'i **sürüm artırmadı**: fiyat görünümü cache DIŞI hesaplanıyor (her okumada yeniden), kural idempotent (test edildi). Doğrulama: `KartFiyatGorunumuTests` 8/8 (kaynak kombinasyonları, geçersiz değerler, idempotenslik, web guard'ı, üye satırı izdüşümü); paket yeşil; izole 5051 canlı veriyle: P-00021945 price 679,99 / compareAt 799,99 / 2 rozet, P-00020386 price 299,99 / compareAt 399,99 / 2 rozet, detayda rozetler ✓. publish/demo/staging eşitlendi ⚠️ **restart**. Kalan: `campaignPrice`/`minPrice` eski alanları mobil `price`'a geçtikten sonra kaldırılacak (ayrı sürüm); yorumlarım/sorularım satırları hâlâ yalnız ad+görsel taşır (ekip istemedi).

**PUSH — favorite_back_in_stock + cart_price_drop TAMAMLANDI (2026-09-08, Mobil API; push işinden kalan iki senaryo):** **favorite_back_in_stock** (P): `storefront.favorites`'e `WasOutOfStock` (migration `20260908222737_AddFavoriteStockState`, varsayılan false → hâlihazırda tükenmiş favori ilk taramada işaretlenir, bildirim gitmez); tarama favorilenen **RENGİN** varyantlarına bakar (yeni `VaryantRenkSql`: renk ekseni, yoksa `filtre_rengi`; renk yoksa/artık yoksa ürün geneli — canlıda 66 favorinin 37'sinde renk dolu), stok 0→pozitif geçişinde bildirir ve işareti sıfırlar; dedup gün başına 1 (`…:{Bugun}` — stok 0 çevresinde salınım tekrar üretmez); aynı ürün için **stock_alert** ("gelince haber ver") son 6 saatte gittiyse tekrarlanmaz (StokAlarmiAsync aynı taramada önce çalışır). `favorite_low_stock` bilinçli olarak ÜRÜN geneli kaldı (2026-09-07 davranışı değişmedi). **cart_price_drop** (P): `crm.crm_cart_items`'a `EffectivePriceAtAdd` (migration `20260908222742_AddCartItemEffectivePriceAtAdd`) — `AddedPrice` kampanya ÖNCESİ birim fiyat olduğundan taban olarak kullanılamazdı (her kampanyalı üründe yanlış "indirim" üretirdi); taban ilk taramada dolar (Favorite.PriceAtAdd kalıbı), karşılaştırma **kart fiyatıyla** (kanal/base min + etkin kampanya, `CampaignPricing.EffectivePrice`); eşik `Push:PriceDropPercent` (%10), pencere `Push:CartPriceDropDays`=14 gün, üye başına EN BÜYÜK düşüş / tarama başına 1, dedup `cart_price_drop:{cartId}:{variantId}:{yeniFiyat}`. Gönderim anı iptali: `cart_price_drop` için O KALEM hâlâ sepette olmalı (cart_reminder eskisi gibi "sepette ürün var mı"). Tarama adımı `("cart_price_drop", SepetFiyatDususuAsync)` eklendi (PushTarayici'ye `ICrmDbContext`). Seed'e 2 şablon (ikon favorite/cart, pazarlama 30 gün, TTL 6 sa) — **restart'ta oluşur, o ana kadar senaryolar sessizdir**. Testler: `PushScanDecisionTests` 4/4 (stok geçişi 5 hâl, fiyat eşiği tam-eşik/artış/sıfır taban, renk kırılımı yedeği, + gerçek şemada salt-okunur sorgu çalıştırma); paket **158/158**. Migration'lar canlı DB'ye uygulandı (eklemeli → eski binary etkilenmez). publish/demo/staging eşitlendi (DLL 4251b532) ⚠️ **restart (ecspros, ecspros-staging, ecspros-demo)**. Not: `docs/PUSH_BILDIRIM_ENTEGRASYONU.md` root'a ait (salt-okunur) — senaryo notu `docs/mobil-api-referansi.md` §13'e yazıldı; kalan tek eksik misafir sipariş bildirimi.

**BİLDİRİMLERİM (uygulama içi bildirim listesi) UYGULANDI (2026-09-08, Mobil API + Admin panel; `docs/BILDIRIMLERIM_BACKEND_ISTEGI.md`):** `storefront.push_notifications` + `push_templates`'e migration `20260908195826_AddNotificationInbox` (şablon: Inbox/Icon/ExpiresDays/DismissOnOpen; bildirim: DeviceId **nullable**, Inbox/Icon/DismissOnOpen/ExpiresAt/ReadAt/**DismissedAt** [belgedeki deleted_at; BaseEntity soft delete admin logunu gizlerdi]; kısmi indeksler; veri adımı: 21 şablona §5 ikonu + pazarlama 30 gün, mevcut bildirimlere ikon/süre/ReadAt=OpenedAt) — **canlı DB'ye uygulandı**; demo DB açılışta migrate eder. `PushKuyruk`: üyeye hedeflenen bildirim FCM'e gidemese de (cihaz yok / izin yok / günlük-haftalık sınır) `inbox=true` ise cihazsız `skipped` satır (`errorCode` no_device|no_consent|daily_limit|weekly_limit; platform: istekten, yoksa üyenin son cihazından); ikon/süre/dismiss şablondan. Yeni `Services/Push/BildirimKutusu` + `StoreNotificationInboxController` (`/api/store/account/notifications` GET liste [pageSize≤50, her yanıtta unreadCount, çok cihazlı üyede dedupId başına TEK satır], GET unread-count, POST {id}/read [dismissOnOpen → listeden düşer], POST read-all, DELETE {id}, DELETE tümü; idempotent; yabancı/yok 404; **cihaz/web token'ı ve kimliksiz 401** — MemberOnly politikası 403 verdiğinden AllowAnonymous + elle üye denetimi); eski `StoreAccountController.GetNotifications` kaldırıldı. `push-devices/opened` artık `ReadAt` (+dismiss) set eder. Panel Push Şablonları: Liste kolonu + düzenlemede "Listede göster / İkon / Listede kalma (gün) / Açılınca düşsün"; admin push-log satırına inbox/readAt/dismissedAt/expiresAt. Seed yeni şablonlara ikon + sınıfa göre süre verir. Test `NotificationInboxDbTests` (§7 prosedürü gerçek DB'de tek işlem + rollback: 3 satır/unread 3, sıra, ikon/görsel/sınıf, sayfalama+tavan, okundu idempotent, dismissOnOpen düşme, sil idempotent, yabancı id 404, tümü okundu/sil, süresi dolan görünmez, inbox=false listeye girmez) ✓; paket yeşil; docs/mobil-api-referansi.md §14. publish/demo/staging eşitlendi ⚠️ **restart (ecspros, ecspros-staging, ecspros-demo)** — restart'a kadar yeni uçlar 404, eski binary cihazsız satır üretmez. Bilinen: `favorite_back_in_stock`/`cart_price_drop` senaryoları hâlâ yok (push işinden kalan); belge root'a ait, uygulama notu §14'te.

**2026-09-08 CANLI ARIZA — ürün sorusu cevaplama / yorum onay-ret / iade onay-ret 500 (Admin panel + API):** journal 19:40 `No service for type 'ECSPros.Api.Services.Push.PushEtkilesim' has been registered` — 2026-09-07 push işinde eklenen `PushEtkilesim` (soru/yorum/iade push kancaları) 5 controller aksiyonunda `[FromServices]` ile isteniyor ama DI'a hiç kaydedilmemişti; yetki filtresi model bağlamadan önce çalıştığından kimliksiz duman testleri yakalamadı. Düzeltme: `AddScoped<PushEtkilesim>` (Program.cs push bloğu) + yeni `FromServicesRegistrationTests` (Api assembly'sindeki tüm controller aksiyonlarının `[FromServices]` somut tipleri için kaynakta gerçek `Add(Scoped|Singleton|Transient)<…Tip>` çağrısı arar; `ILogger<Tip>` gibi jenerik kullanımlar sayılmaz — kayıt silinince 5 aksiyonu listeleyerek kırmızıya düştüğü doğrulandı). publish/demo/staging eşitlendi ⚠️ **restart ecspros** (ve staging/demo) — restart'a kadar soru cevaplama, yorum onay/ret, iade onay/ret çalışmaz.

**DATAGRID — KOLON BAZLI SABİTLEME UYGULANDI (2026-09-08, Admin panel; kullanıcı kararı "kritik kolonları sabit tut anahtarı yerine kullanıcı istediği kolonu sabitlesin / kritik kolonu çıkarabilsin"):** `GridPrefs.frozenKeys` (tanımsız → kritik kolonlar varsayılan; tanımlıysa yalnız o liste), Kolonlar menüsünde satır başına raptiye (Pin/PinOff, `aria-pressed`, `data-pin=<key>`), eski "Kritik kolonları sabit tut" anahtarı kaldırıldı (eski `frozen:'off'` tercihi boş küme olarak okunmaya devam eder). Sabit kolonlar sola alınır (kendi sırası korunur; ▲/▼ grup içinde). Bütçe: varsayılan küme breakpoint adedi + %35 hedef (eskisi gibi); kullanıcı seçiminde adet sınırı yok, yalnız %40 mutlak sınır — sığmayan serbest kalır ve menüde "(sığmıyor)" görünür. Kaydedilmiş görünümler `frozenKeys`'i de taşır. Mobilde raptiye yok. Düzenek `grid-f8-check.mjs`; regresyon F1/F5/F6/F7. `admin/dist` canlı (yalnız frontend).

**2026-09-08 DEMO ARIZASI — telemania.ecspros.com ana sayfa `IDX10703 key length is zero` (ecspros-demo 5050):** Demo ortamında `Jwt:Secret` HİÇ tanımlı değildi (`appsettings.json` boş, `appsettings.Demo.json`'da Jwt bölümü yok); 7 Eyl 09:26'dan beri her ana sayfa isteği JwtBearer şemasını ilklendirip (Program.cs `AddJwtBearer` içindeki `SymmetricSecurityKey`) 500 dönüyordu (journal'da 80+ kayıt; staging/canlı Production.json'daki anahtarla sorunsuz). Düzeltme: `publish-demo/appsettings.Demo.json` + `src/ECSPros.Api/appsettings.Demo.json` (ikisi de untracked-sırlı) içine 64 karakterlik rastgele `Jwt:Secret` yazıldı (yedekler `*.bak-jwt-20260908`, gitignore'da; **bu anahtar demo'ya özgü, yedeğe dahil**); Program.cs artık boş/32 karakterden kısa anahtarda AÇILIŞTA ortam adını ve dosyayı söyleyen mesajla durur (per-istek 500 yerine). publish/demo/staging eşitlendi ⚠️ **restart ecspros-demo** (anahtar + hardening); ecspros/staging restart'ı önceki F6 işiyle birlikte bekliyor. Not: demo'daki mevcut üye/personel oturumları (varsa) yeni anahtarla geçersizleşir — demo için kabul edilebilir.

**2026-09-08 DÜZELTME — Ürünler "Geçersiz sıralama alanı: newest" (Admin panel + API):** ProductsPage adlandırılmış parametre olarak `sort=newest` gönderiyordu; `toParams` içinde extra parametreler grid'in `sort`'unu ezdiğinden HER istek `sort=newest` ile gidiyor, controller aynı query parametresini `GridRequest.Sort`'a da düşürdüğünden `ProductGrid.ApplySort` şema yoluna girip 400 fırlatıyordu (başlık sıralaması da hiç çalışmıyordu). Düzeltme: sayfa named sort göndermez (grid varsayılanı `createdAt desc` = eski newest; f4 düzeneğinde istek `sort=createdAt&dir=desc` doğrulandı); sunucu `ProductGrid.ApplySort` beyaz listede olmayan sort değerini (eski istemciler) legacy kurala düşürür, 400 üretmez. Ekip arkadaşının kaynak-metin testi `AdminProductListOrderingTests` yeni mekanizmaya uyarlandı (10/10). `admin/dist` canlı → sayfa HEMEN düzeldi; API tarafı publish/demo/staging eşitlendi (DLL 4771fe62) ⚠️ restart (eski istemci uyumluluğu için).

**DATAGRID — STICKY BAŞLIK + STICKY SAYFALAMA UYGULANDI (2026-09-08, Admin panel; kullanıcı isteği "header ve page navigasyonu da sticky olsun, yatay scroll gibi davransın"):** thead yatay kaydırma kabının (overflow) içinde olduğundan window'a göre sticky yapılamaz → ghost scrollbar kalıbıyla `grid/stickyChrome.ts` (`useStickyChrome`): gerçek kolon başlığı uygulama başlığının (`[data-top-bar]`, Header.tsx) altına girince ve tablo gövdesi hâlâ görünürken body'ye portal edilen **sabit başlık kopyası** (`.grid-ghost-header`; aynı `headerRow()` JSX → sıralama tıklaması ve başlık filtreleri çalışır; `<colgroup>` genişlikleri gerçek th'lerden; gerçek kaydırmayla scrollLeft eşlenir; frozen kolon kopyada da sticky; üzerinde yatay tekerlek gerçek kaba iletilir); gerçek sayfalama viewport altında kalınca **sabit sayfalama kopyası** (`.grid-ghost-pagination`, `data-bottom-bar` → ghost scrollbar onun üstüne çıkar; `grid-chrome-change` olayı ile offset yenilenir). Mobil kompakt görünümde başlık kopyası yok, sayfalama var. Düzenek `grid-f7-check.mjs` **35/35** (tablet 900/masaüstü/mobil: başlık uygulama başlığının hemen altında, sol/genişlik hizası, kolon metin+genişlik eşitliği, ghost scrollbar sayfalamanın üstünde, yatay eşleme, sticky frozen, kopyadan sıralama+sayfa 2, en altta/en üstte kaybolma, mobil taşma yok); regresyon F1 29/29, F2 37/37, F4 210/210, F5 21/21, F6 17/17. Yalnız frontend, `admin/dist` canlı (restart gerekmez).

**DATAGRID — TÜM SÜTUNLARDA FİLTRE UYGULANDI (2026-09-08, Admin panel + API; 4 paralel fork, kullanıcı kararı "tüm sütunlarda filtre olsun"):** 17 DataGrid sayfasında başlığı olan HER kolonda başlık filtresi (aksiyon/detay kolonları hariç) — yeni düzenek `grid-f6-check.mjs` **17/17**. (A) Grid destekli 8 sayfada boş kolonlar dolduruldu: Siparişler DURUM; Ürünler ÜRÜN→ad (`GridJson.Text(NameI18n,"tr")`) + VARYANT sayısı; Üyeler DURUM/AD/SOYAD; Talepler Tür(+konu)/Kontrol (`readByMe` — TicketReads üzerinden handler'da özel)/Durum; Stoklar ÜRÜN (`f.product` → IProductService varyant kümesi, facets+export aynı)/DEPO/KISIM/RAF (guid enum, named seçicilerle AND); Faturalar TİP/DURUM/ENTEGRATÖR; İadeler TİP/DURUM/KARGO İADE KODU (+ GERİ ÖDEME text→enum 400 düzeltmesi); Kampanyalar AD (name jsonb filtre+sıralama+arama)/TİP/DURUM. (B) 9 sayfa backend grid'e alındı (sunucu filtre/sıralama/global arama/Excel export + views): **Kullanıcılar** (`UserGrid`, `role` çoklu rol kodu handler'da; `POST iam/users/export`), **Denetim logları** (`AuditLogGrid`, KULLANICI kolonu; `POST iam/audit-logs/export`), **Teklifler** (`QuoteGrid`; `POST orders/quotes/export`), **Hediye kartları** (`GiftCardGrid`; `POST orders/gift-cards/export`), **Tedarikçi faturaları** (`SupplierInvoiceGrid`; `POST finance/supplier-invoices/export`), **Toplama planları** (`PickingPlanGrid`; orderCount/progress/assignment CASE ifadeleri; PickingPlan.Lines navigasyonu, migration YOK; `POST fulfillment/picking-plans/export`), **POS satışları** (`PosSaleGrid`, KASA kolonu; `POST pos/sales/export`), **Entegrasyon logları** (`IntegrationLogGrid`; sekme `error`→`failure` DB değeri düzeltildi; `POST integrations/logs/export`), **Tracking outbox** (`TrackingOutboxGrid`, hedefler yalnız bool; `POST tracking/outbox/export`). Çekirdek: `GridSchema.Date` artık **DateOnly/DateOnly?** kolonları da alır (gün karşılaştırması, between iki uç dahil; `GridSchemaTests` DateOnly testi). `GridSchemasDbTests` 17 şema (9 yeni) gerçek DB'de SQL'e çevrildi ✓; paket 151/151; düzenekler F1 29/29, F2 37/37, F3 11/11, F4 **210/210** (14 sayfa artık tam grid), F5 21/21, F6 17/17; tsc+eslint temiz; `admin/dist` canlı. İzole 5051: açılış temiz, 18 yeni uç kimliksiz 401 (export 5. istekten sonra 429). publish/demo/staging eşitlendi (DLL 81c1dbdb) ⚠️ **restart (ecspros, ecspros-staging, ecspros-demo)** — restart'a kadar 9 sayfada filtre/sıralama yok sayılır, export 404. Bilinen: Kullanıcılar'da `lastLogin→lastLoginAt`, `status→isActive` kolon anahtarı değişti (kayıtlı kolon tercihi varsayılana döner); Tracking durum sekmesi hâlâ useState; PagesController'daki ayrı CMS audit-logs ucu kapsam dışı; gerçek veri testi kullanıcıda.
...
**DATAGRID STANDARDI F5 — kaydedilmiş görünümler + mobil kompakt + satır seçimi UYGULANDI (2026-09-08; plan kapsamı TAMAM):** backend `GET/PUT /api/iam/users/me/preferences` (`Iam.Application/Queries/GetMyPreferences/MyPreferences.cs`; `iam.Users.Preferences` jsonb — ölü sütun canlandı; anahtar ≤80 karakter, değer ≤64 KB, yalnız kendi kaydı, PUT value null → anahtar silinir). Panel: `useGridViews` (anahtar `grids.<gridId>` → `{views[], defaultViewId}`; görünüm = URL parametreleri (page hariç) + kolon tercihleri; kaydet/güncelle/sil/varsayılan; aktif görünüm URL eşleşmesiyle; **varsayılan görünüm yalnız TEMİZ girişte bir kez** (URL'de grid parametresi varsa ezmez), filtreler çipte görünür), `ViewsMenu` ("Görünüm ▾"), DataGrid `views`/`compact`/`selection` prop'ları — **mobil kompakt görünüm** (compact tanımı olan grid'de mobil varsayılan; başlık/alt başlık/sağ değer/rozet, dokununca görünür kolonlar açılır + "Detay →", Kompakt/Tablo geçişi tercihi localStorage `mobileView`), **satır seçimi standardı** (onay kutusu kolonu, sayfada tümünü seç, "n seçili" çubuğu + sayfaya ait aksiyon slotu — henüz hiçbir sayfa bağlamadı, toplu aksiyon çıkınca kullanılacak). Siparişler'de `views` + `compact` açık; diğer 7 sayfada `views` açık. Düzenek `grid-f5-check.mjs` **21/21**; regresyon F1 29/29, F2 33/33 (mobil kompakt varsayılana uyarlandı), F3 11/11, F4 105/105; backend paket 151/151. publish/demo/staging eşitlendi ⚠️ **restart** (tercih uçları). Bilinen: görünüm paylaşımı yok (K7); kompakt görünüm yalnız Siparişler'de tanımlı (diğer sayfalar `compact` prop'u verince açılır); seçim kolonu frozen değil.

**DATAGRID STANDARDI F4 — yaygınlaştırma UYGULANDI (2026-09-08, Admin panel + API; 3 paralel fork):** ortak `Api/Grid/GridExportEndpoint` (tavan→MiniExcel→audit→File; named yardımcıları) + `Shared.Kernel.Grid.GridExportSource<T>`; `GridSchema.FieldTypeOf/AllowedValuesOf`. 7 entity: **Ürünler** (`ProductGrid`; code/supplierProductCode/slug/sourceType/isSaleOpen/basePrice/taxRate/createdAt/productGroupId/supplierId; sort code/createdAt/basePrice/isSaleOpen/sourceType; `POST catalog/products/export` 13 kolon; eski `sort=newest` korunur), **Üyeler** (`MemberGrid`; ad/soyad/e-posta/telefon/şirket/vergi no, gender[none], isActive/isRegistered/isEmailVerified/isPhoneVerified, createdAt/lastLoginAt, memberGroupId/cityId, legacyMemberId; `POST crm/members/export` 15 kolon), **Talepler** (`TicketGrid`; trackingNo/type/status/customer/telefonlar/orderNumber/createdByName/createdAt/lastActivityAt/activityCount/hidden + guid'ler; sayaçlar durum hariç aynı filtrelerle; eski `sort` sözlüğü uyumlu; `POST crm/tickets/export` 16 kolon), **Stoklar** (`Api/Grid/StockGrid`; quantity/reserved/available/inStock/stockType/createdAt/updatedAt + depo/kısım/raf/varyant; facets aynı filtreleri alır; `POST inventory/stocks/admin-list/export` 12 kolon, ürün adı/barkod IProductService ile zenginleştirilir), **Faturalar** (`InvoiceGrid`; invoiceNumber/recipient/taxNumber/externalDocumentId/status/invoiceType/numberSource/integratorStatus/invoiceDate/createdAt/total/hasPdf/orderId; `POST orders/invoices/export` 20 kolon), **İadeler** (`ReturnGrid`; returnNumber/trackingNumber/cargoReturnCode/status/returnType/refundMethod/refundStatus/createdAt/cargoReceivedAt/refundAmount/orderId/memberId; `POST orders/returns/export` 15 kolon), **Kampanyalar** (`CampaignGrid` + yeni sayfalı `GetCampaignsGridQuery` — `page` parametresi varsa sayfalı, yoksa eski dizi; `POST promotion/campaigns/export` 13 kolon). Panel: 7 sayfa DataGrid'e taşındı (sekmeler `?tab=`/named, RowActions, export, hata satırı). Testler: `GridSchemasDbTests` — 8 şemanın TÜM filtre/sıralama alanları gerçek DB'de SQL'e çevrildi ✓; paket 151/151 (ekip arkadaşının `AdminProductListOrderingTests` ürün newest sıralaması ProductGrid'e taşındığı için yeni yere uyarlandı, davranış aynı); düzenekler F1 29/29, F2 32/32, F3 11/11, **F4 `grid-f4-check.mjs` 105/105** (7 sayfa × masaüstü/mobil). Bilinen: kampanya adı (jsonb) aramada yok; Talepler satır sol kenar rengi kaldırıldı (grid `rowStyle` yok); stok export'ta parti zenginleştirme senkron bekler. publish/demo/staging eşitlendi ⚠️ **restart (ecspros, ecspros-staging, ecspros-demo)** — restart'a kadar 7 sayfada filtre/sıralama parametreleri eski uçlarca yok sayılır, export 404. **SIRADA F5** (kaydedilmiş görünümler, mobil kompakt görünüm, satır seçimi) — kullanıcı kararıyla.

**DATAGRID STANDARDI F3 — Excel export düğmesi UYGULANDI (2026-09-08, Admin panel):** `grid/ExportButton.tsx` ("Excel'e aktar ▾": *Yalnız görünür kolonlar* = o anki görünür+exportable anahtarlar, *Tüm kolonlar* = `columns: []` → sunucunun tüm export kolonları; gövde search/sort/dir/filters + `named` (sekme statuses), sayfalama YOK; `responseType: blob` + Content-Disposition dosya adı + createObjectURL indirme; 400 tavan / 429 limit JSON gövdesi blob'dan okunup düğme altında `role=alert`), DataGrid `export` prop'u (Kolonlar'ın solunda), Siparişler `POST /orders/export` bağlandı. Düzenek `tools/tests/datagrid/grid-f3-check.mjs` **11/11** (indirme + dosya adı, gövde eşitliği, görünür kolon kümesi ÖDEME/detay hariç, named.statuses, page yok, tüm kolonlar boş liste, 429/400 mesajları, düğme yeniden etkin); F1 29/29 + F2 32/32 regresyon temiz. `admin/dist` canlı. **PİLOT KAPANIŞI (E7): kullanıcı Siparişler'de gerçek veriyle uçtan uca test edecek** (backend F0 restart'ı yapılmış olmalı); onaydan sonra **F4 yaygınlaştırma** (Ürünler, Üyeler, Talepler, Stoklar, Faturalar, İadeler, Kampanyalar).

**DATAGRID STANDARDI F2 — FilterBar + Siparişler pilotu UYGULANDI (2026-09-08, Admin panel):** `grid/FilterBar.tsx` (global arama 400 ms debounce + Enter anında + temizle; hızlı filtreler satırda, kalanı "Gelişmiş ▾ (n)" panelinde; tipli girişler: text op+metin (debounce), enum tek/çoklu (checkbox listbox), tarih hızlı seçim Bugün/Dün/Son 7/Son 30/Bu ay/Geçen ay/Özel aralık (`f.x=between:gün,gün` + `fq.x` etiketi), sayı op/aralık (`min;max`), boolean Tümü/Evet/Hayır; aktif çipler tek tek × + "Tümünü temizle"; mobilde "Filtreler (n)" rozeti + bottom sheet, çipler kapalıyken görünür), `filterUtils.ts` (hızlı tarih aralıkları, çip metinleri), `useGridState.mutate` (toplu URL güncellemesi — sekme + tarih tek adımda), DataGrid `search`/`extraFilters`/`error` prop'ları (400 → kırmızı satır, çipler kalır). **Siparişler sayfası tam göç:** durum sekmeleri `?tab=` (aktif = parametresiz; Teslim/İptal/Tümü'ne geçişte tarih yoksa Son 30 gün çipi), sayaçlar listeyle aynı `search/f.*` parametreleriyle (durum hariç), sıralama başlıkları (createdAt vars. azalan, orderNumber/total/status/customer/paymentStatus), frozen Sipariş No + Müşteri, hızlı: Tarih + Ödemesi alınan; gelişmiş: Müşteri, Tutar, Ödeme durumu (çoklu), Ödeme yöntemi (çoklu, none), Telefon, Dış sipariş no, Kargo; global arama sipariş no/dış no/alıcı/telefon. Doğrulama: tsc+eslint temiz; `tools/tests/datagrid/grid-f2-check.mjs` **32/32** (debounce zamanlaması, sayaç ucu aynı filtreler, çoklu enum/sayı/tarih URL kodlaması, çip kaldırma, sıralama asc/desc, Tümünü temizle sort+tab korur, heavy sekme tek URL güncellemesi, geri tuşu, 400 çökmez, mobil rozet/sheet/çipler/p1 kolonlar/taşma yok). `admin/dist` canlı. Görsel düzeltmeler: `.inp` utility'leri `!` ile ezildi (arama ikonu, çoklu seçim). **SIRADA F3** ExportButton (tüm/görünür kolon) + Siparişler export uçtan uca (backend F0 hazır) → pilot kapanışı.

**DATAGRID STANDARDI F1 — DataGrid çekirdeği UYGULANDI (2026-09-08, Admin panel):** `admin/src/components/grid/` — `types.ts` (GridColumn: key/sortable/filter/priority/frozen/lockVisible/exportable/stopRowClick; GridState; GridPrefs), `useGridState` (URL: page/search/sort/dir/f.*/fq.* → geri-ileri + paylaşılabilir link; localStorage `ecspros-grid:<gridId>:<userId>`: order/manualVisible/manualHidden/pageSize/frozen; toParams/queryKey/toggleSort/setFilter/clearFilters), `useBreakpoint` (matchMedia 768/1024), `GridScrollRegistry`+`gridScrollContext` (tek ghost yatay scrollbar: en görünür/son etkileşimli grid, IntersectionObserver gövde+alt sentinel, rAF+kaynak bayrağıyla çift yönlü senkron, `[data-bottom-bar]` alt offset; mobilde kapalı), `DataGrid` (priority varsayılanı + manuel tercih ezilmez [E5], frozen bütçesi %35 hedef/%40 sınır + breakpoint adedi {2,1,0} + kullanıcı kapatabilir, kenar gölgeleri data-scroll start/middle/end, sıralama başlıkları, satır tıklama → detay klavye dahil + `RowActions`/`stopRowClick`, ince yenileme çubuğu, tablo min-width), `ColumnsMenu` (göster/gizle, ▲/▼, kilitli kolon, sabit aç/kapa, varsayılana dön), `GridPagination` (25/50/100/250 + x–y/toplam (filtreli)); CSS `.grid-*` index.css; `MainLayout` GridScrollProvider; `clearSessionStoragePreservingFavorites` grid tercihlerini korur. **Smoke-test Kullanıcılar sayfası** DataGrid'e taşındı (arama/sayfa URL'de; frozen kullanıcı adı; telefon/departman/ünvan kolonları). Doğrulama: tsc+eslint temiz, headless Chromium düzeneği `tools/tests/datagrid/grid-f1-check.mjs` (API sahte, kimlik gerekmez) **29/29** (ghost senkron iki yön, tek ghost, alt scrollbar görünürken ghost yok, frozen sticky, Kolonlar gizle/taşı/kilit, tablet p3 gizli, mobilde manuel açılan kolon korunur + frozen/ghost yok + gövde taşmaz, geri tuşu). `admin/dist` derlendi (canlı). **SIRADA F2** FilterBar + Siparişler tam göç (backend F0 hazır).

**DATAGRID STANDARDI F0 — backend temeli UYGULANDI (2026-09-08, Admin panel; plan `docs/datagrid-standardi-plani.md` v2 onaylı):** `Shared.Kernel.Grid` (`GridRequest`/`GridFilter`/`GridExportRequest`/`GridException`→400, `GridSchema<T>` beyaz listeli text|enum|date|number|bool|guid filtre + sort/tie-breaker ifade motoru; gün tarihleri İstanbul, between bitiş günü DAHİL, ISO exclusive; sayı aralığı `min;max`), `Api/Grid` (`GridRequestParser` `page,pageSize,search,sort,dir,f.*` + merkezi clamp 1..250; `GridExportWriter` MiniExcel 1.46 geçici dosya akışı + kolon seçimi/kilitli + `iam.audit_logs` grid_export izi; `OrderExportColumns` 18 kolon). **Siparişler pilotu:** `OrderGrid` (şema: orderNumber, externalOrderNumber, customer, phone, cargo, status, paymentStatus, paymentMethod[none], orderType, createdAt, total, paid, firmPlatformId, memberId; sort createdAt/orderNumber/total/status/customer/paymentStatus; global arama + dış sipariş no + telefon), `GetOrdersQuery.Grid`, `status-counts` listeyle aynı filtreleri alır (durum hariç), `POST /api/orders/export` (gövde search/sort/dir/filters/columns/named; tavan `Grid:ExportMaxRows` 100K → 400; `grid-export` RateLimiter KULLANICI bazlı `Grid:ExportPerMinute` 5/dk → 429 özel mesaj). Testler: `GridSchemaTests`+`GridExportWriterTests`+`OrderGridDbTests` (ECSPROS_TEST_DB ile salt-okunur SQL çevirisi) 12/12; paket 149/149. İzole 5051: açılış temiz, kimliksiz 401, 5. export'tan sonra 429. Mevcut istemciler değişmedi (eski parametreler korunur). publish/demo/staging eşitlendi ⚠️ restart. **SIRADA F1** DataGrid çekirdeği (panel).

**2026-09-08 push: bakiye yüklemesi anında kuyruğa:** panel `POST /crm/members/{id}/wallet/adjust` (credit) başarıdan sonra `wallet_credit` push'unu doğrudan kuyruğa alır (dedup tarayıcıyla aynı `wallet_credit:{txId}` → 15 dk tarama ikinci satır açmaz; push hatası bakiye işlemini bozmaz). Kullanıcı "3 dk oldu gelmedi" → sebep tarama aralığıydı. publish/demo/staging eşitlendi ⚠️ restart.

**2026-09-08 push: üye başına tek (en güncel) cihaz:** aynı telefon her kurulumda yeni `deviceId` ürettiğinden 3 satır açılmış, eski token'lar FCM'de bir süre geçerli kalıp bildirim tekrarlanmış, sonra UNREGISTERED→revoked. Kuyruk artık üye hedeflemesinde platform başına yalnız EN SON GÖRÜLEN aktif cihaza gönderir (`Push:LatestDevicePerPlatform`, varsayılan true; doğrudan DeviceId ile deneme etkilenmez); `POST /store/push-devices/opened` bildirimi açan cihazın `LastSeenAt`'ini tazeler; mobil sözleşmeye "her açılışta upsert" maddesi eklendi (§10). publish/demo/staging eşitlendi ⚠️ restart. Mobil ekipten yine de sabit deviceId beklenir.
...
colorValueId/sizeValueId` (VariantDisplayInfo genişledi, GetOrderDetail IProductService ile zenginleşir);
**A3** `renk` ekseni isColor:true + filtre_rengi'siz üründe liste `colors[]` renk ekseninden; **A4**
`CartConvertedToOrderEvent` (Order.Domain) → Crm handler sepet kalemlerini siler (kapıda checkout'ta, kartta
PayTR/mock ödeme onayında); **A5** `products/{slug}`, `products/by-slug`, `channel-categories/by-slug/{slug}`,
detayda slug/variantSlugs/selectedColorValueId; **A6** `X-Firm-Platform` başlığı → `FirmPlatformHeaderFilter`
(global IActionFilter; query'de değer yokken parametre ActionArguments'a girmediği için ActionDescriptor
parametrelerinden dolduruluyor — ilk sürümdeki hata izole testte yakalandı); **A7** boş sepet nesnesi; **A8**
arama listesinde stoksuz renk elenir + ana görsel stoklu renge kayar; **A9** `GET /api/store/payment-options`;
**A10** detayda minPrice/compareAtPrice/campaignPrice/campaignName/productGroupId; **A11** reviewable zengin
liste + `GetMemberDeliveredItemsQuery` (eski N+1 sipariş detayı kaldırıldı); **B1** favorites/viewed/reviews/
questions/collections `StoreKartZenginlestirici` ile zengin (`light=true` eski şekil); **B2** `POST
/viewed-products` (üye); **B3** `DELETE /stock-alerts/{alertId}` + `?variantId=` (CancelStockAlertCommand);
**B4** web adres formu mahalle seçici (kimlik yazar) + seed `SeedAddressNeighborhoodBackfillAsync` (56 adres);
**B5** iki kart DTO'suna takma ad özellikleri (`id/productId`, `minPrice/basePrice`); **B6** paytr/init
`data.html`; **B7** `StoreSiteRoutes` (Cms.Application) kod↔site slug/alias, legal ucunda `slug/pageType/
aliases`, `pages/by-slug/{slug}`, seed kurumsal SlugI18n = site URL; **B8** `search` yerleşimi + doküman
`homepage`; **C2** `GET/POST /api/store/orders/track` (sipariş no + TELEFON — siparişte e-posta yok;
store-sensitive limit, tek tip 404); **C3** sadakat hesabı yoksa sıfır hesap 200. ⏳ **Açık:** A2 renk adı
normalizasyonu (renk havuzu 5.690 serbest değer, "Siyahbeyaz" gibi bitişik yazımlar — katalog veri kararı),
B2 misafir (device) gezinme kaydı (cihaz kimliği kolonu gerekir), C4 Play Integrity (kod dışı), SSS içeriği için
store FAQ ucu yok (faq section'ları legal ucunda dönmez). Doğrulama: izole 5051 demo DB'de anonim uçlar
(gate kapalı), izole 5051 canlı DB'de (seed kapalı) üye uçları mobil.test üyesiyle — hepsi ✓; testler 120/120.
`publish`/`publish-demo`/`publish-staging` eşitlendi ⚠️ restart kullanıcıda (`ecspros`, `ecspros-staging`
mobil ekip 5055'i kullanır, `ecspros-demo`). Not: `docs/BACKEND_ISTEKLERI.md` root sahipli/untracked (mobil
ekibin dosyası) — commit edilmedi.
...
`appsettingsTest.json` korunup GitHub kapsamına alınmadı. `origin/main` üzerindeki `c01a8832` commit'ine
kadar 21 yeni commit merge edildi. Tek çatışma `SeedErpSourceAttributeValuesAsync` içindeki iki eşdeğer
düzeltmeydi: sözlük projeksiyonu ve `TryGetValue` kullanan yerel sürüm korunurken uzaktaki yeni seed
adımları da alındı. Diğer yerel değişiklikler korundu; `appsettings.json` iki tarafın seçenekleriyle birleşti.
...
**FAZ 15.4 e/f UYGULANDI, c KURGU (2026-09-10 gece, Admin panel) ⚠️ restart bekliyor (ecspros + staging + demo), admin build alındı:**
15.4f Koli Sorgu `/fulfillment/box-lookup` (GetBoxOrdersQuery Fulfillment + sipariş durumu/müşteri/fatura tarihi Order'dan API'de birleştirilir;
sipariş no → kolinin tamamı). 15.4e Personele Dağıt `/fulfillment/assignments` (GetPickingAssignmentSummary + AutoAssignPickingLines →
mevcut AssignPickingLines'ı sarar). Menü: Sipariş Operasyonu › Personele Dağıt, Koli Sorgu; rehber 71 + 77. 15.4c Site Bileşen:
`docs/site-bilesen-yonetimi-kurgu.md` K1-K4 — misharix'te popup/üst bar tasarımı yok, tasarım gelmeden site tarafı yazılmaz.

**FAZ 15.4 — KISMİ KARŞILIKLAR: g/h/k UYGULANDI (2026-09-10, Admin panel) ⚠️ restart bekliyor (ecspros + staging + demo), admin build alındı:**
15.4g Müşteriye Ödemeler → İadeler'de **Ödenecek** sekmesi (received + geri ödeme pending), `Return.RefundIban/RefundAccountHolder`
(migration `AddReturnRefundBank` canlı+demo), `PUT returns/{id}/refund-bank`, Geri Ödeme Yap havalede IBAN zorunlu (ödeme kaydı
Details'e de yazılır). 15.4h Sipariş Üye Notları → `/orders` NOTLAR kolonu (varsayılan gizli) + "Notu olan" hızlı filtresi + müşteri/iç
not metin filtreleri (`GridJson.TextObj` jsonb DbFunction; ★ `Dictionary<string,object>` parametresi OrderDbContext'te `HasParameter("dict")
.HasStoreType("jsonb")` ile kayıtlı — bunsuz model kurulmuyor; DB testi OrderGridNotesDbTests geçti) + Excel kolonları. 15.4k Ürün
Yorum Ekle → Yorum Moderasyonu "+ Yorum Ekle" (`POST /api/reviews`, `CreateManualProductReviewCommand`, üye isteğe bağlı, onaylı yayın).
15.4d Sipariş Ayır: eski kodda karşılığı yok (`/ortak/data` datakey=siparis-ayir — DB tanımlı) → 15.5'e. Kalan 15.4 kalemleri
(a stüdyo, b kombin, c site bileşeni, e toplama dağıtımı, f set koli, i kargo tarifesi, j script) karar/kurgu bekliyor — rapora yazıldı.

**FAZ 15.3 — RAF/GÖZ OPERASYON EKRANLARI UYGULANDI (2026-09-10, Admin panel) ⚠️ restart bekliyor (ecspros + staging + demo), admin build alındı:**
Kurgu `docs/raf-operasyon-ekranlari-plani.md` (K1-K6 öneriyle onaylandı). Backend `Inventory.Application/Shelf/*`: göz içeriği,
ürün gözleri, göze yerleştir (serbest/rafsız), göz→göz taşı (adetli=serbest adet, tümü=rezervasyonla), iadeden rafa (kapalı kısım
→ açık), mağaza reyon (depo→depo anlık + tamamlanmış transfer kaydı), raf sayımı (start/scan/finish/apply/cancel, `inv_bin_counts`
+ `inv_bin_count_lines` migration canlı+demo), `StokHareketTipi`, yetki `inventory.count.apply` (katalog, firm_admin listesinde).
API `api/inventory/shelf/*`, `bin-counts*`, `stock-authority`. ★ `StockAuthority`: canlı `Legacy:Sync:Stock` açıkken yazma uçları
409 (aynalama kipi) — go-live'da stok senkronu kapanınca panel otorite. Panel `/inventory/shelf` (6 kip, tek okutma kutusu,
`shelf/resolve` göz mü ürün mü) + `/inventory/bin-counts` DataGrid; menü Stok › Raf İşlemleri. Rehber `07-stok/40-raf-islemleri.md`.
Testler: StockAuthorityTests, DataGridKeyConsistency (bin-counts eşlemesi). Bilinen sınır: koli kaynağı Tedarik › Sayım/Teslim akışında.

**FAZ 15.2 — PANEL KÜÇÜK HAZIR İŞLER UYGULANDI (2026-09-10, Admin panel) ⚠️ restart bekliyor (ecspros + staging + demo), admin build alındı:**
15.2a Katalog Ayarları › **Mankenler** sekmesi (kadro CRUD; `/api/catalog/mannequins`) + Ürün detayı › Özellikler'de `manken`
(json) özelliği için kadro seçici (görünüm + ölçü snapshot'ı, `CustomValue {"mankenler":[…]}`, spec). 15.2b sipariş detayı ve
Faturalar penceresinde **Yazdır** (`/yazdir/fatura/{id}`). 15.2c `/orders` KARGO kolonu + "Ürün kodu / adı (kalem)" filtresi
(`OrderGrid.ApplyProductFilter`, şema dışı koleksiyon filtresi → ApplyFilters'ta "product" atlanır; DataGridKeyConsistencyTests
geçti). 15.2d 5 ölü sayfa dosyası + 2 ölü request kaydı silindi. Rehber: katalog-ayarları, ürün-detay, siparişler, sipariş-detay,
faturalar güncellendi. ★ 15.1a 301 yönlendirme go-live ŞARTI DEĞİL (eski ürün/kategori URL'leri FAZ 18 + menü aktarımıyla
çalışıyor) — küçük/düşük öncelik. Sırada FAZ 15.3 raf ekran ailesi (önce ekran kurgusu, K16).

**İade akışı planı v1.1 (2026-09-10, Admin panel + Sipariş modülü) — F0-F5 UYGULANDI (K1-K8 plandaki ÖNERİ değerleriyle) ⚠️ restart bekliyor (ecspros + ecspros-staging + ecspros-demo):**
...
pazaryerinde ASLA. Kod tespitleri E1-E9 (returned'a geçiren komut yok, CompleteRefund tahsilat/pazaryeri
kontrolsüz, kapıda ödeme teslimde paid olmuyor, ReturnType serbest metin). Hedef: `IadeOdemeKurali` TEK kural,
`MarkUndeliveredReturnCommand`, `not_applicable` geri ödeme durumu, `closed` iade durumu; fazlar F0-F5.
★ Legacy senkron/aktarım eşlemeleri KAPSAM DIŞI — plan bitince ayrı düzeltilecek (kullanıcı kararı).

**Mobil push cihaz kaydı (2026-09-05, Mobil API) — UYGULANDI ⚠️ restart bekliyor:**
Mobil ekip talebi: bildirimler için üyeyle birlikte cihaz push kimliği (FCM/APNs token) tutulması.
`storefront.push_devices` (migration `AddPushDevices` CANLI DB'ye uygulandı; kanal+token unique, üye ve
deviceId indeksli). Uçlar: `POST /api/store/push-devices` (anonim/üye — upsert; girişli istek üyeye bağlar,
girişsiz koparır; aynı deviceId yeni token'la gelince eski token'lar revoked), `POST .../revoke` (idempotent),
`GET .../mine` (üye, token maskeli); admin izleme `GET /api/store-notifications/push-devices` (tam token,
memberId filtreli). Panel (K16): üye detayına "Mobil Bildirim Cihazları" bölümü. Doküman: mobil referans §10
(istemci sözleşmesi: token değişiminde + giriş/çıkış sonrası POST). İzole 5051 ✓: kayıt/upsert/rotasyon-revoke/
idempotent iptal + 2 negatif (platform, kısa token); test satırları silindi. Vitrin kapısı prod'da AÇIK olduğundan
uç, cihaz token'lı (attestation) çağrı bekler. Bildirim GÖNDERİMİ (FCM entegrasyonu) ayrı iş — bu tablo adres defteri.
NOT (aynı deploy): ekip arkadaşının bekleyen 4 migration'ı (Crm legacy adres kimliği, Integration 2 checkpoint,
Order legacy kimlikleri — Up'ları salt eklemeli doğrulandı) DB'ye uygulandı; publish merge'lü kodu içeriyor.
...
giriş modalı; Hesabım → Sorularım (/sorularim, yan menüde fa-comment-dots — circle-question alt kümede yok).
Panel: Ürün Soruları sayfası (/storefront/questions; Cevapla-ve-Yayınla, cevabı güncelle, yayından kaldır/al,
...
aramalar STATİK chip, popüler ürünler generic products çağrısı (sort'suz) — kullanıcı kararı: "product servis
yeterli, bu haliyle kalsın" (mobil için sort=sales_desc önerisi iletildi).

**Renk filtresi kartı boyar (2026-08-31, Web sitesi) ✅ CANLIDA (2026-09-01 restart; kullanıcı: kart+galeri+mobil menü testleri olumlu):** /urunler'de filtre_rengi/renk
...
**A2** `Node:Id/Role` — 10 worker yalnız Worker/Both rolde kayıt olur (DashboardMetricsWorker BİLİNÇLİ her düğümde:
düğüm-yerel hub yayını), Serilog NodeId etiketi, `/health/detail` (nodeId+rol+worker listesi);
**A7** `Node:MigrateOnStartup=false` → açılış migrate+seed atlanır (varsayılan true — tek sunucu aynen);
**A8** `/live` (bağımlılıksız) + `/ready` (PG+DP zorunlu 503; Redis şimdilik degraded=200, A3'te zorunlulaşır).
İzole 5051 Production testi ✓ (Role=Api: worker'lar başlamadı, seed atlandı, 3 uç doğru; DP roundtrip DB'den ✓).
Varsayılanlarla davranış birebir bugünkü gibi — restart güvenli. SIRADA: A3 (device state→Redis) + A4 (login
sayacı→Redis + CF-Connecting-IP) → A9 → A5/A6 (A0 mount ile) → A10 → A-T. Plan: FAZ 10, `docs/acik-isler-yol-haritasi.md`.
...
rehber 11-tedarik bölümü (70 sayfa). İzole 5051 ✓ (negatifler dahil; test PO silindi).
**T2 MAL KABUL UYGULANDI (2026-08-24) ⚠️ restart bekliyor; SIRADA T3 Etiket Şablonları:** receipt_batches/_items/
...
(73 sayfa). İzole 5051 ✓ (test verileri silindi).
**T4 REVİZYONU (2026-08-24, kullanıcı kurgusu) UYGULANDI ⚠️ restart bekliyor:** ayrıştırma fizikî/sistem dışı;
ETİKET BASIMI ayrı+keyfî (sayım üretmez): /procurement/labels yığın listesi → Tümünü Yazdır (çoklu deste
/yazdir/etiket?items=vid:adet,... ≤50 ürün/≤2000 etiket); GERÇEK SAYIM = depoya teslim okutması:
...
F2+F3 de UYGULANDI; **F4 CANLI ✓ (2026-08-04): MIS0000041 eski sisteme yazıldı
(legacy Id 934178099, tutarlar/KDV/üye tekilleştirme doğrulandı)**. Kritik dersler:
form binding VİRGÜL kültürü (DecimalComma=true — nokta 100× yazdırdı, hatalı kayıt
silinip yeniden gönderildi), member.address zorunlu, DataRow null korumasız alanlar,
eşleşme anahtarı kalem Sku'su değil VARYANT BARKODU. Kalan gözlem: ilk gerçek KAPIDA
...
- **Kampanya F4 UYGULANDI ⚠️ prod restart bekliyor (2026-07-31, commit c2687f7) — AKIŞ UÇTAN UCA TAMAM (F0→F4):** Kampanyalı fiyat artık SATIN ALMAYA yansıyor (sunucu-taraflı, F3 kartıyla aynı hesap; fraud-güvenli). CheckoutCommand `IProductCampaignResolver.ResolveCartAsync` → ürün-bazlı kampanyalı birim fiyat (subtotal+OrderItem) + sepet-seviyesi indirim (buy_x_get_y/min_cart, CampaignEngine → kupon indirimine); uygulanan kampanyalar sipariş notunda. İzole test: ürün-bazlı %25 (29.99→22.49) + 1-alana-1-bedava (cartDiscount 29.99), öncelik doğru/çift sayım yok. **Kampanya modülü (tip tanımı→platform kampanya→ürün ilişkilendirme→kart/detay gösterim→checkout) uçtan uca çalışır.** Detay: memory/project_kampanya_uctan_uca_plani_2026-07-31.md.
- **Kampanya F3 UYGULANDI ⚠️ prod restart bekliyor (2026-07-31, commit ee49594) — KAMPANYALAR İLK KEZ SİTEDE:** Tüm listelemelerde (kategori SSR+continuation, ana sayfa carousel, arama, son-baktıklarım) + ürün detayında kampanya bandı (adı/rozeti) + ürün-bazlı kampanyalı fiyat (F2 resolver post-cache; PageComposer cache c3). discount(selected/koşulsuz/%/₺)→fiyat, diğerleri yalnız bant (cart_only, gerçek indirim F4 checkout). İzole test: -%30 kampanya kategori/carousel/detayda doğru. SIRADA F4 (checkout). Detay: memory/project_kampanya_uctan_uca_plani_2026-07-31.md.
- **Kampanya F2 UYGULANDI ⚠️ prod restart bekliyor (2026-07-31, commit 8f29a38):** IProductCampaignResolver — ürün başına etkin kampanya (platform+kapsam+dışlama+öncelik) + CampaignPricing.EffectivePrice (F3 vitrin/F4 checkout ortak). discount(applyTo=selected+koşulsuz+%/₺)→ürün-bazlı fiyat; diğerleri cart_only. Şema değişikliği yok. İzole test: kapsam/platform/öncelik/cart_only ✓. SIRADA F3 (kart/detay gösterim) → F4 checkout. Detay: memory/project_kampanya_uctan_uca_plani_2026-07-31.md.
- **Kampanya F1 UYGULANDI ⚠️ prod restart bekliyor (2026-07-31, commit e232aa0):** Platform kampanya oluşturma + kategori-tarzı ürün ilişkilendirme, detay sayfası (/promotion/campaigns/:id, sekmeli: Genel + SettingsSchema'dan üretilen Parametreler + Ürün Kapsamı FillType/FilterBuilder/manuel). Campaign: FirmPlatformId + FillType(all/manual/filter/mixed) + FilterDef; CampaignProductMaterializer (kategori Sync aynası, ProductFilterHelper) → prm_campaign_products; Settings tip şablonuna göre doğrulanır; Kopyala. Migration canlı DB'de (0 kampanya). İzole test: all→0/manual→2/filter→3090 materyalize ✓. ⚠️ ürün→varyant çözümü+motor entegrasyonu F2'de. Detay: memory/project_kampanya_uctan_uca_plani_2026-07-31.md.
- **Kampanya Faz 0 BACKEND UYGULANDI ⚠️ prod restart bekliyor (2026-07-31, commit e0011a5):** Kampanya tipleri `definition.campaign_types`'a taşındı (platformdan bağımsız yetenek); 6 birleştirilmiş parametrik tip (discount/buy_x_get_y/cross_group_gift/bundle/free_shipping/review_reward) SettingsSchema JSON şablonlarıyla seed edildi; `CampaignSchemaField` alan şekli. Migration canlı DB'ye uygulandı (0 kampanya). ⚠️ restart öncesi eski binary admin kampanya tip-ucunda 500 (kampanyalar kullanılmıyor). SIRADA: tip yönetim ekranı (K16) → F1. Taslak `docs/kampanya-tip-sablonlari-taslak.md`; detay `memory/project_kampanya_uctan_uca_plani_2026-07-31.md`.
- **Kampanya uçtan uca akış — PLAN (2026-07-31):** kampanya→ürün ilişkilendir→liste/detay göster→fiyat hesapla→checkout UÇTAN UCA çalışmıyor; veri modeli+sepet motoru+admin CRUD var ama ürün ilişkilendirme/storefront gösterim/checkout entegrasyonu YOK. Plan `docs/kampanya-uctan-uca-plani.md` (F1 ürün ilişkilendirme→F2 çözümleme servisi→F3 kart/detay→F4 checkout); kullanıcı "önce plan" dedi, KOD YOK. Detay: `memory/project_kampanya_uctan_uca_plani_2026-07-31.md`.
- **Ürün videosu + yorum/puan aktarımı (MigrationTool Faz 29) ✅ CANLIDA doğrulandı (2026-07-31, kullanıcı gördü):** Eski sistemde aktarılmamış veriler geldi — **866 ürün videosu** (hotlink) + **19.437 yorum** (13.562 onaylı→sitede görünür, 5.875 onaysız→moderasyon havuzu). Kart video rozeti + puan/yorum artık tüm listelemelerde (kategori SSR+continuation+carousel). `dotnet run 29 dry`=rapor. ⚠️ AÇIK: yorum fotoğrafları (319) kaynak barındırma adresi çözülemediği için ATLANDI — kullanıcı eski `GUID.jpg` görsellerinin sunucu/klasörünü verirse ayrı kısa geçişle eklenir. Ürün kartı zenginleştirme öğelerinden #2 (video) + #7 (puan/yorum) böylece TAMAM. Detay: `memory/project_video_yorum_aktarimi_faz29_2026-07-31.md`.
- **Ürün kartı zenginleştirme — Faz 1 (indirim fiyatı) CANLIDA ⚠️ restart bekliyor (2026-07-31):** ptplus tasarım kartındaki fiyat senaryolarından gerçek verisi olan öğe — satış fiyatı + üstü çizili eski fiyat + `-%X` rozeti (CompareAtPrice > satış fiyatı; "varsa yazdırılır") — tüm listelemelerde: kategori SSR + sonsuz-kaydırma continuation + ana sayfa carousel + son-baktıklarım + arama. Yolda düzeltilen 2 gizli hata: `PageBlockSourceResolver.KanalKartinaCevir` CompareAtPrice'ı null geçiyordu; paylaşımlı Redis cache eski serileştirmeyle indirimi maskeliyordu → kategori cache v9, sayfa-paketi cache c2. **Kalan tasarım öğeleri (Süper Fırsat, kampanya badge/indirim hesabı, sosyal kanıt sayaçları, Sepette/Plus fiyatı, fotolu-yorum ikonu) SONRAKİ FAZA ertelendi (kullanıcı kararı 2026-07-31) — her biri veri kaynağı/altyapı ister; öğe→veri tablosu `memory/project_urun_karti_zenginlestirme_2026-07-31.md`.**
