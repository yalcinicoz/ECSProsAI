...
- **Ürün videosu + yorum/puan aktarımı (MigrationTool Faz 29) ⚠️ restart bekliyor (2026-07-31):** Eski sistemde aktarılmamış veriler geldi — **866 ürün videosu** (hotlink) + **19.437 yorum** (13.562 onaylı→sitede görünür, 5.875 onaysız→moderasyon havuzu). Kart video rozeti + puan/yorum artık tüm listelemelerde (kategori SSR+continuation+carousel). `dotnet run 29 dry`=rapor. Yorum fotoğrafları (319) kaynak barındırma adresi çözülemediği için ATLANDI (bulununca ayrı geçiş). Ürün kartı zenginleştirme öğelerinden #2 (video) + #7 (puan/yorum) böylece TAMAM. Detay: `memory/project_video_yorum_aktarimi_faz29_2026-07-31.md`.
