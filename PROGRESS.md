**▶ OTURUM SONU DURUMU (2026-09-09) — YENİ OTURUM BURADAN DEVAM ETSİN:** Aktif alan **Mobil API**. Mobil ekibin 2026-09-09 istek listesinde **M0-M4 tamam** (staging eşitleme · sepet fiyat sözleşmesi + çifte indirim düzeltmesi · `checkout/preview` · ürün detayı tutarlılığı · durum etiketleri/lookups) — hepsi `publish` VE `publish-staging`'e aynı binary olarak yazıldı. **⚠️ M3 + M4'ün restart'ı BEKLİYOR:** `sudo systemctl restart ecspros && sudo systemctl restart ecspros-staging`. **SIRADAKİ İŞ: M5** — `discountPercent` yayılımı (liste/arama/favori/gezilen/koleksiyon), `cms/pages/by-slug/sss` alias'ı (gerçek slug `sik-sorulan-sorular`, kod `kurumsal-sss`), misafir `viewed-products` (MK2 kararı: cihaz token'ıyla), adres mahalle alanı (canlıda 97 adresin 67'sinde dolu — web formu zorunlu tutmuyor), eski fiyat alanlarının "kaldırılacak" işaretlenmesi (MK3). Plan ve madde madde durum: `docs/mobil-backend-istekleri-2026-09-09.md`.
