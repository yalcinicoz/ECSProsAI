...
| 4 | 🏪 **Satıcı paneli** | `satici/` + `/api/supplier/*` | **TAMAMLANDI ⚠️ restart+nginx+DNS bekliyor (2026-08-11)**: ürün ekleme/düzenleme (onay akışlı), fiyat&stok, siparişler+kargo+fatura, mali durum, kampanya opt-in, Hesabım (kargo modu); partner API ile AYNI komutlar; subdomain satici.misharitalia.com | DNS A kaydı + nginx restart (kullanıcı) → kullanıcı kabul testi | `docs/satici-paneli-tasarimi.md` |
