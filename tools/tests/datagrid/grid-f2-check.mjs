...
// ── Masaüstü ──
{
  ...
  // gelişmiş panel: ödeme durumu çoklu seçim + tutar > 1000
  await page.click('button:has-text("Gelişmiş")')
  await page.click('button[aria-label="Ödeme durumu"]')
  ...
  await page.selectOption('select[aria-label="Tutar operatör"]', 'gt')
  await page.fill('input[aria-label="Tutar"]', '1000'); await page.keyboard.press('Enter')
  await page.waitForTimeout(400)
  ok('sayı filtresi f.total=gt:1000', decodeURIComponent(page.url()).includes('f.total=gt:1000'), page.url())
  ok('Gelişmiş rozetinde 2 aktif', (await page.locator('button:has-text("Gelişmiş")').innerText()).includes('2'))
