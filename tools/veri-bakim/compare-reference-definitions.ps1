...
try {
 ...
 $null=$snapshot|ConvertFrom-Json
...
$sql=Get-Content -Raw (Join-Path $PSScriptRoot 'compare-reference-definitions.sql')
if($Action -ne 'Inspect'){$sql=(Get-Content -Raw (Join-Path $PSScriptRoot 'fill-reference-group-links.sql'))+"`n"+$sql}
