...
$aiPreviousConnection = $env:ECSPROS_TEST_DB
...
try {
    ...
    if (15433 -in [Net.NetworkInformation.IPGlobalProperties]::GetIPGlobalProperties().GetActiveTcpListeners().Port) { throw 'Existing listener preserved' }
    ...
    $aiArgs = @('-N', '-T', '-o', 'BatchMode=yes', '-o', 'StrictHostKeyChecking=yes', '-o', 'ExitOnForwardFailure=yes',
        ...
        '-L', '127.0.0.1:15433:192.168.0.241:5432', "$($aiNode.Username)@$($aiNode.Host)")
    ...
    for ($aiAttempt = 0; $aiAttempt -lt 100; $aiAttempt++) {
        ...
        if (15433 -in [Net.NetworkInformation.IPGlobalProperties]::GetIPGlobalProperties().GetActiveTcpListeners().Port) { $aiReady = $true; break }
        Start-Sleep -Milliseconds 200
    }
    if (-not $aiReady) { throw 'Tunnel not ready' }
    $env:ECSPROS_TEST_DB = $aiConnection.get_ConnectionString()
    $aiStage = 'rollback-only database tests'
    & dotnet test (Join-Path $aiRoot 'tests/ECSPros.Api.Tests/ECSPros.Api.Tests.csproj') --no-build --no-restore --filter 'FullyQualifiedName~EtkinYetkiServisiDbTests|FullyQualifiedName~PermissionKatalogSenkronuDbTests|FullyQualifiedName~NotificationInboxDbTests' --verbosity minimal
    $aiExitCode = $LASTEXITCODE
} catch {
    Write-Output ('Stopped at ' + $aiStage + ': ' + $_.Exception.GetType().Name)
} finally {
    $env:ECSPROS_TEST_DB = $aiPreviousConnection
